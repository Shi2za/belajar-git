 ...
 */
package hotelfx.view.feature_tools.tools_stock;
 ...
 * @author Andreas
 */
public class ToolsStockController implements Initializable{
    ...
    private void setTableDataEquipmentStock(){
      ...
      TableColumn<TblItemLocation,String>itemCode = new TableColumn("ID");
       ...
       TableColumn<TblItemLocation,String>itemName = new TableColumn("Nama Peralatan");
      ...
      tableView.setItems(loadAllDataToolsStock());
...
//        AnchorPane.setLeftAnchor(cft, 15.0);
    ...
    private ObservableList<TblItemLocation>loadAllDataToolsStock(){
        List<TblItemLocation> list = parentController.getFToolsManager().getAllDataToolsStock();
    ...
    public String getNameLocation(TblLocation location){
     //System.out.println("Hsl>>"+location.getRefLocationType().getIdtype());
      ...
      {
          case 0:
          List<TblRoom> room = parentController.getFToolsManager().getRoomByIdLocation(location.getIdlocation());
          for(TblRoom getRoom : room){
            name = getRoom.getRoomName();
          }
          break;
          case 1:
          List<TblLocationOfWarehouse> warehouse = parentController.getFToolsManager().getWarehouseByIdLocation(location.getIdlocation());
          for(TblLocationOfWarehouse getWarehouse : warehouse){
             name = getWarehouse.getWarehouseName();
          }
          break;
          case 2:
          List<TblLocationOfLaundry> laundry = parentController.getFToolsManager().getLaundryByIdLocation(location.getIdlocation());
          for(TblLocationOfLaundry getLaundry : laundry){
             name = getLaundry.getLaundryName();
          }
          break;
          case 3:
          List<TblSupplier> supplier = parentController.getFToolsManager().getSupplierByIdLocation(location.getIdlocation());
          for(TblSupplier getSupplier : supplier){
             name = getSupplier.getSupplierName();
          }
          break;
          case 4:
          List<TblLocationOfBin> bin = parentController.getFToolsManager().getBinByIdLocation(location.getIdlocation());
    ...
    public ToolsStockController(FeatureToolsController parentController)
    {
