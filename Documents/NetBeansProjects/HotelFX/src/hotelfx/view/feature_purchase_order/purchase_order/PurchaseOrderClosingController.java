...
public class PurchaseOrderClosingController implements Initializable {
    ...
    private void initFormDataClosing() {
...
//            spFormDataPO.pseudoClassStateChanged(onScrollPseudoClass, nowScroll);
    ...
    private void setSelectedDataToInputForm() {
        //label code
        lblCodeData.setText((parentController.selectedData.getCodePo() != null
                ? (parentController.selectedData.getCodePo()
                + (parentController.selectedData.getTblPurchaseRequest() != null
                        ? (" (" + parentController.selectedData.getTblPurchaseRequest().getCodePr() + ") ")
                        : "")
                + (parentController.selectedData.getTblSupplier() != null
                        ? (" - " + parentController.selectedData.getTblSupplier().getSupplierName())
                        : ""))
                + " : "
                + parentController.selectedData.getRefPurchaseOrderItemArriveStatus().getStatusName()
                + " - "
                + parentController.selectedData.getRefPurchaseOrderPaymentStatus().getStatusName()
                : ""));
        //data PO
    ...
    private void setSelectedDataToInputFormPO() {
        if (parentController.selectedData.getPodueDate() != null) {
            txtDueDate.setText(ClassFormatter.dateFormate.format(parentController.selectedData.getPodueDate()));
        } else {
            txtDueDate.setText("-");
        }

        txtPOPaymentTypeInformation.textProperty().bindBidirectional(parentController.selectedData.popaymentTypeInformationProperty());
        txtPONote.textProperty().bindBidirectional(parentController.selectedData.canceledNoteProperty());

        Bindings.bindBidirectional(txtDiscount.textProperty(), parentController.selectedData.nominalDiscountProperty(), new ClassFormatter.CBigDecimalStringConverter());
        parentController.selectedData.nominalDiscountProperty().addListener((obs, oldVal, newVal) -> {
            if (newVal != null) {
                refreshDataBill();
            }
        });

        spnTaxPercentage.getValueFactory().setValue(parentController.selectedData.getTaxPecentage() != null
                ? parentController.selectedData.getTaxPecentage().doubleValue() * 100
                : 0);
        spnTaxPercentage.valueProperty().addListener((obs, oldVal, newVal) -> {
            if (newVal != null) {
                parentController.selectedData.setTaxPecentage(BigDecimal.valueOf(newVal / 100));
            }
        });
        parentController.selectedData.taxPecentageProperty().addListener((obs, oldVal, newVal) -> {
            if (newVal != null) {
                refreshDataBill();
            }
        });

        Bindings.bindBidirectional(txtDeliveryCost.textProperty(), parentController.selectedData.deliveryCostProperty(), new ClassFormatter.CBigDecimalStringConverter());
        parentController.selectedData.deliveryCostProperty().addListener((obs, oldVal, newVal) -> {
            if (newVal != null) {
                refreshDataBill();
            }
        });
    ...
    private String getCodeInvoice() {
        if (parentController.selectedData != null
                && parentController.selectedData.getTblHotelPayable() != null
                && parentController.selectedData.getTblHotelPayable().getTblHotelInvoice() != null) {
            return parentController.selectedData.getTblHotelPayable().getTblHotelInvoice().getCodeHotelInvoice();
        }
        return "-";
    }
    ...
    private String getPaymentDueDate() {
        if (parentController.selectedData != null
                && parentController.selectedData.getTblHotelPayable() != null
                && parentController.selectedData.getTblHotelPayable().getTblHotelInvoice() != null) {
            return ClassFormatter.dateFormate.format(parentController.selectedData.getTblHotelPayable().getTblHotelInvoice().getDueDate());
        }
        return "-";
    }
    ...
    private BigDecimal getTotalHotelPayableBySupplier() {
        if (parentController.selectedData != null
                && parentController.selectedData.getTblHotelPayable() != null
                && parentController.selectedData.getTblHotelPayable().getTblHotelInvoice() != null) {
            return parentController.selectedData.getTblHotelPayable().getHotelPayableNominal();
        }
    ...
    private BigDecimal getTotalHotelPayableBySystem() {
        ...
        if (parentController.selectedData != null
            ...
            result = result.subtract(parentController.selectedData.getNominalDiscount());
            result = ((new BigDecimal("1")).add((parentController.selectedData.getTaxPecentage().divide(new BigDecimal("1"))))).multiply(result);
            result = result.add(parentController.selectedData.getDeliveryCost());
        }
        return result;
    }
    ...
    private BigDecimal getTotalHotelPayment() {
        ...
        if (parentController.selectedData != null
                && parentController.selectedData.getTblHotelPayable() != null
                && parentController.selectedData.getTblHotelPayable().getTblHotelInvoice() != null) {
            List<TblHotelFinanceTransactionHotelPayable> list = parentController.getService().getAllDataHotelFinanceTransactionHotelPayableByIDHotelPayable(parentController.selectedData.getTblHotelPayable().getIdhotelPayable());
    ...
    private String getTotalHotelPaymentStatus() {
        if (parentController.selectedData != null
                && parentController.selectedData.getTblHotelPayable() != null
                && parentController.selectedData.getTblHotelPayable().getTblHotelInvoice() != null) {
            return "(" + parentController.selectedData.getTblHotelPayable().getRefFinanceTransactionStatus().getStatusName() + ")";
        }
        return "";
    }
    ...
    private BigDecimal calculationTax() {
        return (parentController.selectedData.getTaxPecentage()).multiply(calculationSubTotal().subtract(parentController.selectedData.getNominalDiscount()));
    }

    private BigDecimal calculationTotal() {
        return (((new BigDecimal("1")).add(parentController.selectedData.getTaxPecentage())).multiply(calculationSubTotal()
                .subtract(parentController.selectedData.getNominalDiscount())))
                .add(parentController.selectedData.getDeliveryCost());
    }
    ...
    private void dataPOClosingSaveHandle() {
        ...
        if (alert.getResult() == ButtonType.OK) {
            //dummmy
            TblPurchaseOrder dummySelectedData = new TblPurchaseOrder(parentController.selectedData);
            dummySelectedData.setRefPurchaseOrderStatus(new RefPurchaseOrderStatus(dummySelectedData.getRefPurchaseOrderStatus()));
            if (parentController.getService().updateClosingDataPurchaseOrder(dummySelectedData)) {
                ...
                HotelFX.showAlert(Alert.AlertType.ERROR, "GAGAL", "Data PO gagal ditutup..!!", parentController.getService().getErrorMessage());
            }
        }
    }
    ...
    private List<PurchaseOrderDetailCreated> loadAllDataPODetailCreated() {
        ...
        //data po - detail
        List<TblPurchaseOrderDetail> poList = parentController.getService().getAllDataPurchaseOrderDetailByIDPurchaseOrder(parentController.selectedData.getIdpo());
        for (TblPurchaseOrderDetail poData : poList) {
            //set data po
            poData.setTblPurchaseOrder(parentController.selectedData);
    ...
    private BigDecimal getAllowancePercentage(TblMemorandumInvoiceDetail data) {
        SysDataHardCode sdhc = parentController.getService().getDataSysDataHardCode(28);   //ReceivingPercentageAllowance = '28'
        if (sdhc != null
    ...
    private BigDecimal getQuatityItemHasBeenGet(TblMemorandumInvoiceDetail data) {
        if (data != null
                && data.getTblItem() == null) {  //isn't a bonus
            BigDecimal result = new BigDecimal("0");
            List<TblMemorandumInvoiceDetail> listMD = parentController.getService().getAllDataMemorandumInvoiceDetailByIDPurchaseOrderAndIDSupplierItem(data.getTblMemorandumInvoice().getTblPurchaseOrder().getIdpo(),
    ...
    private BigDecimal getQuantityItemHasBeenRetur(TblMemorandumInvoiceDetail data) {
        if (data != null
            ...
            List<TblMemorandumInvoiceDetail> listMD = parentController.getService().getAllDataMemorandumInvoiceDetailByIDPurchaseOrderAndIDSupplierItem(data.getTblMemorandumInvoice().getTblPurchaseOrder().getIdpo(),
                    data.getTblSupplierItem().getIdrelation());
            for (TblMemorandumInvoiceDetail dataMD : listMD) {
                List<TblReturDetail> listRD = parentController.getService().getAllDataReturDetailByIDMemorandumInvoiceAndIDSupplierItem(dataMD.getTblMemorandumInvoice().getIdmi(),
                        data.getTblSupplierItem().getIdrelation());
    ...
    private List<MIDetail> loadAllDataMIDetail() {
        ...
        //not a bonus
        List<TblPurchaseOrderDetail> poDetails = parentController.getService().getAllDataPurchaseOrderDetailByIDPurchaseOrder(parentController.selectedData.getIdpo());
        ...
        //a bonus
        List<TblMemorandumInvoice> mis = parentController.getService().getAllDataMemorandumInvoiceByIDPurchaseOrder(parentController.selectedData.getIdpo());
    ...
    public String getLocationName(TblLocation location) {
        ...
        switch (location.getRefLocationType().getIdtype()) {
            case 0:
                TblRoom room = parentController.getService().getDataRoomByIDLocation(location.getIdlocation());
                if (room != null) {
                    name = room.getRoomName();
                }
                break;
            case 1:
                TblLocationOfWarehouse warehouse = parentController.getService().getDataWarehouseByIDLocation(location.getIdlocation());
                if (warehouse != null) {
                    name = warehouse.getWarehouseName();
                }
                break;
            case 2:
                TblLocationOfLaundry laundry = parentController.getService().getDataLaundryByIDLocation(location.getIdlocation());
                if (laundry != null) {
                    name = laundry.getLaundryName();
                }
                break;
            case 4:
                TblLocationOfBin bin = parentController.getService().getDataBinByIDLocation(location.getIdlocation());
                if (bin != null) {
                    name = bin.getBinName();
                }
                break;
    ...
    private List<MIReturDetail> loadAllDataMIReturDetail() {
        ...
        for (TblMemorandumInvoice mi : mis) {
            ...
            for (TblMemorandumInvoiceDetail mid : mids) {
                if (mid.getTblItem() != null) {   //bonus
                    if (mid.getTblItem().getPropertyStatus()) {   //Property barcode
                        List<TblMemorandumInvoiceDetailPropertyBarcode> midpbs = parentController.getService().getAllDataMemorandumInvoiceDetailPropertyBarcodeByIDMemorandumInvoiceDetail(mid.getIddetail());
                        ...
                        if (mid.getTblItem().getConsumable()) {   //Consumable
                            List<TblMemorandumInvoiceDetailItemExpiredDate> midieds = parentController.getService().getAllDataMemorandumInvoiceDetailItemExpiredDateByIDMemorandumInvoiceDetail(mid.getIddetail());
            ...
            //retur detail
            List<TblReturDetail> returds = parentController.getService().getAllDataReturDetailByIDMemorandumInvoice(mi.getIdmi());
            for (TblReturDetail returd : returds) {
                if (returd.getTblRetur().getRefReturStatus().getIdstatus() == 1) {  //Disetujui = '1'
                    if (returd.getTblSupplierItem().getTblItem().getPropertyStatus()) {   //Property barcode
                        List<TblReturDetailPropertyBarcode> returdpbs = parentController.getService().getAllDataReturDetailPropertyBarcodeByIDReturDetail(returd.getIddetailRetur());
                        ...
                        if (returd.getTblSupplierItem().getTblItem().getConsumable()) {   //Consumable
                            List<TblReturDetailItemExpiredDate> returdieds = parentController.getService().getAllDataReturDetailItemExpiredDateByIDReturDetail(returd.getIddetailRetur());
    ...
    private List<TblHotelFinanceTransactionHotelPayable> loadAllDataHotelFinanceTransactionHotelPayable() {
        ...
        if (parentController.selectedData != null
                && parentController.selectedData.getTblHotelPayable() != null) {
            list = parentController.getService().getAllDataHotelFinanceTransactionHotelPayableByIDHotelPayable(parentController.selectedData.getTblHotelPayable().getIdhotelPayable());
        }
        return list;
    }
    ...
    public PurchaseOrderClosingController(PurchaseOrderController parentController) {
