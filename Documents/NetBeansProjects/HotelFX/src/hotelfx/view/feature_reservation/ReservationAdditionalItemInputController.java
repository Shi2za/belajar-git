...
public class ReservationAdditionalItemInputController implements Initializable {
    ...
    @FXML
    private AnchorPane ancReservationBillTypeLayout;
    private JFXCComboBoxTablePopup<RefReservationBillType> cbpReservationBillType;
    ...
    private void initFormDataAdditionalItem() {
        ...
        if (reservationController.selectedData.getRefReservationStatus().getIdstatus() == 1) {    //Reserved = '1'
            cbpReservationBillType.setVisible(false);
        } else {
            cbpReservationBillType.setVisible(true);
        }
    ...
    private void initDataAdditionalItemPopup() {
        ...
        TableView<RefReservationBillType> tableReservationBillType = new TableView<>();

        TableColumn<RefReservationBillType, String> rbTypeName = new TableColumn<>("Jenis Tagihan");
        rbTypeName.setCellValueFactory(cellData -> cellData.getValue().typeNameProperty());
        rbTypeName.setMinWidth(140);

        tableReservationBillType.getColumns().addAll(rbTypeName);

        ObservableList<RefReservationBillType> reservationBillTypeItems = FXCollections.observableArrayList(loadAllDataReservationBillType());

        cbpReservationBillType = new JFXCComboBoxTablePopup<>(
                RefReservationBillType.class, tableReservationBillType, reservationBillTypeItems, "", "Tagihan Pembayaran *", true, 200, 200
        );
        ...
        AnchorPane.setBottomAnchor(cbpReservationBillType, 0.0);
        AnchorPane.setLeftAnchor(cbpReservationBillType, 0.0);
        AnchorPane.setRightAnchor(cbpReservationBillType, 0.0);
        AnchorPane.setTopAnchor(cbpReservationBillType, 0.0);
        ancReservationBillTypeLayout.getChildren().clear();
        ancReservationBillTypeLayout.getChildren().add(cbpReservationBillType);
    }
    ...
    private void refreshDataPopup() {
        ...
        //Reservation Bill Type
        ObservableList<RefReservationBillType> reservationBillTypeItems = FXCollections.observableArrayList(loadAllDataReservationBillType());
        cbpReservationBillType.setItems(reservationBillTypeItems);

    }

    private List<RefItemType> loadAllDataItemType() {
        List<RefItemType> list = reservationController.getFReservationManager().getAllDataItemType();
        return list;
    }
    ...
    private List<RefReservationBillType> loadAllDataReservationBillType() {
        List<RefReservationBillType> list = reservationController.getFReservationManager().getAllDataReservationBillType();
        for (int i = list.size() - 1; i > -1; i--) {
            //remove data not used
            if (list.get(i).getIdtype() != 0 //Reservasi = '0'
                    && list.get(i).getIdtype() != 1) {   //Check Out = '1'
                list.remove(i);
            }
        }
        return list;
    ...
    private List<TblReservationRoomTypeDetail> loadAllDataReservationRoomTypeDetail() {
        return reservationController.dataReservationRoomTypeDetails;
    }
...
//            reservationController.selectedAdditionalItem.setItemQuantity(new BigDecimal("1"));
        ...
        cbpReservationBillType.valueProperty().bindBidirectional(reservationController.selectedAdditionalItem.refReservationBillTypeProperty());

        cbpItem.hide();
        cbpRRTD.hide();
        cbpReservationBillType.hide();
    }
...
//                ); //Item, Date(start), Date(end), Status = 'Reserved'
            ...
            if (alert.getResult() == ButtonType.OK) {
                ...
                for (RTDListDate dataDate : tabelAdditionalDate.getItems()) {
                    if (dataDate.getSelectedDate()) {
                        ...
                        if (reservationController.selectedData.getRefReservationStatus().getIdstatus() == 1) {   //Reserved = '1' -> reservation bill
                            setAdditionalItemDiscount(dataAddtional, reservationController.dataReservationBill);
                        } else {  //checkout bill
                            setAdditionalItemDiscount(dataAddtional, reservationController.dataCheckOutBill);
                        }
    ...
    private boolean checkDataInputDataAdditionalItem() {
        ...
        if (cbpReservationBillType.getValue() == null) {
            dataInput = false;
            errDataInput += "Jenis Tagihan Pembayaran : " + ClassMessage.defaultErrorNullValueMessage + " \n";
        }
        return dataInput;
    }
