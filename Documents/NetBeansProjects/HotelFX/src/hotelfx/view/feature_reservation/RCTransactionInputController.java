...
//            });
            ...
            if (reservationController.dataInputTransactionStatus.equals("checkout")) {
                lblServiceChargePercentage.setText("(" + ClassFormatter.decimalFormat.cFormat(reservationController.dataCheckOutBill.getServiceChargePercentage().multiply(new BigDecimal("100"))) + " %)");
                lblTaxPercentage.setText("(" + ClassFormatter.decimalFormat.cFormat(reservationController.dataCheckOutBill.getTaxPercentage().multiply(new BigDecimal("100"))) + " %)");
//            cbpHotelEvent.valueProperty().bindBidirectional(reservationController.selectedDataCheckOutBillHotelEvent.tblHotelEventProperty());
//            reservationController.selectedDataCheckOutBillHotelEvent.tblHotelEventProperty().addListener((obs, oldVal, newVal) -> {
//                if (newVal != null) {
//                    discountPercentage = newVal.getDiscountPercentage();
//                }
//                refreshDataBill();
    ...
    private void refreshDataBill() {
        if (reservationController.dataInputTransactionStatus.equals("reservation")) {
            txtBill.setText(ClassFormatter.currencyFormat.cFormat(reservationController.calculationTotalReservationRoom()
                    ...
                    .add(reservationController.calculationTotalAdditional(reservationController.dataInputTransactionStatus))));
            txtDiscount.setText(ClassFormatter.currencyFormat.cFormat(reservationController.calculationTotalDiscount(reservationController.dataInputTransactionStatus)));
            txtTotalServiceCharge.setText(ClassFormatter.currencyFormat.cFormat(reservationController.calculationTotalServiceCharge(reservationController.dataInputTransactionStatus)));
            txtTotalTax.setText(ClassFormatter.currencyFormat.cFormat(reservationController.calculationTotalTax(reservationController.dataInputTransactionStatus)));
            txtTotalBill.setText(ClassFormatter.currencyFormat.cFormat(reservationController.calculationTotalBillAfterRounding(reservationController.dataInputTransactionStatus)));
            lblRoundingValue.setText("(Pembulatan : " + ClassFormatter.currencyFormat.cFormat(reservationController.calculationTotalReservationTransactionRoundingValue(reservationController.dataInputTransactionStatus)) + ")");
            txtTotalPayment.setText(ClassFormatter.currencyFormat.cFormat(reservationController.calculationTotalReservationTransaction(reservationController.dataInputTransactionStatus)));
            txtTotalDiscountPayment.setText(ClassFormatter.currencyFormat.cFormat(reservationController.calculationTotalReservationTransactionDisountPayment(reservationController.dataInputTransactionStatus)));
            txtTotalRestOfBill.setText(ClassFormatter.currencyFormat.cFormat(reservationController.calculationTotalRestOfBill(reservationController.dataInputTransactionStatus)));
        } else {    //checkout
            if (reservationController.dataInputTransactionStatus.equals("checkout")) {
                txtBill.setText(ClassFormatter.currencyFormat.cFormat(reservationController.calculationTotalAdditional(reservationController.dataInputTransactionStatus)
...
//            reservationController.tableDataReservationTransaction.getTableView().setItems(FXCollections.observableArrayList(reservationController.dataReservationPayments));
            ...
            if (reservationController.dataInputTransactionStatus.equals("checkout")) {
                reservationController.selectedDataTransaction.setRefReservationBillType(reservationController.getFReservationManager().getDataReservationBillType(1));   //CheckOut = '1'
                if (dataInputRCTransactionInputDetailStatus == 0) {   //insert
                    reservationController.dataCheckOutPayments.add(reservationController.selectedDataTransaction);
                } else {  //udapte
                    for (int i = 0; i < reservationController.dataCheckOutPayments.size(); i++) {
                        if (reservationController.dataCheckOutPayments.get(i).equals(tableDataRCTransactionInputDetail.getTableView().getSelectionModel().getSelectedItem())) {
                            reservationController.dataCheckOutPayments.set(i, reservationController.selectedDataTransaction);
                            break;
                        }
                    }
                }
//            reservationController.tableDataCheckOutTransaction.getTableView().setItems(FXCollections.observableArrayList(reservationController.dataCheckOutPayments));
            } else {  //rco
                //RCO###
            }
        }
        ...
        reservationController.resetDataTransaction(reservationController.dataInputTransactionStatus);
//        if (reservationController.dataInputTransactionStatus.equals("reservation")) {
        chbHotelDiscount.setDisable((!useHotelEventAvaliable()));
//        } else {
//            chbHotelDiscount.setDisable(true);
        ...
        if (!ClassSetting.isMultipleDiscountAllowed) {
            for (TblReservationPaymentWithBankCard dataPaymentWithBankCard : reservationController.dataReservationPaymentWithBankCards) {
                if (dataPaymentWithBankCard.getTblBankEventCard() != null) {
                    if (reservationController.dataInputTransactionStatus.equals("reservation")
                            && dataPaymentWithBankCard.getTblReservationPayment().getRefReservationBillType().getIdtype() == 0) {    //Reservasi = '0'
                        available = false;
                        break;
                    }
                    if (reservationController.dataInputTransactionStatus.equals("checkout")
                            && dataPaymentWithBankCard.getTblReservationPayment().getRefReservationBillType().getIdtype() == 1) {    //Check Out = '1'
                        available = false;
                        break;
                    }
                    if (reservationController.dataInputTransactionStatus.equals("rco")
                            && dataPaymentWithBankCard.getTblReservationPayment().getRefReservationBillType().getIdtype() == 0) {    //Reservasi = '0'
                        available = false;
                        break;
                    }
...
//                dataGLID.setTotalDiscountNominal((long) reservationController.calculationRoomTypeDetailDiscount(dataRRTD));
            ...
            //data additional item
            for (TblReservationAdditionalItem dataAdditionalItem : reservationController.dataReservationAdditionalItems) {
                if (dataAdditionalItem.getRefReservationBillType().getIdtype() == 1) {    //CheckOut = '1'
                    //data guarantee letter - item detail
                    TblGuaranteeLetterItemDetail dataGLID = new TblGuaranteeLetterItemDetail();
                    dataGLID.setTblReservationPaymentWithGuaranteePayment(dataGL);
                    dataGLID.setCodeRtd(dataAdditionalItem.getTblReservationRoomTypeDetail().getCodeDetail());
                    ...
                    dataGLID.setTotalDiscountNominal((dataAdditionalItem.getItemCharge().multiply(dataAdditionalItem.getItemQuantity()))
                            .multiply(dataAdditionalItem.getDiscountPercentage().divide(new BigDecimal("100"))));
                    dataGLID.setServiceChargePercentage(reservationController.dataCheckOutBill.getServiceChargePercentage());
                    dataGLID.setTaxPercentage(reservationController.dataCheckOutBill.getTaxPercentage());
                    dataGLID.setDetailType("Item");
                    //add data to list
                    reservationController.dataGuaranteeLetterItemDetails.add(dataGLID);
                }
            ...
            //data additional service
            for (TblReservationAdditionalService dataAdditionalService : reservationController.dataReservationAdditionalServices) {
                if (dataAdditionalService.getRefReservationBillType().getIdtype() == 1) {    //CheckOut = '1'
                    //data guarantee letter - item detail
                    TblGuaranteeLetterItemDetail dataGLID = new TblGuaranteeLetterItemDetail();
                    dataGLID.setTblReservationPaymentWithGuaranteePayment(dataGL);
                    dataGLID.setCodeRtd(dataAdditionalService.getTblReservationRoomTypeDetail().getCodeDetail());
    ...
    public void dataRCTTransactionSaveHandle() {
        if ((reservationController.dataInputTransactionStatus.equals("reservation") && reservationController.dataReservationPayments.isEmpty())
                || (reservationController.dataInputTransactionStatus.equals("checkout") && reservationController.dataCheckOutPayments.isEmpty())) {
            ...
            if (reservationController.calculationTotalRestOfBill(reservationController.dataInputTransactionStatus).compareTo(new BigDecimal("0")) != 0) {
                ...
                if (alert.getResult() == ButtonType.OK) {
                    ...
                    if (reservationController.dataInputTransactionStatus.equals("reservation")) {
                        ...
                        reservationController.selectedData.setDepartureTime(Timestamp.valueOf(LocalDateTime.now()));    //set departure date-time
...
//                }
    ...
    private void setTableDataRCTransactionInputDetail() {
        ...
        if (reservationController.dataInputTransactionStatus.equals("reservation")) {
            ...
            tableView.setItems(reservationController.loadAllDataCheckOutTransaction());
        }
                ...
                for (TblReservationPayment data : reservationController.dataCheckOutPayments) {
                    if (data.equals(tableDataRCTransactionInputDetail.getTableView().getSelectionModel().getSelectedItem())) {
                        if (data.getIdpayment() == 0L) {
                            ...
                            HotelFX.showAlert(Alert.AlertType.WARNING, "DATA TIDAK DAPAT DIHAPUS", "Data yang sudah lewat tidak dapat dihapus..!", "", reservationController.dialogStage);
                        }
                        break;
                    }
                }
...
//            }
    ...
    private void showRCTransactionInputDetailDialog() {
        try {
            ...
            loader.setLocation(HotelFX.class.getResource("view/feature_reservation/RCTransactionInputDetailDialog.fxml"));
