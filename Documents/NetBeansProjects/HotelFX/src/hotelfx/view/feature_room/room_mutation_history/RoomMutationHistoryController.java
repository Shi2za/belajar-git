 ...
 */
package hotelfx.view.feature_room.room_mutation_history;
 ...
 * @author Andreas
 */
public class RoomMutationHistoryController implements Initializable {
    
    @FXML
    private AnchorPane tableDataRoomMutationHistoryLayout;
    ...
    private TableView tableDataRoomMutationHistory;

    private void initTableDataRoomMutationHistory() {
        setTableDataRoomMutationHistory();

        ancBodyLayout.getChildren().clear();
        AnchorPane.setTopAnchor(tableDataRoomMutationHistory, 15.0);
        AnchorPane.setLeftAnchor(tableDataRoomMutationHistory, 15.0);
        AnchorPane.setBottomAnchor(tableDataRoomMutationHistory, 15.0);
        AnchorPane.setRightAnchor(tableDataRoomMutationHistory, 15.0);
        ancBodyLayout.getChildren().add(tableDataRoomMutationHistory);
    }

    private void setTableDataRoomMutationHistory() {
...
//        locationDestination.getColumns().addAll(locationDestinationType,nameLocationDestination);
        ...
        tableDataRoomMutationHistory = tableView;
        
        //set filter
        cft = new ClassFilteringTable<>(
                ClassDataMutation.class,
                tableDataRoomMutationHistory,
                tableDataRoomMutationHistory.getItems());
    ...
    public String getNameLocation(TblLocation location) {
     //System.out.println("Hsl>>"+location.getRefLocationType().getIdtype());
        ...
        switch (location.getRefLocationType().getIdtype()) {
            case 0:
                List<TblRoom> room = parentController.getFRoomManager().getRoomByIdLocation(location.getIdlocation());
                for (TblRoom getRoom : room) {
                    name = getRoom.getRoomName();
                }
                break;
            case 1:
                List<TblLocationOfWarehouse> warehouse = parentController.getFRoomManager().getWarehouseByIdLocation(location.getIdlocation());
                for (TblLocationOfWarehouse getWarehouse : warehouse) {
                    name = getWarehouse.getWarehouseName();
                }
                break;
            case 2:
                List<TblLocationOfLaundry> laundry = parentController.getFRoomManager().getLaundryByIdLocation(location.getIdlocation());
                for (TblLocationOfLaundry getLaundry : laundry) {
                    name = getLaundry.getLaundryName();
                }
                break;
            case 3:
                List<TblSupplier> supplier = parentController.getFRoomManager().getSupplierByIdLocation(location.getIdlocation());
...
//                break;
    ...
    public ObservableList<ClassDataMutation> loadAllDataMutationHistory() {
        ...
        List<TblItemMutationHistory> imhs = parentController.getFRoomManager().getAllDataItemMutationHistory();
        for (TblItemMutationHistory imh : imhs) {
            if (imh.getTblItem().getPropertyStatus()) {   //Property
                TblItemMutationHistoryPropertyBarcode imhpb = parentController.getFRoomManager().getDataItemMutationHistoryPropertyBarcodeByIDItemMutationHistory(imh.getIdmutation());
                if (imhpb != null) {
                    ...
                    classDataMutation.getMutationHistoryPropertyBarcode().setTblPropertyBarcode(parentController.getFRoomManager().getDataPropertyBarcode(classDataMutation.getMutationHistoryPropertyBarcode().getTblPropertyBarcode().getIdbarcode()));
                    list.add(classDataMutation);
                }
            } else {
                if (imh.getTblItem().getConsumable()) {   //Consumable
                    List<TblItemMutationHistoryItemExpiredDate> imhieds = parentController.getFRoomManager().getAllDataItemMutationHistoryItemExpiredDateByIDItemMutationHistory(imh.getIdmutation());
                    for (TblItemMutationHistoryItemExpiredDate imhied : imhieds) {
                        ClassDataMutation classDataMutation = new ClassDataMutation();
                        classDataMutation.setMutationHistory(imh);
                        classDataMutation.setMutationHistoryItemExpiredDate(imhied);
                        classDataMutation.getMutationHistoryItemExpiredDate().setTblItemExpiredDate(parentController.getFRoomManager().getDataItemExpiredDate(classDataMutation.getMutationHistoryItemExpiredDate().getTblItemExpiredDate().getIditemExpiredDate()));
                        list.add(classDataMutation);
                    }
                } else {
                    ClassDataMutation classDataMutation = new ClassDataMutation();
    ...
    public void initialize(URL location, ResourceBundle resources) {
        ...
        initTableDataRoomMutationHistory();
        
    }

    public RoomMutationHistoryController(FeatureRoomController parentController) {
