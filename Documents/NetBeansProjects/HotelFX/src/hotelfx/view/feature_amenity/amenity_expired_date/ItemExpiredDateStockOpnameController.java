...
public class ItemExpiredDateStockOpnameController implements Initializable {
    ...
    private void setSelectedDataToInputForm() {
        ...
        txtCodeSO.setText(amenityExpiredDateController.selectedDataSOIED.getCodeStockOpnameIed() != null
                ? amenityExpiredDateController.selectedDataSOIED.getCodeStockOpnameIed() : "-");
        txtSODate.setText(ClassFormatter.dateTimeFormate.format(amenityExpiredDateController.selectedDataSOIED.getStockOpnameIeddate()));
        txtCodeItemName.setText(amenityExpiredDateController.selectedDataSOIED.getTblItem().getCodeItem()
                + " - "
                + amenityExpiredDateController.selectedDataSOIED.getTblItem().getItemName());
        txtItemExpiredDate.setText(ClassFormatter.dateFormate.format(amenityExpiredDateController.selectedDataSOIED.getExpiredDate()));

        txtSONote.textProperty().bindBidirectional(amenityExpiredDateController.selectedDataSOIED.stockOpnameIednoteProperty());
    ...
    private void refreshDataStock() {
        amenityExpiredDateController.selectedDataSOIED.setItemQuantityReal(new BigDecimal(calculationItemQuantityReal()));
        
        txtItemStockSystem.setText(ClassFormatter.decimalFormat.cFormat(amenityExpiredDateController.selectedDataSOIED.getItemQuantitySystem()));
        txtItemStockReal.setText(ClassFormatter.decimalFormat.cFormat(amenityExpiredDateController.selectedDataSOIED.getItemQuantityReal()));
        txtDifferenceStock.setText(ClassFormatter.decimalFormat.cFormat((amenityExpiredDateController.selectedDataSOIED.getItemQuantityReal()
                .subtract(amenityExpiredDateController.selectedDataSOIED.getItemQuantitySystem()))));
    }
    ...
    private List<ItemExpiredDateSelected> loadAllDataItemExpiredDateSelected(){
        ...
        List<TblItemExpiredDate> tempList = amenityExpiredDateController.getService().getAllDataItemExpiredDateByIDItemAndItemExpiredDateAndItemExpiredDateStatus(
                    amenityExpiredDateController.selectedDataSOIED.getTblItem().getIditem(),
                    amenityExpiredDateController.selectedDataSOIED.getExpiredDate(),
                    1); //Ada = '1'
        for(TblItemExpiredDate tempData : tempList){
//            //data item expired date status
//            tempData.setRefItemExpiredDateStatus(amenityExpiredDateController.getService().getDataItemExpiredDateStatus(tempData.getRefItemExpiredDateStatus().getIdstatus()));
    ...
    private void dataSOIEDSaveHandle() {
        if (checkDataInputDataSOIED()) {
            Alert alert = ClassMessage.showConfirmationSavingDataMessage("", amenityExpiredDateController.dialogStage);
            if (alert.getResult() == ButtonType.OK) {
                //dummy entry
                TblStockOpnameItemExpiredDate dummySelectedDataSOIED = new TblStockOpnameItemExpiredDate(amenityExpiredDateController.selectedDataSOIED);
...
//                        dummyDetail.getTblItemExpiredDate().setRefItemExpiredDateStatus(new RefItemExpiredDateStatus(dummyDetail.getTblItemExpiredDate().getRefItemExpiredDateStatus()));
                ...
                if (amenityExpiredDateController.getService().insertDataStockOpnameItemExpiredDate(dummySelectedDataSOIED, 
                        dummyDetails) != null) {
                    ClassMessage.showSucceedInsertingDataMessage("", amenityExpiredDateController.dialogStage);
                    //refresh data table item expired date
                    amenityExpiredDateController.refreshData();
                    //close form data item expired date - stock opname
                    amenityExpiredDateController.dialogStage.close();
                } else {
                    ClassMessage.showFailedInsertingDataMessage(amenityExpiredDateController.getService().getErrorMessage(), amenityExpiredDateController.dialogStage);
                }
            }
        } else {
            ClassMessage.showWarningInputDataMessage(errDataInput, amenityExpiredDateController.dialogStage);
        }
    }
    ...
    public ItemExpiredDateStockOpnameController(AmenityExpiredDateController parentController) {
        amenityExpiredDateController = parentController;
    }

    private final AmenityExpiredDateController amenityExpiredDateController;
