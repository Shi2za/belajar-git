 ...
 */
package hotelfx.view.feature_laundry.laundry_mutation_history;
 ...
 * @author Andreas
 */
public class LaundryMutationHistoryController implements Initializable {

    @FXML
    private AnchorPane tableDataLaundryMutationHistoryLayout;
    ...
    private TableView tableDataLaundryMutationHistory;

    private void initTableDataLaundryMutationHistory() {
        setTableDataLaundryMutationHistory();

        ancBodyLayout.getChildren().clear();
        AnchorPane.setTopAnchor(tableDataLaundryMutationHistory, 15.0);
        AnchorPane.setLeftAnchor(tableDataLaundryMutationHistory, 15.0);
        AnchorPane.setBottomAnchor(tableDataLaundryMutationHistory, 15.0);
        AnchorPane.setRightAnchor(tableDataLaundryMutationHistory, 15.0);
        ancBodyLayout.getChildren().add(tableDataLaundryMutationHistory);
    }

    private void setTableDataLaundryMutationHistory() {
...
//        locationDestination.getColumns().addAll(locationDestinationType,nameLocationDestination);
        ...
        tableDataLaundryMutationHistory = tableView;
        
        //set filter
        cft = new ClassFilteringTable<>(
                ClassDataMutation.class,
                tableDataLaundryMutationHistory,
                tableDataLaundryMutationHistory.getItems());
...
//        AnchorPane.setLeftAnchor(cft, 15.0);
    ...
    public String getNameLocation(TblLocation location) {
        ...
        switch (location.getRefLocationType().getIdtype()) {
            case 0:
                List<TblRoom> room = parentController.getFLaundryManager().getRoomByIdLocation(location.getIdlocation());
                for (TblRoom getRoom : room) {
                    name = getRoom.getRoomName();
                }
                break;
            case 1:
                List<TblLocationOfWarehouse> warehouse = parentController.getFLaundryManager().getWarehouseByIdLocation(location.getIdlocation());
                for (TblLocationOfWarehouse getWarehouse : warehouse) {
                    name = getWarehouse.getWarehouseName();
                }
                break;
            case 2:
                List<TblLocationOfLaundry> laundry = parentController.getFLaundryManager().getLaundryByIdLocation(location.getIdlocation());
                for (TblLocationOfLaundry getLaundry : laundry) {
                    name = getLaundry.getLaundryName();
                }
                break;
//            case 4:
//                List<TblLocationOfBin> bin = parentController.getFLaundryManager().getBinByIdLocation(location.getIdlocation());
...
//                break;
    ...
    public ObservableList<ClassDataMutation> loadAllDataMutationHistory() {
        ...
        List<TblItemMutationHistory> imhs = parentController.getFLaundryManager().getAllDataItemMutationHistory();
        for (TblItemMutationHistory imh : imhs) {
            if (imh.getTblItem().getPropertyStatus()) {   //Property
                TblItemMutationHistoryPropertyBarcode imhpb = parentController.getFLaundryManager().getDataItemMutationHistoryPropertyBarcodeByIDItemMutationHistory(imh.getIdmutation());
                if (imhpb != null) {
                    ...
                    classDataMutation.getMutationHistoryPropertyBarcode().setTblPropertyBarcode(parentController.getFLaundryManager().getDataPropertyBarcode(classDataMutation.getMutationHistoryPropertyBarcode().getTblPropertyBarcode().getIdbarcode()));
                    list.add(classDataMutation);
                }
            } else {
                if (imh.getTblItem().getConsumable()) {   //Consumable
                    List<TblItemMutationHistoryItemExpiredDate> imhieds = parentController.getFLaundryManager().getAllDataItemMutationHistoryItemExpiredDateByIDItemMutationHistory(imh.getIdmutation());
                    for (TblItemMutationHistoryItemExpiredDate imhied : imhieds) {
                        ClassDataMutation classDataMutation = new ClassDataMutation();
                        classDataMutation.setMutationHistory(imh);
                        classDataMutation.setMutationHistoryItemExpiredDate(imhied);
                        classDataMutation.getMutationHistoryItemExpiredDate().setTblItemExpiredDate(parentController.getFLaundryManager().getDataItemExpiredDate(classDataMutation.getMutationHistoryItemExpiredDate().getTblItemExpiredDate().getIditemExpiredDate()));
                        list.add(classDataMutation);
                    }
                } else {
                    ClassDataMutation classDataMutation = new ClassDataMutation();
    ...
    public void initialize(URL location, ResourceBundle resources) {
        ...
        initTableDataLaundryMutationHistory();

    }

    public LaundryMutationHistoryController(FeatureLaundryController parentController) {
