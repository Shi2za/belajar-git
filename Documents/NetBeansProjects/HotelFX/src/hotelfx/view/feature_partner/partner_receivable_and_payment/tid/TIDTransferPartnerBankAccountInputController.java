 ...
 * @author ANDRI
 */
public class TIDTransferPartnerBankAccountInputController implements Initializable {
    ...
    private void setSelectedDataToInputForm() {
        ...
        txtCodePartnerBankAccount.textProperty().bindBidirectional(tidTransferController.selectedDataPartnerBankAccount.getTblBankAccount().codeBankAccountProperty());
        txtBankAccountHolderName.textProperty().bindBidirectional(tidTransferController.selectedDataPartnerBankAccount.getTblBankAccount().bankAccountHolderNameProperty());

        cbpBank.valueProperty().bindBidirectional(tidTransferController.selectedDataPartnerBankAccount.getTblBankAccount().tblBankProperty());
    ...
    private void dataPartnerBankAccountSaveHandle() {
        if (checkDataInputDataPartnerBankAccount()) {
            ...
            if (alert.getResult() == ButtonType.OK) {
                //dummy entry
                TblPartnerBankAccount dummySelectedData = new TblPartnerBankAccount(tidTransferController.selectedDataPartnerBankAccount);
                ...
                if (tidTransferController.getParentController().getParentController().getService().insertDataPartnerBankAccount(dummySelectedData) != null) {
                    ClassMessage.showSucceedInsertingDataMessage("", tidTransferController.dialogStage);
                    //refresh data partner - bank account popup
                    tidTransferController.refreshPartnerBankAccountDataPopup();
                    //reload and set data on field
                    tidTransferController.selectedDataPartnerBankAccount = tidTransferController.getParentController().getParentController().getService().getPartnerBankAccount(dummySelectedData.getIdrelation());
                    tidTransferController.selectedDataPartnerBankAccount.setTblPartner(tidTransferController.getParentController().getParentController().getService().getDataPartner(tidTransferController.selectedDataPartnerBankAccount.getTblPartner().getIdpartner()));
                    tidTransferController.selectedDataPartnerBankAccount.setTblBankAccount(tidTransferController.getParentController().getParentController().getService().getBankAccount(tidTransferController.selectedDataPartnerBankAccount.getTblBankAccount().getIdbankAccount()));
                    tidTransferController.getParentController().getParentController().selectedDataHFTWithTransfer.setTblBankAccountBySenderBankAccount(tidTransferController.selectedDataPartnerBankAccount.getTblBankAccount());
...
//        }
    ...
    public TIDTransferPartnerBankAccountInputController(TIDTransferController parentController) {
