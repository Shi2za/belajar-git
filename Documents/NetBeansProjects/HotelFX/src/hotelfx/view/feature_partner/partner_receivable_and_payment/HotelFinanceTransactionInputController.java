...
import hotelfx.view.feature_partner.partner_receivable_and_payment.tid.TIDCashController;
import hotelfx.view.feature_partner.partner_receivable_and_payment.tid.TIDCekGiroController;
import hotelfx.view.feature_partner.partner_receivable_and_payment.tid.TIDTransferController;
...
public class HotelFinanceTransactionInputController implements Initializable {
    ...
    private List<RefFinanceTransactionPaymentType> loadAllDataFinanceTransactionPaymentType() {
        List<RefFinanceTransactionPaymentType> list = partnerReceivableController.getService().getAllDataFinanceTransactionPaymentType();
    ...
    private void setSelectedDataToInputForm() {
        ...
        cbpFinanceTransactionPaymentType.valueProperty().bindBidirectional(partnerReceivableController.selectedDataHFT.refFinanceTransactionPaymentTypeProperty());
    ...
    public void setDataTableHotelReceivable() {
        ...
        TableColumn<TblHotelFinanceTransactionHotelReceivable, String> codeReservationPayment = new TableColumn("No. Transaksi\n (Reservasi)");
        codeReservationPayment.setCellValueFactory((TableColumn.CellDataFeatures<TblHotelFinanceTransactionHotelReceivable, String> param)
                -> Bindings.createStringBinding(() -> getCodeReservationPayment(param.getValue().getTblHotelReceivable()),
                        param.getValue().tblHotelReceivableProperty()));
        codeReservationPayment.setMinWidth(100);
        ...
        tableHotelReceivable.getColumns().addAll(codeReservationPayment, codeInvoice, dueDate, nominalRestOfBill, nominalTransaction);
        tableHotelReceivable.setItems(FXCollections.observableArrayList(partnerReceivableController.selectedDataHFTHRs));
    ...
    private String getCodeReservationPayment(TblHotelReceivable hotelReceivable) {
        ...
        if (hotelReceivable != null) {
            TblReservationPaymentWithGuaranteePayment rpwgp = partnerReceivableController.getService().getDataReservationPaymentWithGuaranteePaymentByIDHotelReceivable(hotelReceivable.getIdhotelReceivable());
            if (rpwgp != null) {
                TblReservationPayment rp = partnerReceivableController.getService().getDataReservationPayment(rpwgp.getTblReservationPayment().getIdpayment());
                if (rp != null) {
                    result = rp.getCodePayment();
                }
            }
        }
        return result;
    ...
    private String getCodeInvoice(TblHotelReceivable hotelReceivable) {
        ...
        if (hotelReceivable != null
            ...
            TblHotelInvoice invoice = partnerReceivableController.getService().getDataHotelInvoice(hotelReceivable.getTblHotelInvoice().getIdhotelInvoice());
            if (invoice != null) {
    ...
    private BigDecimal getRestOfBill(TblHotelReceivable hotelReceivable) {
        ...
        if (hotelReceivable != null) {
            ...
            List<TblHotelFinanceTransactionHotelReceivable> list = partnerReceivableController.getService().getAllDataHotelFinanceTransactionHotelReceivableByIDHotelReceivable(hotelReceivable.getIdhotelReceivable());
    ...
    private void refreshDataHotelFinanceTransaction() {
        ...
        if (cbpFinanceTransactionPaymentType.getValue() != null
            ...
            partnerReceivableController.selectedDataHFT.setTransactionNominal(getNominalAfterRounding(nominalTransaction));
            partnerReceivableController.selectedDataHFT.setTransactionRoundingValue(getRoundingValue(nominalTransaction));
        } else {
            partnerReceivableController.selectedDataHFT.setTransactionNominal(calculationHotelFinanceTransactionNominal());
            partnerReceivableController.selectedDataHFT.setTransactionRoundingValue(new BigDecimal("0"));
        }
    ...
    private void setTransactionTypeInputForm(RefFinanceTransactionPaymentType transactionType) {
        try {
            ...
            switch (transactionType.getIdtype()) {
                case 0:    //Tunai = '0'
                    partnerReceivableController.selectedDataHFTWithCash = new TblHotelFinanceTransactionWithCash();
                    partnerReceivableController.selectedDataHFTWithCash.setTblHotelFinanceTransaction(partnerReceivableController.selectedDataHFT);

                    loader.setLocation(HotelFX.class.getResource("view/feature_partner/partner_receivable_and_payment/tid/TIDCash.fxml"));

                    TIDCashController tidCashController = new TIDCashController(this);
                    loader.setController(tidCashController);
                    break;
                case 1:    //Transfer = '1'
                    partnerReceivableController.selectedDataHFTWithTransfer = new TblHotelFinanceTransactionWithTransfer();
                    partnerReceivableController.selectedDataHFTWithTransfer.setTblHotelFinanceTransaction(partnerReceivableController.selectedDataHFT);

                    loader.setLocation(HotelFX.class.getResource("view/feature_partner/partner_receivable_and_payment/tid/TIDTransfer.fxml"));

                    TIDTransferController tidTransferController = new TIDTransferController(this);
                    loader.setController(tidTransferController);
                    break;
                case 4: //Cek = '4'
                    partnerReceivableController.selectedDataHFTWithCekGiro = new TblHotelFinanceTransactionWithCekGiro();
                    partnerReceivableController.selectedDataHFTWithCekGiro.setTblHotelFinanceTransaction(partnerReceivableController.selectedDataHFT);

                    loader.setLocation(HotelFX.class.getResource("view/feature_partner/partner_receivable_and_payment/tid/TIDCekGiro.fxml"));
    ...
    private void dataDetailSaveHandle() {
        if (checkDataInputDataHotelFinanceTransaction()) {
            ...
            ClassMessage.showWarningInputDataMessage(errDataInput, partnerReceivableController.dialogStageDetal);
        }
    }

    private void dataDetailWithCashSaveHandle() {
        if (checkDataInputDataHotelFinanceTransactionWithCash()) {
            Alert alert = ClassMessage.showConfirmationSavingDataMessage("", partnerReceivableController.dialogStageDetal);
            if (alert.getResult() == ButtonType.OK) {
                //dummy
                TblHotelFinanceTransaction dummySelectedData = new TblHotelFinanceTransaction(partnerReceivableController.selectedDataHFT);
                ...
                TblHotelFinanceTransactionWithCash dummySelectedDataWithCash = new TblHotelFinanceTransactionWithCash(partnerReceivableController.selectedDataHFTWithCash);
                ...
                for (TblHotelFinanceTransactionHotelReceivable selectedDataHFTHR : partnerReceivableController.selectedDataHFTHRs) {
                    if (selectedDataHFTHR.getNominalTransaction().compareTo(new BigDecimal("0")) == 1) {    //payment was found
                        ...
                        if (getRestOfBill(dummySelectedDataHFTHR.getTblHotelReceivable())
                            ...
                            dummySelectedDataHFTHR.getTblHotelReceivable().setRefFinanceTransactionStatus(partnerReceivableController.getService().getDataFinanceTransactionStatus(2)); //Sudah Dibayar = '2'
                        } else {
                            if (getRestOfBill(dummySelectedDataHFTHR.getTblHotelReceivable())
                                    .compareTo(dummySelectedDataHFTHR.getNominalTransaction()) == 1) {
                                dummySelectedDataHFTHR.getTblHotelReceivable().setRefFinanceTransactionStatus(partnerReceivableController.getService().getDataFinanceTransactionStatus(1)); //Dibayar Sebagian = '1'
                            } else {
                                if (getRestOfBill(dummySelectedDataHFTHR.getTblHotelReceivable())
                                        .compareTo(dummySelectedDataHFTHR.getNominalTransaction()) == -1) {
                                    dummySelectedDataHFTHR.getTblHotelReceivable().setRefFinanceTransactionStatus(partnerReceivableController.getService().getDataFinanceTransactionStatus(5)); //Kelebihan Bayar = '5'   
                                }
                            }
                        }
                        //data PO (status) ...?
                ...
                if (partnerReceivableController.getService().insertDataHotelFinanceTransaction(
                        dummySelectedData,
                        dummySelectedDataWithCash,
                        dummySelectedDataHFTHRs, 
                        partnerReceivableController.selectedData.getPartnerName())
                        != null) {
                    ClassMessage.showSucceedInsertingDataMessage("", partnerReceivableController.dialogStageDetal);
                    //refresh data table supplier - hotel receivable
                    partnerReceivableController.refreshDataTablePartnerReceivable();
                    //auto selected current data..
                    partnerReceivableController.refreshSelectedData();
                    //close form data hotel finance transaction input
                    partnerReceivableController.dialogStageDetal.close();
                } else {
                    ClassMessage.showFailedInsertingDataMessage(partnerReceivableController.getService().getErrorMessage(), partnerReceivableController.dialogStageDetal);
                }
            }
        } else {
            ClassMessage.showWarningInputDataMessage(errDataInput, partnerReceivableController.dialogStageDetal);
    ...
    private void dataDetailWithTransferSaveHandle() {
        if (checkDataInputDataHotelFinanceTransactionWithTransfer()) {
            ...
            if (alert.getResult() == ButtonType.OK) {
                ...
                TblHotelFinanceTransactionWithTransfer dummySelectedDataWithTransfer = new TblHotelFinanceTransactionWithTransfer(partnerReceivableController.selectedDataHFTWithTransfer);
    ...
    private void dataDetailWithCekGiroSaveHandle() {
        if (checkDataInputDataHotelFinanceTransactionWithCekGiro()) {
            ...
            if (alert.getResult() == ButtonType.OK) {
                ...
                TblHotelFinanceTransactionWithCekGiro dummySelectedDataWithCekGiro = new TblHotelFinanceTransactionWithCekGiro(partnerReceivableController.selectedDataHFTWithCekGiro);
    ...
    private boolean checkDataInputDataHotelFinanceTransactionWithCash() {
        boolean dataInput = true;
        errDataInput = "";
        if (partnerReceivableController.selectedDataHFT.getTransactionNominal()
                .compareTo(new BigDecimal("0")) < 1) {
            dataInput = false;
            errDataInput += "Nominal Transaksi : Tidak boleh kurang dari atau sama dengan '0'\n";
        }
        if (partnerReceivableController.selectedDataHFTWithCash.getTblCompanyBalance() == null) {
            dataInput = false;
    ...
    private boolean checkDataInputDataHotelFinanceTransactionWithTransfer() {
        ...
        if (partnerReceivableController.selectedDataHFTWithTransfer.getSenderName() == null
                || partnerReceivableController.selectedDataHFTWithTransfer.getSenderName().equals("")) {
            dataInput = false;
            errDataInput += "Nama (Pengirim) : " + ClassMessage.defaultErrorNullValueMessage + " \n";
        }
        if (partnerReceivableController.selectedDataHFTWithTransfer.getTblBankAccountBySenderBankAccount() == null) {
            dataInput = false;
            errDataInput += "Nomor Rekening (Pengirim) : " + ClassMessage.defaultErrorNullValueMessage + " \n";
        }
        if (partnerReceivableController.selectedDataHFTWithTransfer.getReceiverName() == null
                || partnerReceivableController.selectedDataHFTWithTransfer.getReceiverName().equals("")) {
            dataInput = false;
            errDataInput += "Nama (Penerima) : " + ClassMessage.defaultErrorNullValueMessage + " \n";
        }
        if (partnerReceivableController.selectedDataHFTWithTransfer.getTblBankAccountByReceiverBankAccount() == null) {
            dataInput = false;
    ...
    private boolean checkDataInputDataHotelFinanceTransactionWithCekGiro() {
        ...
        if (partnerReceivableController.selectedDataHFTWithCekGiro.getCodeCekGiro() == null
                || partnerReceivableController.selectedDataHFTWithCekGiro.getCodeCekGiro().equals("")) {
            dataInput = false;
            errDataInput += "Nomor Cek/Giro : " + ClassMessage.defaultErrorNullValueMessage + " \n";
        }
        if (partnerReceivableController.selectedDataHFTWithCekGiro.getIssueDateTime() == null) {
            dataInput = false;
            errDataInput += "Tanggal Buat : " + ClassMessage.defaultErrorNullValueMessage + " \n";
        }
        if (partnerReceivableController.selectedDataHFTWithCekGiro.getValidUntilDateTime() == null) {
            dataInput = false;
            errDataInput += "Tanggal Jatuh Tempo : " + ClassMessage.defaultErrorNullValueMessage + " \n";
        }
        if (partnerReceivableController.selectedDataHFTWithCekGiro.getTblBank() == null) {
            dataInput = false;
            errDataInput += "Bank Penerbit : " + ClassMessage.defaultErrorNullValueMessage + " \n";
        }
        if (partnerReceivableController.selectedDataHFTWithCekGiro.getSenderName() == null
                || partnerReceivableController.selectedDataHFTWithCekGiro.getSenderName().equals("")) {
            dataInput = false;
            errDataInput += "Nama (Pembuat) : " + ClassMessage.defaultErrorNullValueMessage + " \n";
        }
//        if (partnerReceivableController.selectedDataHFTWithCekGiro.getTblBankAccountBySenderBankAccount() == null) {
//            dataInput = false;
//            errDataInput += "Nomor Rekening (Pembuat) : " + ClassMessage.defaultErrorNullValueMessage + " \n";
//        }
        if (partnerReceivableController.selectedDataHFTWithCekGiro.getReceiverName() == null
                || partnerReceivableController.selectedDataHFTWithCekGiro.getReceiverName().equals("")) {
            dataInput = false;
            errDataInput += "Nama (Penerima) : " + ClassMessage.defaultErrorNullValueMessage + " \n";
        }
        if (partnerReceivableController.selectedDataHFTWithCekGiro.getTblBankAccountByReceiverBankAccount() == null) {
            dataInput = false;
            errDataInput += "Nomor Rekening (Penerima) : " + ClassMessage.defaultErrorNullValueMessage + " \n";
        }
        return dataInput;
    ...
    public HotelFinanceTransactionInputController(PartnerReceivableController parentController) {
        partnerReceivableController = parentController;
    }

    private final PartnerReceivableController partnerReceivableController;

    public PartnerReceivableController getParentController() {
        return partnerReceivableController;
    }
