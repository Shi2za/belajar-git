...
public class TIDTransferController implements Initializable {
    ...
    @FXML
    private JFXButton btnAddRestoBankAccount;

    private void initFormDataTIDTransfer() {
        initDataTIDTransferPopup();
        
        btnAddRestoBankAccount.setTooltip(new Tooltip("Tambahkan (Nomor Rekening Resto)"));
        btnAddRestoBankAccount.setOnAction((e) -> {
            dataRestoBankAccountAddHandle();
        });
    ...
    private void initDataTIDTransferPopup() {
        ...
        ObservableList<TblBankAccount> receiverBankAccountItems = FXCollections.observableArrayList(loadAllDataRestoBankAccount());
    ...
    private List<TblBankAccount> loadAllDataRestoBankAccount() {
        List<TblRestoBankAccount> cbaList = hotelFinanceTransactionInputController.getParentController().getService().getAllDataRestoBankAccountByIDResto(hotelFinanceTransactionInputController.getParentController().selectedData.getIdresto());
        List<TblBankAccount> list = new ArrayList<>();
        for (TblRestoBankAccount cbbaData : cbaList) {
            //data bank account
    ...
    private void refreshDataPopup() {
        //Receiver - Bank Account (Resto)
        refreshRestoBankAccountDataPopup();
    ...
    public void refreshRestoBankAccountDataPopup() {
        ObservableList<TblBankAccount> receiverBankAccountItems = FXCollections.observableArrayList(loadAllDataRestoBankAccount());
    ...
    /**
     * DATA RESTO BANK ACCOUNT (ADD - HANDLER)
     */
    public TblRestoBankAccount selectedDataRestoBankAccount;

    public Stage dialogStage;

    private void dataRestoBankAccountAddHandle() {
        selectedDataRestoBankAccount = new TblRestoBankAccount();
        selectedDataRestoBankAccount.setTblResto(hotelFinanceTransactionInputController.getParentController().selectedData);
        selectedDataRestoBankAccount.setTblBankAccount(new TblBankAccount());
        //open form data suplier bank account
        showRestoBankAccountDialog();
    }

    private void showRestoBankAccountDialog() {
        try {
            // Load the fxml file and create a new stage for the popup dialog.
            FXMLLoader loader = new FXMLLoader();
            loader.setLocation(HotelFX.class.getResource("view/feature_resto/resto_payable_and_payment/tid/TIDTransferRestoBankAccountInputDialog.fxml"));

            TIDTransferRestoBankAccountInputController controller = new TIDTransferRestoBankAccountInputController(this);
