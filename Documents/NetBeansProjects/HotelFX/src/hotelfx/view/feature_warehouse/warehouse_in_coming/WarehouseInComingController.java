 ...
 */
public class WarehouseInComingController implements Initializable {
    ...
    private void setTableDataIC() {
        ...
        TableColumn<TblInComing, String> destinationLocation = new TableColumn<>("Gudang"); //Ke
        ...
        TableColumn<TblInComing, String> locationTitle = new TableColumn("Gudang");
...
//        AnchorPane.setLeftAnchor(cft, 15.0);
    ...
    private List<TblInComing> loadAllDataIC() {
        List<TblInComing> list = parentController.getFWarehouseManager().getAllDataInComing();
        ...
        for (int i = list.size() - 1; i > -1; i--) {
            ...
            list.get(i).setTblEmployeeByIdsender(parentController.getFWarehouseManager().getDataEmployee(list.get(i).getTblEmployeeByIdsender().getIdemployee()));
    ...
    private void initDataPopup() {
        ...
        cbpSourceLocation = new JFXCComboBoxTablePopup<>(
                ClassLocation.class, tableLocationSource, sourceLocationItems, "", "Gudang *", true, 150, 200
        );
        ...
        cbpDestinationLocation = new JFXCComboBoxTablePopup<>(
                ClassLocation.class, tableLocationDestination, destinationLocationItems, "", "Gudang *", true, 150, 200
        );
...
//        AnchorPane.setRightAnchor(cbpDestinationLocation, 0.0);
    ...
    private void dataICShowHandle() {
        if (tableDataIC.getTableView().getSelectionModel().getSelectedItems().size() == 1) {
            dataInputStatus = 3;
            selectedData = parentController.getFWarehouseManager().getDataInComing(((TblInComing) tableDataIC.getTableView().getSelectionModel().getSelectedItem()).getIdic());
            selectedData.setTblEmployeeByIdsender(parentController.getFWarehouseManager().getDataEmployee(selectedData.getTblEmployeeByIdsender().getIdemployee()));
        ...
        if (checkDataInputDataIC()) {
            ...
            if (alert.getResult() == ButtonType.OK) {
                ...
                switch (dataInputStatus) {
                    case 0:
                        if (parentController.getFWarehouseManager().insertDataInComing(
                                dummySelectedData,
...
//            super.cancelEdit();
    ...
    private ObservableList<ICDetailItemMutationHistory> loadAllDataICDIMH() {
        ...
        if (dataInputStatus == 3) {   //just for show data
            ...
            List<TblInComingDetail> icds = parentController.getFWarehouseManager().getAllDataInComingDetailByIDInComing(selectedData.getIdic());
            for (TblInComingDetail icd : icds) {
                //set data item
                icd.setTblItem(parentController.getFWarehouseManager().getDataItem(icd.getTblItem().getIditem()));
                //set data unit
                icd.getTblItem().setTblUnit(parentController.getFWarehouseManager().getDataUnit(icd.getTblItem().getTblUnit().getIdunit()));
                //set data item type (hk)
                if (icd.getTblItem().getTblItemTypeHk() != null) {
                    icd.getTblItem().setTblItemTypeHk(parentController.getFWarehouseManager().getDataItemTypeHK(icd.getTblItem().getTblItemTypeHk().getIditemTypeHk()));
                }
                //set data item type (wh)
                if (icd.getTblItem().getTblItemTypeWh() != null) {
                    icd.getTblItem().setTblItemTypeWh(parentController.getFWarehouseManager().getDataItemTypeWH(icd.getTblItem().getTblItemTypeWh().getIditemTypeWh()));
                }
                //data ic detail - imh
                TblInComingDetailItemMutationHistory icdimh = parentController.getFWarehouseManager().getDataInComingDetailItemMutationHistoryByIDInComingDetail(icd.getIddetail());
                icdimh.setTblInComingDetail(icd);
                icdimh.setTblItemMutationHistory(parentController.getFWarehouseManager().getDataItemMutationHistory(icdimh.getTblItemMutationHistory().getIdmutation()));
                //data imh - property (s)
                List<TblItemMutationHistoryPropertyBarcode> imhpbs = parentController.getFWarehouseManager().getAllDataItemMutationHistoryPropertyBarcodeByIDItemMutationHistory(icdimh.getTblItemMutationHistory().getIdmutation());
                ...
                List<TblItemMutationHistoryItemExpiredDate> imhieds = parentController.getFWarehouseManager().getAllDataItemMutationHistoryItemExpiredDateByIDItemMutationHistory(icdimh.getTblItemMutationHistory().getIdmutation());
    ...
    public void dataDetailCreateHandle() {
        if (selectedData.getTblLocationByIdlocationSource() != null
            ...
            HotelFX.showAlert(Alert.AlertType.WARNING, "TIDAK DAPAT MENAMBAH DATA", "Pilih data gudang terlebih dahulu..!", null);
        }
    }
    ...
    public ICDetailItemMutationHistory generateDataICDetailItemMutationHistory(TblItem dataItem) {
        if (selectedData != null
            ...
            //set data item
            icd.setTblItem(parentController.getFWarehouseManager().getDataItem(dataItem.getIditem()));
            //set data unit
            icd.getTblItem().setTblUnit(parentController.getFWarehouseManager().getDataUnit(dataItem.getTblUnit().getIdunit()));
            //set data item type (hk)
            if (dataItem.getTblItemTypeHk() != null) {
                icd.getTblItem().setTblItemTypeHk(parentController.getFWarehouseManager().getDataItemTypeHK(dataItem.getTblItemTypeHk().getIditemTypeHk()));
            }
            //set data item type (wh)
            if (dataItem.getTblItemTypeWh() != null) {
                icd.getTblItem().setTblItemTypeWh(parentController.getFWarehouseManager().getDataItemTypeWH(dataItem.getTblItemTypeWh().getIditemTypeWh()));
            }
            //item quantity
    ...
    private void showDataDetailDialog() {
        try {
            ...
            loader.setLocation(HotelFX.class.getResource("view/feature_warehouse/warehouse_in_coming/WarehouseInComingDetailDialog.fxml"));

            WarehouseInComingDetailController controller = new WarehouseInComingDetailController(this);
    ...
    public WarehouseInComingController(FeatureWarehouseController parentController) {
