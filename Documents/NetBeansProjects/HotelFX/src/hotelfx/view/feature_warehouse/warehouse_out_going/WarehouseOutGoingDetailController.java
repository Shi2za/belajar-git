 ...
 * @author ANDRI
 */
public class WarehouseOutGoingDetailController implements Initializable {
    ...
    private List<TblItem> loadAllDataItem() {
        List<TblItem> list = warehouseOutGoingController.getService().getAllDataItem();
        for (TblItem data : list) {
            //data unit
            data.setTblUnit(warehouseOutGoingController.getService().getDataUnit(data.getTblUnit().getIdunit()));
            //data item type hk
            if (data.getTblItemTypeHk() != null) {
                data.setTblItemTypeHk(warehouseOutGoingController.getService().getDataItemTypeHK(data.getTblItemTypeHk().getIditemTypeHk()));
            }
            //data item type wh
            if (data.getTblItemTypeWh() != null) {
                data.setTblItemTypeWh(warehouseOutGoingController.getService().getDataItemTypeWH(data.getTblItemTypeWh().getIditemTypeWh()));
            }
        }
        for (int i = list.size() - 1; i > -1; i--) {
            for (int j = 0; j < warehouseOutGoingController.tableDataDetail.getTableView().getItems().size(); j++) {
                if (((WarehouseOutGoingController.OGDetailItemMutationHistory) warehouseOutGoingController.tableDataDetail.getTableView().getItems().get(j)).getDataOGDIMH().getTblOutGoingDetail().getTblItem().getIditem()
    ...
    private void dataDetailSaveHandle() {
        if (checkDataInputDataDetail()) {
            Alert alert = ClassMessage.showConfirmationSavingDataMessage("", warehouseOutGoingController.dialogStageDetal);
            if (alert.getResult() == ButtonType.OK) {
                switch (warehouseOutGoingController.dataInputDetailStatus) {
                    case 0:
                        warehouseOutGoingController.selectedDataDetail = warehouseOutGoingController.generateDataOGDetailItemMutationHistory(cbpItem.getValue());
                        if (warehouseOutGoingController.selectedDataDetail != null) {
                            ClassMessage.showSucceedAddingDataMessage("", warehouseOutGoingController.dialogStageDetal);
                            warehouseOutGoingController.tableDataDetail.getTableView().getItems().add(warehouseOutGoingController.selectedDataDetail);
                            //close form data detail
                            warehouseOutGoingController.dialogStageDetal.close();
                        } else {
                            ClassMessage.showFailedAddingDataMessage("", warehouseOutGoingController.dialogStageDetal);
                        }
                        break;
                    default:
                        break;
            ...
            ClassMessage.showWarningInputDataMessage(errDataInput, warehouseOutGoingController.dialogStageDetal);
        }
    }
    ...
    public WarehouseOutGoingDetailController(WarehouseOutGoingController parentController) {
        warehouseOutGoingController = parentController;
    }

    private final WarehouseOutGoingController warehouseOutGoingController;
