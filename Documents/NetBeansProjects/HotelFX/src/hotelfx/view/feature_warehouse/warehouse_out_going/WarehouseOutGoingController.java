 ...
 * @author ANDRI
 */
public class WarehouseOutGoingController implements Initializable {
    ...
    private void setTableDataOG() {
        ...
        TableColumn<TblOutGoing, String> sourceLocation = new TableColumn<>("Dari");
        ...
        TableColumn<TblOutGoing, String> locationTitle = new TableColumn("Gudang");
        locationTitle.getColumns().addAll(sourceLocation, destinationLocation);

        tableView.getColumns().addAll(ogDate, codeOG, codeSR, byTitle, locationTitle);
...
//        AnchorPane.setLeftAnchor(cft, 15.0);
    ...
    public String getNameLocation(TblLocation dataLocation) {
        if (dataLocation != null
            ...
            switch (dataLocation.getRefLocationType().getIdtype()) {
                case 0: //Room
                    TblRoom room = parentController.getFWarehouseManager().getDataRoomByIDLocation(dataLocation.getIdlocation());
                    return room != null ? room.getRoomName() : "-";
                case 1: //Warehouse
                    TblLocationOfWarehouse warehouse = parentController.getFWarehouseManager().getDataWarehouseByIdLocation(dataLocation.getIdlocation());
                    return warehouse != null ? warehouse.getWarehouseName() : "-";
                case 2: //Laundry
                    TblLocationOfLaundry laundry = parentController.getFWarehouseManager().getDataLaundryByIDLocation(dataLocation.getIdlocation());
    ...
    private List<TblOutGoing> loadAllDataOG() {
        List<TblOutGoing> list = parentController.getFWarehouseManager().getAllDataOutGoing();
        ...
        for (int i = list.size() - 1; i > -1; i--) {
            ...
            if (list.get(i).getTblStoreRequest() != null) {
                list.get(i).setTblStoreRequest(parentController.getFWarehouseManager().getDataStoreRequest(list.get(i).getTblStoreRequest().getIdsr()));
            }
            //data employee (receiver)
            list.get(i).setTblEmployeeByIdreceiver(parentController.getFWarehouseManager().getDataEmployee(list.get(i).getTblEmployeeByIdreceiver().getIdemployee()));
            //data employee (create)
            list.get(i).setTblEmployeeByCreateBy(parentController.getFWarehouseManager().getDataEmployee(list.get(i).getTblEmployeeByCreateBy().getIdemployee()));
            if (list.get(i).getTblLocationByIdlocationSource().getRefLocationType().getIdtype() == 1) {    //Gudang = '1'
    ...
    private void initDataPopup() {
        ...
        cbpSourceLocation = new JFXCComboBoxTablePopup<>(
                ClassLocation.class, tableLocationSource, sourceLocationItems, "", "Gudang (Sumber) *", true, 150, 200
        );
        ...
        cbpDestinationLocation = new JFXCComboBoxTablePopup<>(
                ClassLocation.class, tableLocationDestination, destinationLocationItems, "", "Gudang (Tujuan) *", true, 150, 200
        );
...
//        List<TblStoreRequest> list = parentController.getFWarehouseManager().getAllDataStoreRequestByIDStoreRequestStatus((int)4);   //Dipesan = '4'
        List<TblStoreRequest> list = parentController.getFWarehouseManager().getAllDataStoreRequestByIDStoreRequestStatus((int) 0);   //Dibuat = '0'
        for (TblStoreRequest data : list) {
            //data location (source)
            data.setTblLocationByIdlocationSource(parentController.getFWarehouseManager().getLocation(data.getTblLocationByIdlocationSource().getIdlocation()));
            //data location type (source)
            data.getTblLocationByIdlocationSource().setRefLocationType(parentController.getFWarehouseManager().getDataLocationType(data.getTblLocationByIdlocationSource().getRefLocationType().getIdtype()));
            //data location (destination)
            data.setTblLocationByIdlocationDestination(parentController.getFWarehouseManager().getLocation(data.getTblLocationByIdlocationDestination().getIdlocation()));
            //data location type (destination)
            data.getTblLocationByIdlocationDestination().setRefLocationType(parentController.getFWarehouseManager().getDataLocationType(data.getTblLocationByIdlocationDestination().getRefLocationType().getIdtype()));
        }
        //data null -> 0L = '-'
    ...
    private List<TblEmployee> loadAllDataEmployee() {
        ...
        for (TblEmployee data : list) {
            ...
            if (data.getTblJob() != null) {
                data.setTblJob(parentController.getFWarehouseManager().getDataJob(data.getTblJob().getIdjob()));
            }
            //data group
            if (data.getTblGroup() != null) {
                data.setTblGroup(parentController.getFWarehouseManager().getDataGroup(data.getTblGroup().getIdgroup()));
            }
        }
        return list;
    }
    ...
    private List<ClassLocation> loadAllDataSourceClassLocation() {
        ...
        List<TblLocation> locations = parentController.getFWarehouseManager().getAllDataLocationByIDLocationType(1); //Gudang = '1'
        for (TblLocation location : locations) {
            //data group
            location.setTblGroup(parentController.getFWarehouseManager().getDataGroup(location.getTblGroup().getIdgroup()));
            //data location type
            location.setRefLocationType(parentController.getFWarehouseManager().getDataLocationType((int) location.getRefLocationType().getIdtype()));
            //data class location
            if (ClassSession.currentUser.getTblEmployeeByIdemployee().getTblGroup() != null) {
    ...
    private List<ClassLocation> loadAllDataDestinationClassLocation() {
        ...
        for (TblLocation location : locations) {
            ...
            location.setTblGroup(parentController.getFWarehouseManager().getDataGroup(location.getTblGroup().getIdgroup()));
            //data location type
            location.setRefLocationType(parentController.getFWarehouseManager().getDataLocationType(location.getRefLocationType().getIdtype()));
            //data class location
            if (ClassSession.currentUser.getTblEmployeeByIdemployee().getTblGroup() != null) {
                if (location.getTblGroup() == null
                        || location.getTblGroup().getIdgroup() != ClassSession.currentUser.getTblEmployeeByIdemployee().getTblGroup().getIdgroup()) {
    ...
    private TblLocation getDataUsedLocation() {
        TblLocation usedLocation = parentController.getFWarehouseManager().getLocationByIDLocationType(6);  //Digunakan = '6'
        if (usedLocation != null) {
            //location type
            usedLocation.setRefLocationType(parentController.getFWarehouseManager().getDataLocationType(usedLocation.getRefLocationType().getIdtype()));
        }
        return usedLocation;
    }
    ...
    private void dataOGShowHandle() {
        if (tableDataOG.getTableView().getSelectionModel().getSelectedItems().size() == 1) {
            dataInputStatus = 3;
            selectedData = parentController.getFWarehouseManager().getDataOutGoing(((TblOutGoing) tableDataOG.getTableView().getSelectionModel().getSelectedItem()).getIdog());
            if (selectedData.getTblStoreRequest() != null) {
                selectedData.setTblStoreRequest(parentController.getFWarehouseManager().getDataStoreRequest(selectedData.getTblStoreRequest().getIdsr()));
            }
            selectedData.setTblEmployeeByIdreceiver(parentController.getFWarehouseManager().getDataEmployee(selectedData.getTblEmployeeByIdreceiver().getIdemployee()));
...
//        }
        ...
        if (checkDataInputDataOG()) {
            ...
            if (alert.getResult() == ButtonType.OK) {
                ...
                switch (dataInputStatus) {
                    case 0:
                        if (parentController.getFWarehouseManager().insertDataOutGoing(
                                dummySelectedData,
    ...
    private boolean checkDataInputDataOGSRDetailQuantityNeeded() {
        if (selectedData.getTblStoreRequest() != null
            ...
            for (OGDetailItemMutationHistory ogdimh : (List<OGDetailItemMutationHistory>) tableDataDetail.getTableView().getItems()) {
                TblStoreRequestDetail srd = parentController.getFWarehouseManager().getDataStoreRequestDetailByIDStoreRequestAndIDItem(
    ...
    private void setTableDataDetail() {
        ...
        TableColumn<OGDetailItemMutationHistory, String> ogItemQuantity = new TableColumn("OG");
        ...
        if (dataInputStatus != 3) {
            tableView.getColumns().addAll(titledItemType, codeItem, itemName, titledItemQuantity, detail, itemStock, unitName);
        } else {
...
//            super.cancelEdit();
    ...
    private ObservableList<OGDetailItemMutationHistory> loadAllDataOGDIMH() {
        ...
        if (dataInputStatus == 3) {   //just for show data
            ...
            List<TblOutGoingDetail> ogds = parentController.getFWarehouseManager().getAllDataOutGoingDetailByIDOutGoing(selectedData.getIdog());
            for (TblOutGoingDetail ogd : ogds) {
                //set data item
                ogd.setTblItem(parentController.getFWarehouseManager().getDataItem(ogd.getTblItem().getIditem()));
                //set data unit
                ogd.getTblItem().setTblUnit(parentController.getFWarehouseManager().getDataUnit(ogd.getTblItem().getTblUnit().getIdunit()));
                //set data item type (hk)
                if (ogd.getTblItem().getTblItemTypeHk() != null) {
                    ogd.getTblItem().setTblItemTypeHk(parentController.getFWarehouseManager().getDataItemTypeHK(ogd.getTblItem().getTblItemTypeHk().getIditemTypeHk()));
                }
                //set data item type (wh)
                if (ogd.getTblItem().getTblItemTypeWh() != null) {
                    ogd.getTblItem().setTblItemTypeWh(parentController.getFWarehouseManager().getDataItemTypeWH(ogd.getTblItem().getTblItemTypeWh().getIditemTypeWh()));
                }
                //data og detail - imh
                TblOutGoingDetailItemMutationHistory ogdimh = parentController.getFWarehouseManager().getDataOutGoingDetailItemMutationHistoryByIDOutGoingDetail(ogd.getIddetail());
                ogdimh.setTblOutGoingDetail(ogd);
                ogdimh.setTblItemMutationHistory(parentController.getFWarehouseManager().getDataItemMutationHistory(ogdimh.getTblItemMutationHistory().getIdmutation()));
                //data imh - property (s)
                List<TblItemMutationHistoryPropertyBarcode> imhpbs = parentController.getFWarehouseManager().getAllDataItemMutationHistoryPropertyBarcodeByIDItemMutationHistory(ogdimh.getTblItemMutationHistory().getIdmutation());
                ...
                List<TblItemMutationHistoryItemExpiredDate> imhieds = parentController.getFWarehouseManager().getAllDataItemMutationHistoryItemExpiredDateByIDItemMutationHistory(ogdimh.getTblItemMutationHistory().getIdmutation());
            ...
            if (selectedData.getTblLocationByIdlocationSource() != null
                ...
                if (selectedData.getTblStoreRequest() != null
                    ...
                    List<TblStoreRequestDetail> srds = parentController.getFWarehouseManager().getAllDataStoreRequestDetailByIDStoreRequest(selectedData.getTblStoreRequest().getIdsr());
                    for (TblStoreRequestDetail srd : srds) {
                        ...
                        //set data item
                        ogd.setTblItem(parentController.getFWarehouseManager().getDataItem(srd.getTblItem().getIditem()));
                        //set data unit
                        ogd.getTblItem().setTblUnit(parentController.getFWarehouseManager().getDataUnit(srd.getTblItem().getTblUnit().getIdunit()));
                        //set data item type (hk)
                        if (srd.getTblItem().getTblItemTypeHk() != null) {
                            ogd.getTblItem().setTblItemTypeHk(parentController.getFWarehouseManager().getDataItemTypeHK(srd.getTblItem().getTblItemTypeHk().getIditemTypeHk()));
                        }
                        //set data item type (wh)
                        if (srd.getTblItem().getTblItemTypeWh() != null) {
                            ogd.getTblItem().setTblItemTypeWh(parentController.getFWarehouseManager().getDataItemTypeWH(srd.getTblItem().getTblItemTypeWh().getIditemTypeWh()));
                        }
                        //item quantity
    ...
    public void dataDetailCreateHandle() {
        if (selectedData.getTblStoreRequest() == null
            ...
            if (selectedData.getTblLocationByIdlocationSource() != null) {
                ...
                HotelFX.showAlert(Alert.AlertType.WARNING, "TIDAK DAPAT MENAMBAH DATA", "Pilih data gudang (sumber) terlebih dahulu..!", null);
            }
        } else {
    ...
    public OGDetailItemMutationHistory generateDataOGDetailItemMutationHistory(TblItem dataItem) {
        if (selectedData != null
            ...
            ogd.setTblOutGoing(selectedData);
            //set data item
            ogd.setTblItem(parentController.getFWarehouseManager().getDataItem(dataItem.getIditem()));
            //set data unit
            ogd.getTblItem().setTblUnit(parentController.getFWarehouseManager().getDataUnit(dataItem.getTblUnit().getIdunit()));
            //set data item type (hk)
            if (dataItem.getTblItemTypeHk() != null) {
                ogd.getTblItem().setTblItemTypeHk(parentController.getFWarehouseManager().getDataItemTypeHK(dataItem.getTblItemTypeHk().getIditemTypeHk()));
            }
            //set data item type (wh)
            if (dataItem.getTblItemTypeWh() != null) {
                ogd.getTblItem().setTblItemTypeWh(parentController.getFWarehouseManager().getDataItemTypeWH(dataItem.getTblItemTypeWh().getIditemTypeWh()));
            }
            //item quantity
            ogd.setItemQuantity(new BigDecimal("0"));
            //data ogd - imh
    ...
    private void showDataDetailDialog() {
        try {
            ...
            loader.setLocation(HotelFX.class.getResource("view/feature_warehouse/warehouse_out_going/WarehouseOutGoingDetailDialog.fxml"));

            WarehouseOutGoingDetailController controller = new WarehouseOutGoingDetailController(this);
    ...
    public WarehouseOutGoingController(FeatureWarehouseController parentController) {
