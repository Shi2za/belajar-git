...
import hotelfx.helper.PrintModel.ClassPrintDataReturDetail;
...
import hotelfx.view.feature_retur.FeatureReturPController;
 ...
 * @author ANDRI
 */
public class ReturPController implements Initializable {
                    ...
                    if (isShowStatus.get()) {
                        ...
                        if ((!row.isEmpty())) {
                            if (DashboardController.feature.getSelectedRoleFeature().getApproveData()
                                    && checkDataPEnableToApprove((TblRetur) tableDataRetur.getTableView().getSelectionModel().getSelectedItem())) {
                                dataReturApproveHandleDetail();
                            } else {
                                dataReturShowHandle();
                            }
                        }
                ...
                if (dataDetail.getTblSupplierItem().getTblItem().getPropertyStatus()) { //Property
                    ...
                    if (dataDetail.getTblSupplierItem().getTblItem().getConsumable()) {    //consumable
                        ...
                        for (TblReturDetailItemExpiredDate dataReturDetailPropertyItemExpiredDate : dataReturDetailPropertyItemExpiredDates) {
                            ...
                            detailLocation.setDetailQuantity(new BigDecimal(String.valueOf(dataReturDetailPropertyItemExpiredDate.getItemQuantity())));
                            //add data to list 'detail_location'
                            list.add(detailLocation);
                        }
                    } else {
...
//    private boolean editing = false;
    private void setSelectedDataToInputForm() {
        ...
        cbpMI.valueProperty().addListener((obs, oldVal, newVal) -> {
            if (dataInputStatus != 3 && dataInputStatus != 4) {
                if (newVal != null) {
                    //reset data details
...
//        setDataDetail();
    ...
    private void setSelectedDataToInputFormFunctionalComponent() {
        ...
        ClassViewSetting.setDisableForAllInputNode(gpFormDataRetur,
                (dataInputStatus == 3 || dataInputStatus == 4),
                txtCodeRetur);
...
//    }
    ...
    private void dataReturApproveHandleDetail() {
        dataInputStatus = 4;
    ...
    private void showDataCanceledDialog() {
        try {
            ...
            loader.setLocation(HotelFX.class.getResource("view/feature_retur/retur_purchasing/ReturCanceledPDialog.fxml"));

            ReturCanceledPController controller = new ReturCanceledPController(this);
    ...
    private void dataReturPrintHandle() {
        if (tableDataRetur.getTableView().getSelectionModel().getSelectedItems().size() == 1) {
            if (checkDataReturEnableToPrint((TblRetur) tableDataRetur.getTableView().getSelectionModel().getSelectedItem())) {
                selectedData = getService().getDataRetur(((TblRetur) tableDataRetur.getTableView().getSelectionModel().getSelectedItem()).getIdretur());
                selectedData.setTblSupplier(getService().getDataSupplier(selectedData.getTblSupplier().getIdsupplier()));
                printRetur(selectedData);
            } else {
                ClassMessage.showWarningInputDataMessage("Data retur yang belum disetujui, tidak dapat dicetak..!!", null);
            }
        } else {
    ...
    private boolean checkDataReturEnableToPrint(TblRetur dataRetur) {
        return dataRetur.getRefReturStatus().getIdstatus() == 1;    //Disetujui = '1'
    }

    private void printRetur(TblRetur dataRetur) {
        List<ClassPrintDataRetur> listDataRetur = new ArrayList();
        ClassPrintDataRetur printDataRetur = new ClassPrintDataRetur();
        printDataRetur.setAlamatSupplier(dataRetur.getTblSupplier().getSupplierAddress());
        printDataRetur.setJabatanApproval(dataRetur.getTblEmployeeByApprovedBy().getTblJob() != null ? dataRetur.getTblEmployeeByApprovedBy().getTblJob().getJobName() : "-");
        printDataRetur.setJabatanPembuat(dataRetur.getTblEmployeeByCreatedBy().getTblJob() != null ? dataRetur.getTblEmployeeByApprovedBy().getTblJob().getJobName() : "-");
        printDataRetur.setKeterangan(dataRetur.getReturNote());
        printDataRetur.setNamaApproval(dataRetur.getTblEmployeeByApprovedBy().getTblPeople().getFullName());
        printDataRetur.setNamaPIC(dataRetur.getTblSupplier().getPicname());
        printDataRetur.setNamaPembuat(dataRetur.getTblEmployeeByCreateBy().getTblPeople().getFullName());
        printDataRetur.setNamaSupplier(dataRetur.getTblSupplier().getSupplierName());
        printDataRetur.setOngkosKirim(dataRetur.getDeliveryCost());
        printDataRetur.setNoTeleponPIC(dataRetur.getTblSupplier().getPicphoneNumber());
        printDataRetur.setNoTeleponSupplier(dataRetur.getTblSupplier().getSupplierPhoneNumber());
        printDataRetur.setKodeRetur(dataRetur.getCodeRetur());
//        printDataRetur.setKodeSuratJalan(dataRetur.getTb);
        List<ClassPrintDataReturDetail> listPrintReturDetail = new ArrayList();
        List<DetailLocation> listReturDetail = loadAllDataReturDetail();
        for (DetailLocation returDetail : listReturDetail) {
            ClassPrintDataReturDetail printReturDetail = new ClassPrintDataReturDetail();
          //  printReturDetail.setDiskon(returDetail.getTblDetail().getItemDiscount());
         //   printReturDetail.setHargaBarang(returDetail.getTblDetail().getItemCost());
            printReturDetail.setJumlah(returDetail.getTblDetail().getItemQuantity());
            printReturDetail.setKodeBarang(returDetail.getTblDetail().getTblSupplierItem().getSupllierItemCode());
            printReturDetail.setNamaBarang(returDetail.getTblDetail().getTblSupplierItem().getSupplierItemName());
            printReturDetail.setSatuan(returDetail.getTblDetail().getTblSupplierItem().getTblItem().getTblUnit().getUnitName());
          //  printReturDetail.setTotalHarga((printReturDetail.getHargaBarang().subtract(printReturDetail.getDiskon())).multiply(printReturDetail.getJumlah()));
            printDataRetur.setKodePO(returDetail.getTblDetail().getTblMemorandumInvoice().getTblPurchaseOrder().getCodePo());
          //  printDataRetur.setPajak(returDetail.getTblDetail().getTblMemorandumInvoice().getTblPurchaseOrder().getTaxPecentage());
            listPrintReturDetail.add(printReturDetail);
            printDataRetur.setKodeSuratJalan(returDetail.getTblDetail().getTblMemorandumInvoice().getDeliveryNumber());
        }
        printDataRetur.setListReturDetail(listPrintReturDetail);
        listDataRetur.add(printDataRetur);
        ClassPrinter.printRetur(listDataRetur, printDataRetur.getKodeRetur());
    }
            ...
            if (checkDataInputDataRetur()) {
                ...
                if (alert.getResult() == ButtonType.OK) {
                    ...
                    switch (dataInputStatus) {
                        case 0: //insert
                            if (getService().insertDataReturPV2(dummySelectedData,
...
//                    break;
    ...
    public void setTableDataDetail() {
        ...
        if (dataInputStatus == 3 || dataInputStatus == 4) {   //just for show
            tableView.getColumns().addAll(itemTitle, codeBarcode, expiredDate, cost, discount, quantityInfo, unitName, totalCostInfo, location);
        } else {
            tableView.getColumns().addAll(itemTitle, cost, codeBarcode, expiredDate, discount, quantity, unitName, totalCost, location);
        }
...
//        if (selectedData.getTblSupplier() != null) {
    ...
    private void showDataDetailItemDialog() {
        try {
            // Load the fxml file and create a new stage for the popup dialog.
            FXMLLoader loader = new FXMLLoader();
            loader.setLocation(HotelFX.class.getResource("view/feature_retur/retur_purchasing/ReturDetailLocationPDialog.fxml"));

            ReturDetailLocationPController controller = new ReturDetailLocationPController(this);
            loader.setController(controller);
...
//        setTableDataDetail();
    ...
    public ReturPController(){
        
    }
    
    public ReturPController(FeatureReturPController parentController) {
        this.parentController = parentController;
        this.fReturManager = parentController.getFReturManager();
    }

    private FeatureReturPController parentController;
