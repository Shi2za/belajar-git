 ...
 * @author ANDRI
 */
public class ReturDetailLocationPController implements Initializable {
    ...
    private void initDataDetailPopup() {
        ...
        tableMIDetail.getColumns().addAll(codeItem, itemName, itemTypeWH, expiredDate, propertyBarcode, quantityTitle);
    ...
    private List<MIDetail> loadAllDataMIDetail(TblMemorandumInvoice dataMI) {
        ...
        if (dataMI != null) {
            List<TblMemorandumInvoiceDetail> miDetails = returPController.getService().getAllDataMemorandumInvoiceDetailByIDMemorandumInvoice(dataMI.getIdmi());
            for (TblMemorandumInvoiceDetail miDetail : miDetails) {
                if (miDetail.getTblSupplierItem() != null) {   //not bonus
                    ...
                    miDetail.setTblSupplierItem(returPController.getService().getDataSupplierItem(miDetail.getTblSupplierItem().getIdrelation()));
                    miDetail.getTblSupplierItem().setTblItem(returPController.getService().getDataItem(miDetail.getTblSupplierItem().getTblItem().getIditem()));
                    if (miDetail.getTblSupplierItem().getTblItem().getTblItemTypeHk() != null) {
                        miDetail.getTblSupplierItem().getTblItem().setTblItemTypeHk(returPController.getService().getDataItemTypeHK(miDetail.getTblSupplierItem().getTblItem().getTblItemTypeHk().getIditemTypeHk()));
                    }
                    if (miDetail.getTblSupplierItem().getTblItem().getTblItemTypeWh() != null) {
                        miDetail.getTblSupplierItem().getTblItem().setTblItemTypeWh(returPController.getService().getDataItemTypeWH(miDetail.getTblSupplierItem().getTblItem().getTblItemTypeWh().getIditemTypeWh()));
                    }
                    if (miDetail.getTblSupplierItem().getTblItem().getConsumable()) {  //consumable
                        List<TblMemorandumInvoiceDetailItemExpiredDate> dataItemExpiredDates = returPController.getService().getAllDataMemorandumInvoiceDetailItemExpiredDateByIDMemorandumInvoiceDetail(miDetail.getIddetail());
                        for (TblMemorandumInvoiceDetailItemExpiredDate dataItemExpiredDate : dataItemExpiredDates) {
                            ...
                            dataItemExpiredDate.setTblItemExpiredDate(returPController.getService().getDataItemExpiredDate(dataItemExpiredDate.getTblItemExpiredDate().getIditemExpiredDate()));
                            dataItemExpiredDate.getTblItemExpiredDate().setTblItem(returPController.getService().getDataItem(dataItemExpiredDate.getTblItemExpiredDate().getTblItem().getIditem()));
                            if (dataItemExpiredDate.getTblItemExpiredDate().getTblItem().getTblItemTypeHk() != null) {
                                dataItemExpiredDate.getTblItemExpiredDate().getTblItem().setTblItemTypeHk(returPController.getService().getDataItemTypeHK(dataItemExpiredDate.getTblItemExpiredDate().getTblItem().getTblItemTypeHk().getIditemTypeHk()));
                            }
                            if (dataItemExpiredDate.getTblItemExpiredDate().getTblItem().getTblItemTypeWh() != null) {
                                dataItemExpiredDate.getTblItemExpiredDate().getTblItem().setTblItemTypeWh(returPController.getService().getDataItemTypeWH(dataItemExpiredDate.getTblItemExpiredDate().getTblItem().getTblItemTypeWh().getIditemTypeWh()));
                            }
                        ...
                        if (miDetail.getTblSupplierItem().getTblItem().getPropertyStatus()) {   //Property
                            List<TblMemorandumInvoiceDetailPropertyBarcode> dataPBs = returPController.getService().getAllDataMemorandumInvoiceDetailPropertyBarcodeByIDMemorandumInvoiceDetail(miDetail.getIddetail());
                            for (TblMemorandumInvoiceDetailPropertyBarcode dataPB : dataPBs) {
                                dataPB.setTblMemorandumInvoiceDetail(miDetail);
                                dataPB.setTblPropertyBarcode(new TblPropertyBarcode(returPController.getService().getDataPropertyBarcode(dataPB.getTblPropertyBarcode().getIdbarcode())));
                                TblPurchaseOrderDetail poDetail = getDataPurchaseOrderDetail(miDetail);
            ...
            for (int i = list.size() - 1; i > -1; i--) {
                for (ReturPController.DetailLocation data : returPController.detailLocations) {
        ...
        if (dataMIDetail != null) {
            if (dataMIDetail.getTblSupplierItem().getTblItem().getConsumable()) {    //Consumable
                if (dataMIDetailIED != null) {
                    List<TblReturDetail> dataReturDetails = returPController.getService().getAllDataReturDetailByIDMemorandumInvoiceAndIDSupplierItem(dataMIDetail.getTblMemorandumInvoice().getIdmi(),
                    ...
                    for (TblReturDetail dataReturDetail : dataReturDetails) {
                        if (dataReturDetail.getTblRetur().getRefReturStatus().getIdstatus() == 1) {   //Disetujui = '1'
                            List<TblReturDetailItemExpiredDate> returDetailItemExpiredDates = returPController.getService().getAllDataReturDetailItemExpiredDateByIDReturDetailAndIDItemExpiredDate(
                ...
                if (dataMIDetail.getTblSupplierItem().getTblItem().getPropertyStatus()) {    //Property
                    if (dataMIDetailPO != null) {
                        TblReturDetailPropertyBarcode dataReturDetailPropertyBarcode = returPController.getService().getDataReturDetailPropertyBarcodeByIDPropertyBarcode(dataMIDetailPO.getTblPropertyBarcode().getIdbarcode());
    ...
    public TblPurchaseOrderDetail getDataPurchaseOrderDetail(TblMemorandumInvoiceDetail dataMIDetail) {
        ...
        if (dataMIDetail != null) {
            dataPODetail = returPController.getService().getDataPurchaseOrderDetailByIDPurchaseOrderAndIDSupplierItem(
    ...
    private List<TblLocationOfWarehouse> loadAllDataLocationOfWarehouse() {
        List<TblLocationOfWarehouse> list = returPController.getService().getAllDataWarehouse();
        for (TblLocationOfWarehouse data : list) {
            //data location
            data.setTblLocation(returPController.getService().getDataLocation(data.getTblLocation().getIdlocation()));
        }
        return list;
    }
    ...
    private void refreshDataPopup() {
        //mi - detail
        ObservableList<MIDetail> miDetailItems = FXCollections.observableArrayList(loadAllDataMIDetail(returPController.cbpMI.getValue()));
    ...
    private void setSelectedDataToInputForm() {
        ...
        txtItemQuantity.textProperty().bindBidirectional(returPController.selectedDataDetailLocation.getTblDetail().itemQuantityProperty(), new ClassFormatter.CBigDecimalStringConverter());

        returPController.selectedDataDetailLocation.getTblDetail().itemQuantityProperty().addListener((obs, oldVal, newVal) -> {
            if (newVal != null) {
                //data retur cost
        ...
        cbpWarehouse.valueProperty().bindBidirectional(returPController.selectedDataDetailLocation.tblLocationWarehouseProperty());

        cbpMIDetail.hide();
        cbpWarehouse.hide();

        if (returPController.dataInputDetailStatus == 1) { //update
            //set current data
            cbpMIDetail.setValue(getMIDetailFromDetailLocation(returPController.selectedDataDetailLocation));
            //disable data input fields
            cbpMIDetail.setDisable(true);
        }
    }

    private MIDetail getMIDetailFromDetailLocation(ReturPController.DetailLocation dataDetailLocation) {
        if (dataDetailLocation != null) {
            TblMemorandumInvoiceDetail miDetail = returPController.getService().getDataMemorandumInvoiceDetailByIDMemorandumInvoiceAndIDSupplierItem(
    ...
    private void refreshDataItem() {
        if (returPController.selectedDataDetailLocation.getTblDetail().getTblMemorandumInvoice() != null
                && returPController.cbpMI.getValue() != null
            ...
            if (returPController.dataInputDetailStatus == 0) {  //insert
                if (cbpMIDetail.getValue().getDataMIDetail().getTblSupplierItem().getTblItem().getPropertyStatus()) {    //Property
                    returPController.selectedDataDetailLocation.getTblDetail().setItemQuantity(new BigDecimal("1"));
                } else {
                    returPController.selectedDataDetailLocation.getTblDetail().setItemQuantity(new BigDecimal("0"));
                }
            }
        } else {
    ...
    private BigDecimal calculationTotalReturCost() {
        ...
        if (returPController.selectedDataDetailLocation.getTblDetail().getTblMemorandumInvoice() != null
                && returPController.cbpMI.getValue() != null
                && cbpMIDetail.getValue() != null) {
            result = (returPController.selectedDataDetailLocation.getTblDetail().getItemQuantity()
                    .multiply((cbpMIDetail.getValue().getItemCost().subtract(cbpMIDetail.getValue().getItemDiscount()))))
                    .multiply((new BigDecimal("1")).add(returPController.cbpMI.getValue().getTblPurchaseOrder().getTaxPecentage()));
        }
        return result;
    }
    ...
    private void dataDetailSaveHandle() {
        if (checkDataInputDataDetail()) {
            Alert alert = ClassMessage.showConfirmationSavingDataMessage("", returPController.dialogStageDetal);
            if (alert.getResult() == ButtonType.OK) {
                if (returPController.dataInputDetailStatus == 0) { //insert
                    ClassMessage.showSucceedAddingDataMessage("", returPController.dialogStageDetal);
                    //set new data
                    returPController.selectedDataDetailLocation.getTblDetail().setTblMemorandumInvoice(returPController.cbpMI.getValue());
                    returPController.selectedDataDetailLocation.getTblDetail().setTblSupplierItem(cbpMIDetail.getValue().getDataMIDetail().getTblSupplierItem());
                    returPController.selectedDataDetailLocation.getTblDetail().setTblLocation(cbpWarehouse.getValue().getTblLocation());
                    returPController.selectedDataDetailLocation.getTblDetail().setItemCost(cbpMIDetail.getValue().getItemCost());
                    returPController.selectedDataDetailLocation.getTblDetail().setItemDiscount(cbpMIDetail.getValue().getItemDiscount());
                    returPController.selectedDataDetailLocation.getTblDetail().setItemTaxPercentage(returPController.cbpMI.getValue().getTblPurchaseOrder().getTaxPecentage());
                    if (cbpMIDetail.getValue().getDataPropertyBarcode() != null) {
                        ...
                        dataReturDetailPropertyBarcode.setTblReturDetail(returPController.selectedDataDetailLocation.getTblDetail());
                        ...
                        returPController.selectedDataDetailLocation.setTblDetailPropertyBarcode(dataReturDetailPropertyBarcode);
                    }
                    if (cbpMIDetail.getValue().getDataItemExpiredDate() != null) {
                        ...
                        dataReturDetailItemExpiredDate.setTblReturDetail(returPController.selectedDataDetailLocation.getTblDetail());
                        ...
                        returPController.selectedDataDetailLocation.setTblDetailItemExpiredDate(dataReturDetailItemExpiredDate);
                    }
                    //add to list
                    returPController.detailLocations.add(returPController.selectedDataDetailLocation);
                } else {  //update
                    ClassMessage.showSucceedUpdatingDataMessage("", returPController.dialogStageDetal);
                    //set to list
                    returPController.detailLocations.set(
                            getSectedIndex((ReturPController.DetailLocation) returPController.tableReturDetail.getTableView().getSelectionModel().getSelectedItem(),
                                    returPController.detailLocations),
                            returPController.selectedDataDetailLocation);
                }
                //refresh data retur
                returPController.refreshData();
    ...
    private int getSectedIndex(ReturPController.DetailLocation dataLocation,
            List<ReturPController.DetailLocation> dataLocations) {
    ...
    private boolean checkDataInputDataDetail() {
        ...
        if (txtItemQuantity.getText() == null
            ...
            if (returPController.selectedDataDetailLocation.getTblDetail().getItemQuantity()
    ...
    public ReturDetailLocationPController(ReturPController parentController) {
