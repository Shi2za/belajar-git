...
public class RCTIReservationVoucherController implements Initializable {
    ...
    private List<TblReservationVoucher> loadAllDataReservationVoucher() {
        List<TblReservationVoucher> list = rcTransactionInputDetailController.getParentController().getParentController().getParentController().getServiceRV2().getAllDataReservationVoucher();
        for (int i = list.size() - 1; i > -1; i--) {
            list.get(i).setRefVoucherStatus(rcTransactionInputDetailController.getParentController().getParentController().getParentController().getServiceRV2().getVoucherStatus(list.get(i).getRefVoucherStatus().getIdstatus()));
