...
//    private final JFXCComboBoxPopup<TblBankEventCard> cbpBankCardEvent = new JFXCComboBoxPopup<>(TblBankEventCard.class);
    private void initFormDataRCTICreditCard() {
        ...
        spnExpiredMonth.valueFactoryProperty().addListener((obs, oldVal, newVal) -> {
            reservationCanceledInputController.getParentController().selectedDataTransactionWithBankCard.setBankCardExprDate(getExpiredDate(newVal.getValue(), spnExpiredYear.getValue()));
        });

        spnExpiredYear.valueFactoryProperty().addListener((obs, oldVal, newVal) -> {
            reservationCanceledInputController.getParentController().selectedDataTransactionWithBankCard.setBankCardExprDate(getExpiredDate(spnExpiredMonth.getValue(), newVal.getValue()));
        });
...
//        cbpBankCardEvent.setItems(bankCardEventItems);
    ...
    private void setSelectedDataToInputForm() {
        ...
        txtCardHolder.textProperty().bindBidirectional(reservationCanceledInputController.getParentController().selectedDataTransactionWithBankCard.bankCardHolderProperty());
...
//            }
//        });
        if (reservationCanceledInputController.getParentController().selectedDataTransactionWithBankCard.getBankCardExprDate() != null) {
            spnExpiredMonth.getValueFactory().setValue(reservationCanceledInputController.getParentController().selectedDataTransactionWithBankCard.getBankCardExprDate().getMonth() + 1);
            spnExpiredYear.getValueFactory().setValue(reservationCanceledInputController.getParentController().selectedDataTransactionWithBankCard.getBankCardExprDate().getYear() + 1900 - 2000);
        } else {
...
//            Alert alert = ClassMessage.showConfirmationSavingDataMessage("", reservationCanceledInputController.getParentController().dialogStage);
//            if (alert.getResult() == ButtonType.OK) {
//                reservationCanceledInputController.getParentController().selectedDataTransactionWithBankCard.setBankCardExprDate(getExpiredDate(spnExpiredMonth.getValue(), spnExpiredYear.getValue()));
//                reservationCanceledInputController.getParentController().selectedDataTransactionWithBankCard.setPaymentDiscount(calculationDiscountPayment());
//                reservationCanceledInputController.getParentController().selectedDataTransactionWithBankCard.setPaymentCharge(calculationMDRChargePayment());
//                reservationCanceledInputController.getParentController().selectedDataTransactionWithBankCard.setTblBankAccount(
//                        reservationCanceledInputController.getParentController().selectedDataTransactionWithBankCard.getTblBankEdc().getTblBankAccount());
//                //save data transaction & close dialog data rc-transaction detail
//                reservationCanceledInputController.getParentController().saveDataTransaction();
//            }
//        } else {
//            ClassMessage.showWarningInputDataMessage(errDataInput, reservationCanceledInputController.getParentController().dialogStage);
//        }
//    }
    ...
    public RCTICreditCardController(ReservationCanceledInputController parentController) {
