 ...
 */
package hotelfx.view.feature_reservation_v2.rr;
...
import hotelfx.view.feature_reservation_v2.ReservationRescheduleController;
 ...
 */
public class RRReservationRoomTypeDetailInputController implements Initializable {
    ...
    private void setCalendarFunction() {
        ...
        btnBack.setOnMouseClicked((e) -> {
            //close dialog
            reservationRescheduleController.dialogStage.close();
        });
...
//            btnGotToCurrentDatePosition.setText(ClassFormatter.dateFormate.format(Date.valueOf(LocalDate.now())));
    ...
    private List<SelectedDiscount> loadAllDataSelectedDiscount() {
        ...
        //data hotel-event
        SelectedDiscount dataHotelDiscount = new SelectedDiscount(null, reservationRescheduleController.getParent().getFReservationManager().getReservationBillDiscountType(0));  //Hotel-Event = '0'
        list.add(dataHotelDiscount);
        //data card-event
        List<TblBankCard> dataBankCards = reservationRescheduleController.getParent().getFReservationManager().getAllDataBankCard();
        for (TblBankCard dataBankCard : dataBankCards) {
            //data bank
            dataBankCard.setTblBank(reservationRescheduleController.getParent().getFReservationManager().getBank(dataBankCard.getTblBank().getIdbank()));
            //data bank card type
            dataBankCard.setRefBankCardType(reservationRescheduleController.getParent().getFReservationManager().getDataBankCardType(dataBankCard.getRefBankCardType().getIdtype()));
            //data selected discount
            SelectedDiscount dataCardDiscount = new SelectedDiscount(dataBankCard, reservationRescheduleController.getParent().getFReservationManager().getReservationBillDiscountType(1));  //Card-Event = '1'
    ...
    private void refreshDataPopup() {
        ...
        cbpSelectedDiscount.setValue(new SelectedDiscount(
                reservationRescheduleController.getParent().dataReservationBill.getTblBankCard() != null ? new TblBankCard(reservationRescheduleController.getParent().dataReservationBill.getTblBankCard()) : null,
                reservationRescheduleController.getParent().dataReservationBill.getRefReservationBillDiscountType() != null ? new RefReservationBillDiscountType(reservationRescheduleController.getParent().dataReservationBill.getRefReservationBillDiscountType()) : null
        ));

        //set disable
        cbpSelectedDiscount.setDisable(!reservationRescheduleController.getParent().dataReservationRoomTypeDetails.isEmpty()
                || (reservationRescheduleController.getParent().selectedData.getTblPartner() != null));
    ...
    private ObservableList<TblRoomType> loadAllDataRoomType() {
        return FXCollections.observableArrayList(reservationRescheduleController.getParent().getFReservationManager().getAllDataRoomType());
    }
    ...
    private String getDataRoomTypeHoverInfo(TblRoomType roomType) {
        ...
        result += "\n";
        List<TblFloor> dataFloors = reservationRescheduleController.getParent().getFReservationManager().getAllDataFloor();
        List<TblRoom> dataRooms = reservationRescheduleController.getParent().getFReservationManager().getAllDataRoomByIDRoomType(roomType.getIdroomType());
        ...
        result += "\n";
        result += "Fasilitas :  \n";
        List<TblRoomTypeItem> dataRoomItems = reservationRescheduleController.getParent().getFReservationManager().getAllDataRoomTypeItemByIDRoomType(roomType.getIdroomType());
        ...
        result += "\n";
        result += "Layanan :  \n";
        List<TblRoomTypeRoomService> dataRoomServices = reservationRescheduleController.getParent().getFReservationManager().getAllDataRoomTypeRoomServiceByIDRoomType(roomType.getIdroomType());
...
//                    gpCalendar.setGridLinesVisible(true);
                    ...
                    for (int j = 0; j < roomTypes.size(); j++) {
                        ...
                        CalendarRoomTypeAvailableButton button = getCalendarRoomTypeAvailableButton(roomTypes.get(j),
                                ...
                                getRoomWantToCancelNumber(roomTypes.get(j), dateInPosition),
    ...
    private int getRoomReservedNumber(TblRoomType roomType, LocalDate date) {
        int result = 0;
        if (reservationRescheduleController.getParent().reservationType.getIdtype() == 0) {   //Customer = '0'
            List<TblReservationRoomPriceDetail> roomPriceDetails = reservationRescheduleController.getParent().getFReservationManager().getAllDataReservationRoomPriceDetailByDetailDate(Date.valueOf(date));
            for (TblReservationRoomPriceDetail roomPriceDetail : roomPriceDetails) {
                TblReservationRoomTypeDetailRoomPriceDetail relation = reservationRescheduleController.getParent().getFReservationManager().getReservationRoomTypeDetailRoomPriceDetailByIDRoomPriceDetail(roomPriceDetail.getIddetail());
                TblReservationRoomTypeDetail roomTypeDetail = reservationRescheduleController.getParent().getFReservationManager().getReservationRoomTypeDetail(relation.getTblReservationRoomTypeDetail().getIddetail());
                roomTypeDetail.setTblReservation(reservationRescheduleController.getParent().getFReservationManager().getReservation(roomTypeDetail.getTblReservation().getIdreservation()));
                roomTypeDetail.getTblReservation().setRefReservationStatus(reservationRescheduleController.getParent().getFReservationManager().getReservationStatus(roomTypeDetail.getTblReservation().getRefReservationStatus().getIdstatus()));
...
//                if (roomTypeDetail.getTblRoomType().getRoomTypeName().equals(roomType.getRoomTypeName())    @@@+++
                if (roomTypeDetail.getTblRoomType().getIdroomType() == roomType.getIdroomType()
                        ...
                        && (roomTypeDetail.getTblReservation().getRefReservationStatus().getIdstatus() == 1 //Reserved = '1'
                        || roomTypeDetail.getTblReservation().getRefReservationStatus().getIdstatus() == 2)) {    //Booked = '2'
                    TblReservationRoomTypeDetailTravelAgentDiscountDetail relation1 = reservationRescheduleController.getParent().getFReservationManager().getReservationRoomTypeDetailTravelAgentDiscountDetailByIDReservationRoomTypeDetail(roomTypeDetail.getIddetail());
                    TblReservationTravelAgentDiscountDetail travelAgentDiscountDetail = reservationRescheduleController.getParent().getFReservationManager().getReservationTravelAgentDiscountDetail(relation1.getTblReservationTravelAgentDiscountDetail().getIddetail());
                    if (reservationRescheduleController.getParent().currentTravelAgent.getTblPartner().getIdpartner() == travelAgentDiscountDetail.getTblPartner().getIdpartner()) {
                        result++;
                    }
                }
            }
    ...
    private int getRoomWantToCancelNumber(TblRoomType roomType, LocalDate date) {
        int result = 0;
        for (TblReservationRoomTypeDetail roomTypeDetail : reservationRescheduleController.getParent().dataReservationRoomTypeDetails) {
...
//                                        roomTypeDetail.getCheckInDateTime().getDate()))) {
    ...
    private int getRoomWantToReserveNumber(TblRoomType roomType, LocalDate date) {
        int result = 0;
        for (TblReservationRoomTypeDetail roomTypeDetail : reservationRescheduleController.newDataReservationRoomTypeDetails) {
            LocalDate checkInDate = LocalDate.of(
                    roomTypeDetail.getCheckInDateTime().getYear() + 1900,
                    roomTypeDetail.getCheckInDateTime().getMonth() + 1,
                    roomTypeDetail.getCheckInDateTime().getDate());
...
//                                        roomTypeDetail.getCheckInDateTime().getDate()))) {
    ...
    private int getRoomAvailableNumber(TblRoomType roomType, LocalDate date) {
        int result = 0;
        if (reservationRescheduleController.getParent().reservationType.getIdtype() == 0) {   //Customer = '0'
            List<TblRoom> listRoom = reservationRescheduleController.getParent().getFReservationManager().getAllDataRoomByIDRoomType(roomType.getIdroomType());
            ...
            List<TblTravelAgentRoomType> travelAgentRoomTypes = reservationRescheduleController.getParent().getFReservationManager().getAllDataTravelAgentRoomTypeByIDRoomTypeAndAvailableDate(
            ...
            TblTravelAgentRoomType travelAgentRoomType = reservationRescheduleController.getParent().getFReservationManager().getDataTravelAgentRoomTypeByIDRoomTypeAndIDPartnerAndAvailableDate(roomType.getIdroomType(),
                    reservationRescheduleController.getParent().currentTravelAgent.getTblPartner().getIdpartner(),
                    Date.valueOf(date));
    ...
    private BigDecimal getRoomTypeCurrentPrice(TblRoomType roomType, LocalDate date) {
        return getRoomPrice(reservationRescheduleController.getParent().reservationType, roomType, date);
    }
        ...
        if (reservationRescheduleController.getParent().reservationType.getIdtype() == 0 //Customer = '0'
            ...
            if (selectedDisount.getDataDiscountType().getIdtype() == 0) { //Hotel-Event = '0'
                if (roomType.getHotelDiscountable()) {
                    List<TblHotelEvent> dataHotelEvents = reservationRescheduleController.getParent().getFReservationManager().getAllDataHotelEventByDateInRangeOrderByMaxRoomDiscountPercentage(Date.valueOf(date));
            ...
            if (selectedDisount.getDataDiscountType().getIdtype() == 1) { //Card-Event = '1'
                if (roomType.getCardDiscountable()) {
                    List<TblBankEventCard> dataCardEvents = reservationRescheduleController.getParent().getFReservationManager().getAllDataBankEventCardByDateInRangeAndIDBankCardAllowNULLAndIDBankOrderByMaxRoomDiscountPercentage(
                            Date.valueOf(date),
    ...
    private BigDecimal getBreakfastCost(boolean usedBreakfast) {
        BigDecimal result = new BigDecimal("0");
        if (usedBreakfast) {
            TblRoomService dataRoomService = reservationRescheduleController.getParent().getFReservationManager().getRoomService(1);   //Breakfast = '1';
    ...
    private BigDecimal getBreakfastDiscountPercentage(LocalDate date,
        ...
        if (reservationRescheduleController.getParent().reservationType.getIdtype() == 0 //Customer = '0'
                && selectedDisount != null
                && selectedDisount.getDataDiscountType() != null) {
            TblRoomService dataBreakfast = reservationRescheduleController.getParent().getFReservationManager().getRoomService(1);    //Breakfast = '1'
            if (selectedDisount.getDataDiscountType().getIdtype() == 0) { //Hotel-Event = '0'
    ...
    private CalendarRoomTypeAvailableButton getCalendarRoomTypeAvailableButton(TblRoomType roomType, LocalDate date,
            int roomReservedNumber, int roomWantToCancelNumber, int roomWantToReserveNumber, int roomAvailableNumber,
        ...
        CalendarRoomTypeAvailableButton button = new CalendarRoomTypeAvailableButton(
                getDataInfo(roomType, roomReservedNumber, roomWantToCancelNumber, roomWantToReserveNumber, roomAvailableNumber, roomPrice, discountRoomPercentage, discountRoomName, breakfastCost, discountBreakfastPercentage, discountBreakfastName),
                roomType, date, roomReservedNumber, roomWantToCancelNumber, roomWantToReserveNumber, roomAvailableNumber, roomPrice, discountRoomPercentage, discountRoomName);
//        button.getStyleClass().add("button-calendar-room-type-available");
        ...
        button.setTooltip(new Tooltip(getDataHoverInfo(roomType, roomReservedNumber, roomWantToCancelNumber, roomWantToReserveNumber, roomAvailableNumber, roomPrice, discountRoomPercentage, discountRoomName, breakfastCost, discountBreakfastPercentage, discountBreakfastName)));
        return button;
    }
...
//        result = "( " + (roomReservedNumber + roomWantToReserveNumber) + " / " + roomAvailableNumber + " )\n";
        int currentRoomAvalableNumber = roomAvailableNumber - (roomReservedNumber - roomWantToCancelNumber + roomWantToReserveNumber);
        ...
        result += ClassFormatter.currencyFormat.cFormat(((roomPrice.multiply((new BigDecimal("1")).subtract(discountRoomPercentage.divide(new BigDecimal("100")))))
                ...
                .multiply((new BigDecimal("1")).add(reservationRescheduleController.getParent().dataReservationBill.getServiceChargePercentage()))
                .multiply((new BigDecimal("1")).add(reservationRescheduleController.getParent().dataReservationBill.getTaxPercentage())));
        return result;
    }
    ...
    private String getDataHoverInfo(TblRoomType roomType,
        ...
        result += "Kamar Dipesan : " + (roomReservedNumber - roomWantToCancelNumber + roomWantToReserveNumber) + " \n";
        result += "Sisa Kamar : " + (roomAvailableNumber - (roomReservedNumber - roomWantToCancelNumber + roomWantToReserveNumber)) + " \n";
        result += "\n";
...
//        result += "Diskon Breakfast : " + ClassFormatter.decimalFormat.cFormat((discountBreakfastPercentage)) + "% " + (discountBreakfastName.equals("") ? "" : "- " + discountBreakfastName) + "\n";
    ...
    private void setAvailableButtonToDoubleClick(CalendarRoomTypeAvailableButton button) {
        ...
        if (LocalDateTime.now().isBefore(LocalDateTime.of(minDateToChange, LocalTime.of(reservationRescheduleController.getParent().defaultCheckInTime.getHours(), reservationRescheduleController.getParent().defaultCheckInTime.getMinutes())))) {
...
//                button.setDisable(true);
    ...
    private CalendarRoomTypeAvailableButton getCalendarRoomTypeAvailableButton(TblRoomType roomType, LocalDate date) {
        CalendarRoomTypeAvailableButton button = new CalendarRoomTypeAvailableButton("", roomType, date, 0, 0, 0, 0,
    ...
    class CalendarRoomTypeAvailableButton extends JFXButton {
        ...
        private int roomWantToCancelNumber;
        ...
        public CalendarRoomTypeAvailableButton(String text, TblRoomType value, LocalDate date,
            ...
            this.roomWantToCancelNumber = roomWantToCancelNumber;
        ...
        public int getRoomWantToCancelNumber(){
            return roomWantToCancelNumber;
        }
        
        public void setRoomWantToCancelNumber(int roomWantToCancelNumber){
            this.roomWantToCancelNumber = roomWantToCancelNumber;
        }
    ...
    private void dataRRTDSaveHandle() {
        if (checkDataInputDataRRTD()) {
            if (checkRoomAvailable()) {
                Alert alert = ClassMessage.showConfirmationSavingDataMessage("", reservationRescheduleController.dialogStage);
                if (alert.getResult() == ButtonType.OK) {
                    ...
                    if (cbpSelectedDiscount.getValue() != null) {
                        ...
                        if (cbpSelectedDiscount.getValue().getDataBankCard() != null) {
                            reservationRescheduleController.getParent().dataReservationBill.setTblBankCard(new TblBankCard(cbpSelectedDiscount.getValue().getDataBankCard()));
                        } else {
                            reservationRescheduleController.getParent().dataReservationBill.setTblBankCard(null);
                        }
                        //data reservation bill discount type
                        if (cbpSelectedDiscount.getValue().getDataDiscountType() != null) {
                            reservationRescheduleController.getParent().dataReservationBill.setRefReservationBillDiscountType(new RefReservationBillDiscountType(cbpSelectedDiscount.getValue().getDataDiscountType()));
                        } else {
                            reservationRescheduleController.getParent().dataReservationBill.setRefReservationBillDiscountType(null);
                        }
                    }
                    //reservation room type detail (selected)
                    reservationRescheduleController.newSelectedDataRoomTypeDetails = new ArrayList<>();
                    reservationRescheduleController.newSelectedDataRoomTypeDetailRoomPriceDetails = new ArrayList<>();
                    reservationRescheduleController.newSelectedDataRoomTypeDetailTravelAgentDiscountDetails = new ArrayList<>();
                    reservationRescheduleController.newSelectedDataAdditionalServices = new ArrayList<>();
                    reservationRescheduleController.newSelectedDataAdditionalItems = new ArrayList<>();
                    //data room servce - breakfast
                    TblRoomService dataBreakfast = reservationRescheduleController.getParent().getFReservationManager().getRoomService(1);    //Breakfast = '1'
                    //data reservation bill type - reservation
                    RefReservationBillType dataReservationBillTypeReservation = reservationRescheduleController.getParent().getFReservationManager().getDataReservationBillType(0);    //Reservation = '0'
                    ...
                    for (int i = 0; i < roomNumber.get(); i++) {
                        ...
                        dataRRTD.setTblReservation(reservationRescheduleController.getParent().selectedData);
                        dataRRTD.setTblRoomType(selectRoomType);
//                    dataRRTD.setCheckInDateTime(Date.valueOf(dtpCheckInDate.getValue()));
                        dataRRTD.setCheckInDateTime(Timestamp.valueOf(LocalDateTime.of(dtpCheckInDate.getValue(), reservationRescheduleController.getParent().defaultCheckInTime.toLocalTime())));
//                    dataRRTD.setCheckOutDateTime(Date.valueOf(dtpCheckOutDate.getValue()));
                        dataRRTD.setCheckOutDateTime(Timestamp.valueOf(LocalDateTime.of(dtpCheckOutDate.getValue(), reservationRescheduleController.getParent().defaultCheckOutTime.toLocalTime())));
                        if (!(LocalDate.of(dataRRTD.getCheckInDateTime().getYear() + 1900,
                            ...
                            dataRRTD.getTblReservationCheckInOut().setRefReservationCheckInOutStatus(reservationRescheduleController.getParent().getFReservationManager().getReservationCheckInOutStatus(0)); //Ready to Check In = '0'
                        }
                        dataRRTD.setRefReservationOrderByType(reservationRescheduleController.getParent().reservationType);
                        ...
                        while (countDate.isBefore(dtpCheckOutDate.getValue())) {
                            ...
                            dataRPD.setDetailPrice(getRoomPrice(reservationRescheduleController.getParent().reservationType, selectRoomType, countDate));
                            //reservation room price detail - discount
                            setReservationRoomPriceDetailDiscount(dataRPD, dataRRTD, reservationRescheduleController.getParent().dataReservationBill);
                            //reservation room price detail - complimentary
                            dataRPD.setDetailComplimentary(new BigDecimal("0"));
                            //reservation room price detail - create date (reserved date)
                            dataRPD.setCreateDate(Timestamp.valueOf(LocalDateTime.now()));
                            ...
                            dataRTDRPD.setCreateDate(Timestamp.valueOf(LocalDateTime.now()));
                            //add to list (reservation room type detail - reservation room price detail)
                            reservationRescheduleController.newSelectedDataRoomTypeDetailRoomPriceDetails.add(dataRTDRPD);
                            reservationRescheduleController.newDataReservationRoomTypeDetailRoomPriceDetails.add(dataRTDRPD);
                            //if reservation with travel agent
                            if (reservationRescheduleController.getParent().reservationType.getIdtype() == 1) {   //Travel Agent = '1'
                                ...
                                dataRTADD.setTblPartner(reservationRescheduleController.getParent().currentTravelAgent.getTblPartner());
                                dataRTADD.setDetailDiscount(reservationRescheduleController.getParent().currentTravelAgent.getRoomTypeDiscountPercentage());
                                dataRTADD.setCreateDate(Timestamp.valueOf(LocalDateTime.now()));
                                ...
                                dataRRTDTADD.setCreateDate(Timestamp.valueOf(LocalDateTime.now()));
                                //add to list (reservation room type detail - reservation travel agent detail)
                                reservationRescheduleController.newSelectedDataRoomTypeDetailTravelAgentDiscountDetails.add(dataRRTDTADD);
                                reservationRescheduleController.newDataReservationRoomTypeDetailTravelAgentDiscountDetails.add(dataRRTDTADD);
                            }
                            ...
                            if (chbUseBreakfast.isSelected()) {
                                ...
                                setReservationAdditionalBreakfastDiscount(dataAdditionalService, reservationRescheduleController.getParent().dataReservationBill);
                                //add to list (reservation additional service : breakfast)
                                reservationRescheduleController.newSelectedDataAdditionalServices.add(dataAdditionalService);
                                reservationRescheduleController.newDataReservationAdditionalServices.add(dataAdditionalService);
                                reservationRescheduleController.tableDataReservationAdditionalServiceNew.getTableView().setItems(FXCollections.observableArrayList(reservationRescheduleController.newDataReservationAdditionalServices));
                            }
                            //data reservation additional service (include)
                            List<TblRoomTypeRoomService> dataRTRSs = reservationRescheduleController.getParent().getFReservationManager().getAllDataRoomTypeRoomServiceByIDRoomType(dataRRTD.getTblRoomType().getIdroomType());
                            for (TblRoomTypeRoomService dataRTRS : dataRTRSs) {
                                if (dataRTRS.getAddAsAdditionalService()) {
                                    if (dataRTRS.getPeopleNumber() == 0) {
                                        ...
                                        dataAdditionalService.setRefReservationBillType(reservationRescheduleController.getParent().getFReservationManager().getDataReservationBillType(4));  //Include = '4'
                                        dataAdditionalService.setCreateDate(Timestamp.valueOf(LocalDateTime.now()));
...
//                                            dataAdditionalService.setNote("Jumlah Orang : 1");
                            ...
                            List<TblRoomTypeItem> dataRTIs = reservationRescheduleController.getParent().getFReservationManager().getAllDataRoomTypeItemByIDRoomType(dataRRTD.getTblRoomType().getIdroomType());
                            for (TblRoomTypeItem dataRTI : dataRTIs) {
                                if (dataRTI.getAddAsAdditionalItem()) {
                                    ...
                                    dataAddtionalItem.setRefReservationBillType(reservationRescheduleController.getParent().getFReservationManager().getDataReservationBillType(4));  //Include = '4'
                                    dataAddtionalItem.setCreateDate(Timestamp.valueOf(LocalDateTime.now()));
                                    //add to list (reservation additional item : include)
                                    reservationRescheduleController.newSelectedDataAdditionalItems.add(dataAddtionalItem);
                                    reservationRescheduleController.newDataReservationAdditionalItems.add(dataAddtionalItem);
                                    reservationRescheduleController.tableDataReservationAdditionalItemNew.getTableView().setItems(FXCollections.observableArrayList(reservationRescheduleController.newDataReservationAdditionalItems));
                                }
                            }
                            //increment countDate
                        ...
                        reservationRescheduleController.newSelectedDataRoomTypeDetails.add(dataRRTD);
                        reservationRescheduleController.newDataReservationRoomTypeDetails.add(dataRRTD);
                    }
                    //recode
                    int countID = 1;
                    for (TblReservationRoomTypeDetail newDataReservationRoomTypeDetail : reservationRescheduleController.newDataReservationRoomTypeDetails) {
                        String codeDetail = "";
                        for (int i = 0; i < 5; i++) {
                            codeDetail += "0";
                        }
                    ...
                    reservationRescheduleController.tableDataReservationRoomTypeDetailNew.getTableView().setItems(FXCollections.observableArrayList(reservationRescheduleController.newDataReservationRoomTypeDetails));
                    //refresh reservation bill
                    reservationRescheduleController.refreshDataBillNew();
                    reservationRescheduleController.initTableDataRCOTransactionNew();
                    ClassMessage.showSucceedInsertingDataMessage("", reservationRescheduleController.dialogStage);
...
//                    selectedDate.set(LocalDate.now());
                ...
                ClassMessage.showFailedInsertingDataMessage("Jumlah kamar tidak mencukupi..!!", reservationRescheduleController.dialogStage);
            }
        } else {
            ClassMessage.showWarningInputDataMessage(errDataInput, reservationRescheduleController.dialogStage);
        }
    }
    ...
    private BigDecimal getRoomPrice(RefReservationOrderByType reservationType,
        ...
        TblReservationCalendarBar calendarBAR = reservationRescheduleController.getParent().getFReservationManager().getReservationCalendarBARByCalendarDate(Date.valueOf(date));
        if (calendarBAR == null) {
            TblReservationDefaultBar defaultBAR = reservationRescheduleController.getParent().getFReservationManager().getReservationDefaultBARByDayOfWeek(date.getDayOfWeek().getValue());
            dataBAR = reservationRescheduleController.getParent().getFReservationManager().getReservationBAR(defaultBAR.getTblReservationBar().getIdbar());
        } else {
            dataBAR = reservationRescheduleController.getParent().getFReservationManager().getReservationBAR(calendarBAR.getTblReservationBar().getIdbar());
        }
        switch (reservationType.getIdtype()) {
            ...
            case 1:    //Travel Agent = '1'
                result = (roomType.getRoomTypePrice().multiply(dataBAR.getBarpercentage()))
                        .multiply((new BigDecimal("1")).subtract(reservationRescheduleController.getParent().currentTravelAgent.getRoomTypeDiscountPercentage().divide(new BigDecimal("100"))));
                break;
            default:
                break;
        }
        ...
        if (dataReservationBill.getRefReservationBillDiscountType() != null) {
            if (dataReservationBill.getRefReservationBillDiscountType().getIdtype() == 0) { //Hotel-Event = '0'
                if (dataRoomTypeDetail.getTblRoomType().getHotelDiscountable()) {
                    List<TblHotelEvent> dataHotelEvents = reservationRescheduleController.getParent().getFReservationManager().getAllDataHotelEventByDateInRangeOrderByMaxRoomDiscountPercentage(dataReservationRoomPriceDetail.getDetailDate());
                    if (!dataHotelEvents.isEmpty()) {
                        for (TblHotelEvent dataHotelEvent : dataHotelEvents) {
        ...
        if (dataReservationBill.getRefReservationBillDiscountType() != null) {
            if (dataReservationBill.getRefReservationBillDiscountType().getIdtype() == 0) { //Hotel-Event = '0'
                if (dataReservationAdditionalService.getTblRoomService().getHotelDiscountable()) {
                    List<TblHotelEvent> dataHotelEvents = reservationRescheduleController.getParent().getFReservationManager().getAllDataHotelEventByDateInRangeOrderByMaxServiceDiscountPercentage(dataReservationAdditionalService.getAdditionalDate());
                    if (!dataHotelEvents.isEmpty()) {
                        for (TblHotelEvent dataHotelEvent : dataHotelEvents) {
                ...
                if (dataReservationBill.getRefReservationBillDiscountType().getIdtype() == 1) { //Card-Event = '1'
                    if (dataReservationAdditionalService.getTblRoomService().getCardDiscountable()) {
                        List<TblBankEventCard> dataCardEvents = reservationRescheduleController.getParent().getFReservationManager().getAllDataBankEventCardByDateInRangeAndIDBankCardAllowNULLAndIDBankOrderByMaxServiceDiscountPercentage(
...
//        selectedDate.set(LocalDate.now());
    ...
    private boolean checkRoomAvailable() {
        ...
        while (dateCount.isBefore(dtpCheckOutDate.getValue())) {
            int availableNumber = getRoomAvailableNumber(selectRoomType, dateCount)
                    ...
                    + getRoomWantToCancelNumber(selectRoomType, dateCount)
    ...
    public RRReservationRoomTypeDetailInputController(ReservationRescheduleController parentController) {
        reservationRescheduleController = parentController;
    }

    private final ReservationRescheduleController reservationRescheduleController;
