 ...
 */
public class RRReservationAdditionalServiceInputController implements Initializable {
    ...
    private List<TblRoomService> loadAllDataService() {
        List<TblRoomService> list = reservationRescheduleController.getParent().getFReservationManager().getAllDataRoomService();
    ...
    private void initDataTableDateSelected(TblReservationRoomTypeDetail dataRRTD) {
        ...
        dataDate.setCellValueFactory((TableColumn.CellDataFeatures<RTDListDate, String> param)
                -> Bindings.createStringBinding(()
                        -> reservationRescheduleController.newSelectedAdditionalService.getTblRoomService() != null
                        && reservationRescheduleController.newSelectedAdditionalService.getTblRoomService().getIdroomService() == 1
...
//        LocalDate dateCount = ((java.sql.Date) reservationController.selectedAdditionalService.getTblReservationRoomTypeDetail().getCheckInDateTime()).toLocalDate();
    ...
    private void setSelectedDataToInputForm() {
        ...
        Bindings.bindBidirectional(txtAdditionalCharge.textProperty(), reservationRescheduleController.newSelectedAdditionalService.priceProperty(), new ClassFormatter.CBigDecimalStringConverter());
...
//        chbUsedPeopleNumber.setSelected(peopleNumber.get() > 0);
        ...
        cbpService.valueProperty().bindBidirectional(reservationRescheduleController.newSelectedAdditionalService.tblRoomServiceProperty());

        cbpService.valueProperty().addListener((obs, olVal, newVal) -> {
            if (newVal != null) {
//                txtServiceName.setText(newVal.getServiceName());
                reservationRescheduleController.newSelectedAdditionalService.setPrice(newVal.getPrice());
            } else {
//                txtServiceName.setText("");
                reservationRescheduleController.newSelectedAdditionalService.setPrice(new BigDecimal("0"));
            }
            initDataTableDateSelected(reservationRescheduleController.newSelectedAdditionalService.getTblReservationRoomTypeDetail());
        });
        ...
        cbpRRTD.valueProperty().bindBidirectional(reservationRescheduleController.newSelectedAdditionalService.tblReservationRoomTypeDetailProperty());
    ...
    private void dataAdditionalServiceSaveHandle() {
        if (checkDataInputDataAdditionalService()) {
            ...
            if (alert.getResult() == ButtonType.OK) {
                reservationRescheduleController.newSelectedAdditionalServices = new ArrayList<>();
                ...
                for (RTDListDate dataDate : tabelAdditionalDate.getItems()) {
                    if (dataDate.getSelectedDate()) {
                        ...
                        dataAddtional.setTblReservationRoomTypeDetail(reservationRescheduleController.newSelectedAdditionalService.getTblReservationRoomTypeDetail());
                        dataAddtional.setTblRoomService(reservationRescheduleController.newSelectedAdditionalService.getTblRoomService());
                        dataAddtional.setPrice(reservationRescheduleController.newSelectedAdditionalService.getPrice());
                        dataAddtional.setAdditionalDate(Date.valueOf(dataDate.getDataDate()));
                        dataAddtional.setRefReservationBillType(new RefReservationBillType(reservationRescheduleController.newSelectedAdditionalService.getRefReservationBillType()));
                        dataAddtional.setCreateDate(Timestamp.valueOf(LocalDateTime.now()));
                        //set reservation additonal service - discount
                        setAdditionalServiceDiscount(dataAddtional, reservationRescheduleController.getParent().dataReservationBill);
                        if (chbUsedPeopleNumber.isSelected()) {
                            for (int i = 0; i < peopleNumber.get(); i++) {
                                ...
                                reservationRescheduleController.newSelectedAdditionalServices.add(intrDataAddtional);
                                reservationRescheduleController.newDataReservationAdditionalServices.add(intrDataAddtional);
                            }
                        } else {
                            reservationRescheduleController.newSelectedAdditionalServices.add(dataAddtional);
                            reservationRescheduleController.newDataReservationAdditionalServices.add(dataAddtional);
                        }
                    }
                }
        ...
        if (dataRCOBill.getRefReservationBillDiscountType() != null) {
            if (dataRCOBill.getRefReservationBillDiscountType().getIdtype() == 0) { //Hotel-Event = '0'
                if (dataAdditionalService.getTblRoomService().getHotelDiscountable()) {
                    List<TblHotelEvent> dataHotelEvents = reservationRescheduleController.getParent().getFReservationManager().getAllDataHotelEventByDateInRangeOrderByMaxServiceDiscountPercentage(dataAdditionalService.getAdditionalDate());
    ...
    private boolean checkDataInputDataAdditionalService() {
        ...
        if (txtAdditionalCharge.getText() == null
            ...
            if (reservationRescheduleController.newSelectedAdditionalService.getPrice().compareTo(new BigDecimal("0")) == -1) {
                dataInput = false;
    ...
    public RRReservationAdditionalServiceInputController(ReservationRescheduleController parentController) {
