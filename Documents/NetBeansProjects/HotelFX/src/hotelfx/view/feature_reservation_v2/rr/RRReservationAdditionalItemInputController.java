 ...
 */
public class RRReservationAdditionalItemInputController implements Initializable {
    ...
    private List<TblItem> loadAllDataItem() {
        List<TblItem> list;
            list = reservationRescheduleController.getParent().getFReservationManager().getAllDataItem();
            for (int i = list.size() - 1; i > -1; i--) {
                //data unit
                list.get(i).setTblUnit(reservationRescheduleController.getParent().getFReservationManager().getDataUnit(list.get(i).getTblUnit().getIdunit()));
    ...
    private List<TblReservationRoomTypeDetail> loadAllDataReservationRoomTypeDetail() {
        ...
        for(TblReservationRoomTypeDetail dataReservationRoomTypeDetail : reservationRescheduleController.newDataReservationRoomTypeDetails){
...
//        });
            ...
            while (dateCount.isBefore(dateLast)) {
                if (LocalDateTime.now().isBefore(
                        LocalDateTime.of(dateCount.plusDays(1), LocalTime.of(
                                        reservationRescheduleController.getParent().defaultCheckOutTime.getHours(),
                                        reservationRescheduleController.getParent().defaultCheckOutTime.getMinutes(),
                                        0)))) {
                    RTDListDate data;
                    if (dataItem != null
                            && dataItem.getPropertyStatus()) { //Property
    ...
    private void setSelectedDataToInputForm() {
        ...
        Bindings.bindBidirectional(txtAdditionalCharge.textProperty(), reservationRescheduleController.newSelectedAdditionalItem.itemChargeProperty(), new ClassFormatter.CBigDecimalStringConverter());
        Bindings.bindBidirectional(txtItemNumber.textProperty(), reservationRescheduleController.newSelectedAdditionalItem.itemQuantityProperty(), new ClassFormatter.CBigDecimalStringConverter());

        txtAvailableStock.setText("");
        txtAvailableStock.setVisible(false);
        lblUnit.setText("");

        reservationRescheduleController.newSelectedAdditionalItem.itemQuantityProperty().addListener((obs, oldVal, newVal) -> {
...
//            txtItemName.setText(newVal.getItemName());
            reservationRescheduleController.newSelectedAdditionalItem.setItemCharge(newVal != null ? newVal.getAdditionalCharge() : new BigDecimal("0"));
//            reservationController.selectedAdditionalItem.setItemQuantity(new BigDecimal("1"));
        ...
        cbpItem.valueProperty().bindBidirectional(reservationRescheduleController.newSelectedAdditionalItem.tblItemProperty());
        cbpRRTD.valueProperty().bindBidirectional(reservationRescheduleController.newSelectedAdditionalItem.tblReservationRoomTypeDetailProperty());
            ...
            if (cbpItem.getValue().getPropertyStatus()) { //Property
                ...
                for (RTDListDate data : tabelAdditionalDate.getItems()) {
                    if (data.getDataDate().equals(date)
                        ...
                        totalStockWantToReserved = reservationRescheduleController.newSelectedAdditionalItem.getItemQuantity() != null
                                ? reservationRescheduleController.newSelectedAdditionalItem.getItemQuantity() : new BigDecimal("0");
                        break;
                    }
                }
                ...
                for (RTDListDate data : tabelAdditionalDate.getItems()) {
                    if (data.getSelectedDate()) {
                        totalStockWantToReserved = totalStockWantToReserved.add(reservationRescheduleController.newSelectedAdditionalItem.getItemQuantity() != null
                                ? reservationRescheduleController.newSelectedAdditionalItem.getItemQuantity() : new BigDecimal("0"));
                    }
                }
...
//                ); //Item, Date(start), Date(end), Status = 'Reserved'
    ...
    private void dataAdditionalItemSaveHandle() {
        if (checkDataInputDataAdditionalItem()) {
            ...
            if (alert.getResult() == ButtonType.OK) {
                reservationRescheduleController.newSelectedAdditionalItems = new ArrayList<>();
                //save and set data additional item
                reservationRescheduleController.newSelectedAdditionalItem.setTaxable(reservationRescheduleController.newSelectedAdditionalItem.getTblItem().getTaxable());
                for (RTDListDate dataDate : tabelAdditionalDate.getItems()) {
                    if (dataDate.getSelectedDate()) {
                        ...
                        dataAddtional.setTblReservationRoomTypeDetail(reservationRescheduleController.newSelectedAdditionalItem.getTblReservationRoomTypeDetail());
                        dataAddtional.setTblItem(reservationRescheduleController.newSelectedAdditionalItem.getTblItem());
                        dataAddtional.setItemCharge(reservationRescheduleController.newSelectedAdditionalItem.getItemCharge());
                        dataAddtional.setItemQuantity(reservationRescheduleController.newSelectedAdditionalItem.getItemQuantity());
                        dataAddtional.setAdditionalDate(Date.valueOf(dataDate.getDataDate()));
                        dataAddtional.setRefReservationBillType(new RefReservationBillType(reservationRescheduleController.newSelectedAdditionalItem.getRefReservationBillType()));
                        dataAddtional.setCreateDate(Timestamp.valueOf(LocalDateTime.now()));
                        //set reservation additional item - discount
                        setAdditionalItemDiscount(dataAddtional, reservationRescheduleController.getParent().dataReservationBill);
                        dataAddtional.setTaxable(reservationRescheduleController.newSelectedAdditionalItem.getTaxable());
                        reservationRescheduleController.newSelectedAdditionalItems.add(dataAddtional);
                        reservationRescheduleController.newDataReservationAdditionalItems.add(dataAddtional);
                    }
                }
        ...
        if (dataRCOBill.getRefReservationBillDiscountType() != null) {
            if (dataRCOBill.getRefReservationBillDiscountType().getIdtype() == 0) { //Hotel-Event = '0'
                if (dataAdditionalItem.getTblItem().getHotelDiscountable()) {
                    List<TblHotelEvent> dataHotelEvents = reservationRescheduleController.getParent().getFReservationManager().getAllDataHotelEventByDateInRangeOrderByMaxItemDiscountPercentage(dataAdditionalItem.getAdditionalDate());
                ...
                if (dataRCOBill.getRefReservationBillDiscountType().getIdtype() == 1) { //Card-Event = '1'
                    if (dataAdditionalItem.getTblItem().getCardDiscountable()) {
                        List<TblBankEventCard> dataCardEvents = reservationRescheduleController.getParent().getFReservationManager().getAllDataBankEventCardByDateInRangeAndIDBankCardAllowNULLAndIDBankOrderByMaxItemDiscountPercentage(
    ...
    private boolean checkDataInputDataAdditionalItem() {
        ...
        if (txtItemNumber.getText() == null
            ...
            if (reservationRescheduleController.newSelectedAdditionalItem.getItemQuantity()
        ...
        if (txtAdditionalCharge.getText() == null
            ...
            if (reservationRescheduleController.newSelectedAdditionalItem.getItemCharge().compareTo(new BigDecimal("0")) == -1) {
                dataInput = false;
        ...
        if (cbpItem.getValue() != null) {
            if (cbpItem.getValue().getPropertyStatus()) {   //Property
                for (RTDListDate data : tabelAdditionalDate.getItems()) {
                    if (data.getSelectedDate()) {
                        BigDecimal totalCurrentStock = getTotalCurrentStock(cbpItem.getValue());    //total stock in room, warehouse, laundry
                        BigDecimal totalStockWantToReserved = reservationRescheduleController.newSelectedAdditionalItem.getItemQuantity();
                ...
                for (RTDListDate data : tabelAdditionalDate.getItems()) {
                    if (data.getSelectedDate()) {
                        totalStockWantToReserved = totalStockWantToReserved.add(reservationRescheduleController.newSelectedAdditionalItem.getItemQuantity());
                    }
                }
//                BigDecimal totalStockHasBeenReserved = getTotalStockHasBeenReservedInAfterDate(
//                        cbpItem.getValue(),
...
//                ); //Item, Date(start), Date(end), Status = 'Reserved'
    ...
    private BigDecimal getTotalCurrentStock(TblItem dataItem) {
        ...
        if (dataItem != null) {
            List<TblItemLocation> itemLocations = reservationRescheduleController.getParent().getFReservationManager().getAllDataItemLocationByIDItem(dataItem.getIditem());
    ...
    private BigDecimal getTotalStockHasBeenReserved(TblItem dataItem) {
        BigDecimal result = new BigDecimal("0");
        if (dataItem != null) {
            List<TblReservationAdditionalItemReserved> rairs = reservationRescheduleController.getParent().getFReservationManager().getAllDataReservationAdditionalItemReservedByIDItemAndIDRAIRStatus(
                    dataItem.getIditem(),
                    1); //Reserved = '1'
    ...
    private BigDecimal getTotalStockHasBeenReserved(TblItem dataItem,
        ...
        if (dataItem != null
                && date != null) {
            List<TblReservationAdditionalItemReserved> rairs = reservationRescheduleController.getParent().getFReservationManager().getAllDataReservationAdditionalItemReservedByIDItemAndReservedDateAndIDRAIRStatus(
                    dataItem.getIditem(),
                    Date.valueOf(date),
                    1); //Reserved = '1'
            for (TblReservationAdditionalItemReserved rair : rairs) {
    ...
    private BigDecimal getTotalStockMustBeStockInAllRoom(TblItem dataItem) {
        BigDecimal result = new BigDecimal("0");
        if (dataItem != null) {
            List<TblRoomType> roomTypes = reservationRescheduleController.getParent().getFReservationManager().getAllDataRoomType();
            for (TblRoomType roomType : roomTypes) {
                BigDecimal subResult = new BigDecimal("0");
                //Room Type - Item (in current situation)
                List<TblRoomTypeItem> roomTypeItems = reservationRescheduleController.getParent().getFReservationManager().getAllDataRoomTypeItemByIDRoomTypeAndIDItem(roomType.getIdroomType(), dataItem.getIditem());
                for (TblRoomTypeItem roomTypeItem : roomTypeItems) {
                    subResult = subResult.add(roomTypeItem.getItemQuantity());
                }
                List<TblRoom> rooms = reservationRescheduleController.getParent().getFReservationManager().getAllDataRoomByIDRoomType(roomType.getIdroomType());
...
//        }
    ...
    private BigDecimal getTotalStockHasBeenReservedInRangedDate(
        ...
        if (dataItem != null
                && startDate != null
                && endDate != null) {
            List<TblReservationAdditionalItemReserved> rairs = reservationRescheduleController.getParent().getFReservationManager().getAllDataReservationAdditionalItemReservedByIDItemAndReservedDateInRangedAndIDRAIRStatus(
                    dataItem.getIditem(),
    ...
    private BigDecimal getTotalStockHasBeenReservedInAfterDate(
        ...
        if (dataItem != null
                && date != null) {
            List<TblReservationAdditionalItemReserved> rairs = reservationRescheduleController.getParent().getFReservationManager().getAllDataReservationAdditionalItemReservedByIDItemAndReservedDateInAfterAndIDRAIRStatus(
                    dataItem.getIditem(),
                    Date.valueOf(date),
                    1); //Reserved = '1'
            for (TblReservationAdditionalItemReserved rair : rairs) {
    ...
    public RRReservationAdditionalItemInputController(ReservationRescheduleController parentController) {
