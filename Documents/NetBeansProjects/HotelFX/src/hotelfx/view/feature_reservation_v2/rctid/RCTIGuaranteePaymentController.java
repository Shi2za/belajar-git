...
public class RCTIGuaranteePaymentController implements Initializable {
    ...
    private void initDataRCTIPartnerPopup() {
        ...
        if (frptName != null) {
            switch (frptName) {
                case "Travel Agent":
                    partnerItems = FXCollections.observableArrayList(rcTransactionInputDetailController.getParentController().getParentController().getFReservationManager().getAllDataPartnerTravelAgent());
                    break;
                case "Guarantee Letter (Corporate)":
                    partnerItems = FXCollections.observableArrayList(rcTransactionInputDetailController.getParentController().getParentController().getFReservationManager().getAllDataPartnerCorporate());
                    break;
                case "Guarantee Letter (Government)":
                    partnerItems = FXCollections.observableArrayList(rcTransactionInputDetailController.getParentController().getParentController().getFReservationManager().getAllDataPartnerGovernment());
                    break;
                default:
                    break;
            }
...
//        gpRCTIGuaranteePayment.add(cbpReceiverBankAccount, 0, 4);
    ...
    private List<TblBankAccount> loadAllDataPartnerBankAccount() {
        ...
        if (rcTransactionInputDetailController.getParentController().getParentController().selectedDataTransactionWithGuaranteePayment.getTblPartner() != null) {
            List<TblPartnerBankAccount> pbaList = rcTransactionInputDetailController.getParentController().getParentController().getFReservationManager().getAllDataPartnerBankAccountByIDPartner(rcTransactionInputDetailController.getParentController().getParentController().selectedDataTransactionWithGuaranteePayment.getTblPartner().getIdpartner());
            for (TblPartnerBankAccount pbbaData : pbaList) {
                //data bank account
                TblBankAccount data = rcTransactionInputDetailController.getParentController().getParentController().getFReservationManager().getBankAccount(pbbaData.getTblBankAccount().getIdbankAccount());
    ...
    private List<TblBankAccount> loadAllDataHotelBankAccount() {
        List<TblCompanyBalanceBankAccount> cbbaList = rcTransactionInputDetailController.getParentController().getParentController().getFReservationManager().getAllDataCompanyBalanceBankAccount((long) 1);  //hotel balance = '1'
        List<TblBankAccount> list = new ArrayList<>();
...
//        }
    ...
    private void showPartnerBankAccountDialog() {
        try {
            ...
            loader.setLocation(HotelFX.class.getResource("view/feature_reservation_v2/rctid/RCTIGuaranteePaymentPartnerBankAccountInputDialog.fxml"));
