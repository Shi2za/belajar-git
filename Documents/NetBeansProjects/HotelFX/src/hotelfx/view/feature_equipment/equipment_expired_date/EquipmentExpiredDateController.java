 ...
 * @author ANDRI
 */
public class EquipmentExpiredDateController implements Initializable {

    /**
     * SPLITPANE BETWEEN TABLE AND FORM, CONTENT LAYOUT
     */
    @FXML
    private SplitPane spDataEquipmentExpiredDate;

    private DoubleProperty dataEquipmentExpiredDateFormShowStatus;

    @FXML
    private AnchorPane contentLayout;

    @FXML
    private AnchorPane tableDataEquipmentExpiredDateLayoutDisableLayer;

    private boolean isTimeLinePlaying = false;
    
    private void setDataEquipmentExpiredDateSplitpane() {
        spDataEquipmentExpiredDate.setDividerPositions(1);
        
        dataEquipmentExpiredDateFormShowStatus = new SimpleDoubleProperty(1.0);

        DoubleProperty divPosition = new SimpleDoubleProperty();

        divPosition.bind(new SimpleDoubleProperty(1.0)
                .subtract(dataEquipmentExpiredDateFormShowStatus)
        );

        divPosition.addListener((obs, oldVal, newVal) -> {
            isTimeLinePlaying = true;
            KeyValue keyValue = new KeyValue(spDataEquipmentExpiredDate.getDividers().get(0).positionProperty(), newVal);
        ...
        SplitPane.Divider div = spDataEquipmentExpiredDate.getDividers().get(0);
        ...
        dataEquipmentExpiredDateFormShowStatus.addListener((obs, oldVal, newVal) -> {
            tableDataEquipmentExpiredDateLayout.setDisable(false);
            tableDataEquipmentExpiredDateLayoutDisableLayer.setDisable(true);
            tableDataEquipmentExpiredDateLayout.toFront();
            if (dataInputStatus != 3) {
                ...
                if (newVal.doubleValue() == 1) {  //disable
                    tableDataEquipmentExpiredDateLayout.setDisable(true);
                    tableDataEquipmentExpiredDateLayoutDisableLayer.setDisable(false);
                    tableDataEquipmentExpiredDateLayoutDisableLayer.toFront();
                }
            }
        });

        dataEquipmentExpiredDateFormShowStatus.set(0.0);
    }
     ...
     */
    @FXML
    private AnchorPane tableDataEquipmentExpiredDateLayout;
    ...
    private ClassTableWithControl tableDataEquipmentExpiredDate;

    private void initTableDataEquipmentExpiredDate() {
        //set table
        setTableDataEquipmentExpiredDate();
        //set control
        setTableControlDataEquipmentExpiredDate();
        //set table-control to content-view
        ancBodyLayout.getChildren().clear();
        AnchorPane.setBottomAnchor(tableDataEquipmentExpiredDate, 15.0);
        AnchorPane.setLeftAnchor(tableDataEquipmentExpiredDate, 15.0);
        AnchorPane.setRightAnchor(tableDataEquipmentExpiredDate, 15.0);
        AnchorPane.setTopAnchor(tableDataEquipmentExpiredDate, 15.0);
        ancBodyLayout.getChildren().add(tableDataEquipmentExpiredDate);
    }

    private void setTableDataEquipmentExpiredDate() {
...
//        });
        ...
        tableDataEquipmentExpiredDate = new ClassTableWithControl(tableView);
        
        //set filter
        cft = new ClassFilteringTable<>(
                TblItemExpiredDate.class,
                tableDataEquipmentExpiredDate.getTableView(),
                tableDataEquipmentExpiredDate.getTableView().getItems());
...
//        AnchorPane.setLeftAnchor(cft, 15.0);
    ...
    private void setTableControlDataEquipmentExpiredDate() {
        ...
        tableDataEquipmentExpiredDate.addButtonControl(buttonControls);
    }
    ...
    private List<TblItemExpiredDate> loadAllDataItemExpiredDate() {
        ...
        List<TblItemExpiredDate> tempList = parentController.getFEquipmentManager().getAllDataItemExpiredDate();
...
//                data.setRefItemExpiredDateStatus(parentController.getFEquipmentManager().getDataItemExpiredDateStatus(data.getRefItemExpiredDateStatus().getIdstatus()));
                //data item (equipment)
                data.setTblItem(parentController.getFEquipmentManager().getEquipment(data.getTblItem().getIditem()));
                //data unit
                data.getTblItem().setTblUnit(parentController.getFEquipmentManager().getUnit(data.getTblItem().getTblUnit().getIdunit()));
    ...
    private double calculationTotalCurrentStock(TblItemExpiredDate itemExpiredDate) {
        ...
        if (itemExpiredDate != null) {
            List<TblItemExpiredDate> list = parentController.getFEquipmentManager().getAllDataItemExpiredDateByIDItemAndItemExpiredDateAndItemExpiredDateStatus(
    ...
    @FXML
    private GridPane gpFormDataEquipmentExpiredDate;

    @FXML
    private ScrollPane spFormDataEquipmentExpiredDate;
    ...
    private void initFormDataEquipmentExpiredDate() {
        isFormScroll.addListener((obs, wasSroll, nowScroll) -> {
            //@@@
            spFormDataEquipmentExpiredDate.pseudoClassStateChanged(onScrollPseudoClass, nowScroll);
        });

        //@@@
        gpFormDataEquipmentExpiredDate.setOnScroll((ScrollEvent scroll) -> {
...
//        gpFormDataEquipmentExpiredDate.setDisable(dataInputStatus == 3);
...
//        tableDataDetail.setItems(FXCollections.observableArrayList(loadAllDataItemExpiredDateDetail(dataIED)));
    ...
    private List<TblItemExpiredDate> loadAllDataItemExpiredDateDetail(TblItemExpiredDate dataIED) {
        ...
        if (dataIED != null
            ...
            list = parentController.getFEquipmentManager().getAllDataItemExpiredDateByIDItemAndItemExpiredDate(
...
//                data.setRefItemExpiredDateStatus(parentController.getFEquipmentManager().getDataItemExpiredDateStatus(data.getRefItemExpiredDateStatus().getIdstatus()));
    ...
    private void dataStockOpnameCreateHandle() {
        if (tableDataEquipmentExpiredDate.getTableView().getSelectionModel().getSelectedItems().size() == 1) {
            selectedDataItemExpiredDate = (TblItemExpiredDate) tableDataEquipmentExpiredDate.getTableView().getSelectionModel().getSelectedItem();
    ...
    private void dataItemExpiredDateShowHandle() {
        if (tableDataEquipmentExpiredDate.getTableView().getSelectionModel().getSelectedItems().size() == 1) {
            ...
            dataEquipmentExpiredDateFormShowStatus.set(1.0);
            isShowStatus.set(true);
        }
    }

    private void dataItemExpiredDateUnshowHandle() {
        tableDataEquipmentExpiredDate.getTableView().setItems(FXCollections.observableArrayList(loadAllDataItemExpiredDate()));
        dataEquipmentExpiredDateFormShowStatus.set(0);
    ...
    private void showDataItemExpiredDateStockOpnameDialog() {
        try {
            ...
            loader.setLocation(HotelFX.class.getResource("view/feature_equipment/equipment_expired_date/ItemExpiredDateStockOpnameDialog.fxml"));
    ...
    public void refreshData() {
        //data table
        refreshDataTableEquipmentExpiredDate();
        //data table detail
    ...
    public void refreshDataTableEquipmentExpiredDate(){
        tableDataEquipmentExpiredDate.getTableView().setItems(FXCollections.observableArrayList(loadAllDataItemExpiredDate()));
        cft.refreshFilterItems(tableDataEquipmentExpiredDate.getTableView().getItems());
    }
    ...
    public void initialize(URL url, ResourceBundle rb) {
        //set splitpane
        setDataEquipmentExpiredDateSplitpane();

        //init table
        initTableDataEquipmentExpiredDate();

        //init form
        initFormDataEquipmentExpiredDate();

        spDataEquipmentExpiredDate.widthProperty().addListener((obs, oldVal, newVal) -> {
            dataEquipmentExpiredDateFormShowStatus.set(0.0);
        });
    }    
    
    public EquipmentExpiredDateController(FeatureEquipmentController parentController) {
        this.parentController = parentController;
    }

    private final FeatureEquipmentController parentController;
    
    public FEquipmentManager getService(){
        return parentController.getFEquipmentManager();
    }
