...
public class DataOutController implements Initializable {
        ...
        cbpItem.valueProperty().addListener((obs, oldval, newVal) -> {
            ...
            if (newVal != null) {
                if (newVal.getPropertyStatus()) {   //Property
                    ...
                    roomCheckInputController.selectedDataOut.getTblItemMutationHistory().setItemQuantity(new BigDecimal("1"));
                    ...
                    if (newVal.getConsumable()) { //consumable
                        ...
                        roomCheckInputController.selectedDataOut.getTblItemMutationHistory().setItemQuantity(new BigDecimal("0"));
                        txtItemQuantity.setEditable(true);
                    }
                }
            ...
            if (newVal != null) {
                switch (newVal.getIdtype()) {
                    ...
                    case 2: //Rusak = '2'
                        ...
                        cbpLocationType.setValue(roomCheckInputController.getParentController().getService().getDataLocationType(4)); //Bin = '4'                        
                ...
                switch (newVal.getIdtype()) {
                    ...
                    case 4: //Bin = '4'
                        ...
                        if (cbpItemMutationType.getValue() != null
                            ...
                            cbpLocationOfBin.setValue(roomCheckInputController.getParentController().getService().getDataBin());
                        }
                        //Layout
    ...
    private List<TblItem> loadAllDataItem() {
        ...
        List<TblItemLocation> tempList = roomCheckInputController.getParentController().getService().getAllDataItemLocationByIDLocation(roomCheckInputController.selectedDataOut.getTblRoomCheck().getTblRoom().getTblLocation().getIdlocation());
        for (TblItemLocation tempData : tempList) {
            if ((tempData.getItemQuantity().compareTo(new BigDecimal("0")) == 1) //item quantity must be greater than '0'
                ...
                //data item
                TblItem data = roomCheckInputController.getParentController().getService().getDataItem(tempData.getTblItem().getIditem());
    ...
    private List<TblPropertyBarcode> loadAllDataPropertyBarcode(TblItem dataItem) {
        ...
        if (dataItem != null
            ...
            TblItemLocation itemLocation = roomCheckInputController.getParentController().getService().getDataItemLocationByIDLocationAndIDItem(
                    roomCheckInputController.getParentController().selectedData.getTblRoom().getTblLocation().getIdlocation(),
                    dataItem.getIditem());
    ...
    private List<RefItemMutationType> loadAllDataItemMutationType() {
        List<RefItemMutationType> list = roomCheckInputController.getParentController().getService().getAllDataItemMutationType();
    ...
    private List<RefItemObsoleteBy> loadAllDataItemObsoleteBy() {
        List<RefItemObsoleteBy> list = roomCheckInputController.getParentController().getService().getAllDataItemObsoleteBy();
        return list;
    }

    private List<RefLocationType> loadAllDataLocationType() {
        List<RefLocationType> list = roomCheckInputController.getParentController().getService().getAllDataLocationType();
        for (int i = list.size() - 1; i > -1; i--) {
    ...
    private List<TblRoom> loadAllDataRoom() {
        List<TblRoom> list = roomCheckInputController.getParentController().getService().getAllDataRoom();
        for (int i = list.size() - 1; i > -1; i--) {
    ...
    private List<TblLocationOfWarehouse> loadAllDataWarehouse() {
        List<TblLocationOfWarehouse> list = roomCheckInputController.getParentController().getService().getAllDataWarehouse();
        for (TblLocationOfWarehouse data : list) {
            data.setTblLocation(roomCheckInputController.getParentController().getService().getDataLocation(data.getTblLocation().getIdlocation()));
        }
        return list;
    }

    private List<TblLocationOfLaundry> loadAllDataLaundry() {
        List<TblLocationOfLaundry> list = roomCheckInputController.getParentController().getService().getAllDataLaundry();
    ...
    private List<TblLocationOfBin> loadAllDataBin() {
        List<TblLocationOfBin> list = roomCheckInputController.getParentController().getService().getAllDataBin();
    ...
    private void setSelectedDataToInputForm() {
        ...
        lblUnit.setText("");
        Bindings.bindBidirectional(txtItemQuantity.textProperty(), roomCheckInputController.selectedDataOut.getTblItemMutationHistory().itemQuantityProperty(), new ClassFormatter.CBigDecimalStringConverter());
    ...
    private void dataOutSaveHandle() {
        if (checkDataOutputDataOut()) {
            Alert alert = ClassMessage.showConfirmationSavingDataMessage("", roomCheckInputController.dialogStageOut);
            if (alert.getResult() == ButtonType.OK) {
                roomCheckInputController.selectedDataOut.getTblItemMutationHistory().setRefItemMutationType(cbpItemMutationType.getValue());
                if (cbpItemMutationType.getValue().getIdtype() == 2) {  //Rusak = '2'
                    roomCheckInputController.selectedDataOut.getTblItemMutationHistory().setRefItemObsoleteBy(cbpItemObsoleteBy.getValue());
                    TblLocationOfBin dataBin = roomCheckInputController.getParentController().getService().getDataBin();
                    if (dataBin != null) {
                        dataBin.setTblLocation(roomCheckInputController.getParentController().getService().getDataLocation(dataBin.getTblLocation().getIdlocation()));
                        roomCheckInputController.selectedDataOut.getTblItemMutationHistory().setTblLocationByIdlocationOfDestination(dataBin.getTblLocation());
                    }
                } else { //Moved
                    roomCheckInputController.selectedDataOut.getTblItemMutationHistory().setTblLocationByIdlocationOfDestination(getDestinationLocation());
                }
                roomCheckInputController.selectedDataOut.getTblItemMutationHistory().setTblItem(cbpItem.getValue());
                switch (roomCheckInputController.dataInputOutStatus) {
                    case 0:
                        ClassMessage.showSucceedAddingDataMessage("", roomCheckInputController.dialogStageOut);
                        //data rc - itmh
                        roomCheckInputController.tableDataOut.getTableView().getItems().add(roomCheckInputController.selectedDataOut);
                        //data itmh - pb
                        if (cbpItem.getValue().getPropertyStatus()) { //Property
                            ...
                            itemMutationHistoryPropertyBarcode.setTblItemMutationHistory(roomCheckInputController.selectedDataOut.getTblItemMutationHistory());
                        ...
                        if (cbpItem.getValue().getConsumable()) { //Consumable
                            ...
                            itemMutationHistoryItemExpiredDate.setTblItemMutationHistory(roomCheckInputController.selectedDataOut.getTblItemMutationHistory());
                        ...
                        //close form data - out
                        roomCheckInputController.dialogStageOut.close();
                        break;
                    case 1:
                        ClassMessage.showSucceedUpdatingDataMessage("", roomCheckInputController.dialogStageOut);
                        //data rc - itmh
                        roomCheckInputController.tableDataOut.getTableView().getItems().remove(roomCheckInputController.tableDataOut.getTableView().getSelectionModel().getSelectedItem());
                        roomCheckInputController.tableDataOut.getTableView().getItems().add(roomCheckInputController.selectedDataOut);
                        //data itmh - pb
                        for (int i = roomCheckInputController.getParentController().itemMutationHistoryPropertyBarcodes.size() - 1; i > -1; i--) {
                            if (roomCheckInputController.getParentController().itemMutationHistoryPropertyBarcodes.get(i).getTblItemMutationHistory().equals(roomCheckInputController.selectedDataOut.getTblItemMutationHistory())) {
                        ...
                        for (int i = roomCheckInputController.getParentController().itemMutationHistoryItemExpiredDates.size() - 1; i > -1; i--) {
                            if (roomCheckInputController.getParentController().itemMutationHistoryItemExpiredDates.get(i).getTblItemMutationHistory().equals(roomCheckInputController.selectedDataOut.getTblItemMutationHistory())) {
            ...
            ClassMessage.showWarningInputDataMessage(errDataInput, roomCheckInputController.dialogStageOut);
        }
    }
    ...
    private boolean checkDataOutputDataOut() {
        ...
        if (txtItemQuantity.getText() == null
            ...
            if (roomCheckInputController.selectedDataOut.getTblItemMutationHistory().getItemQuantity()
    ...
    public DataOutController(RoomCheckInputController parentController) {
        roomCheckInputController = parentController;
    }

    private final RoomCheckInputController roomCheckInputController;

//    public DataOutController(RoomStatusController parentController) {
//        roomCheckInputController.getParentController() = parentController;
//    }
//
//    private final RoomStatusController roomCheckInputController.getParentController();
}
