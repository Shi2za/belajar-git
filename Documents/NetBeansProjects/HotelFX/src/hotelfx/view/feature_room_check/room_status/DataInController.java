...
public class DataInController implements Initializable {
        ...
        cbpItem.valueProperty().addListener((obs, oldVal, newVal) -> {
            ...
            if (newVal != null) {
                if (newVal.getPropertyStatus()) {   //Property
                    ...
                    roomCheckInputController.selectedDataIn.getTblItemMutationHistory().setItemQuantity(new BigDecimal("1"));
                ...
                if (newVal.getConsumable()) {   //Consumable
                    ...
                    roomCheckInputController.selectedDataIn.getTblItemMutationHistory().setItemQuantity(new BigDecimal("0"));
                    txtItemQuantity.setEditable(true);
                }
    ...
    private List<TblItem> loadAllDataItem(DataFilter dataFilter) {
        ...
        if (dataFilter != null) {
            switch (dataFilter.getFilterName()) {
                case "Semua":
                    list = roomCheckInputController.getParentController().getService().getAllDataItem();
                    for (int i = list.size() - 1; i > -1; i--) {
                        if (!list.get(i).getGuestStatus()) {  //!Guest
                            list.remove(i);
                        } else {
                            //data unit
                            list.get(i).setTblUnit(roomCheckInputController.getParentController().getService().getDataUnit(list.get(i).getTblUnit().getIdunit()));
                            //data item type hk
                            if (list.get(i).getTblItemTypeHk() != null) {
                                list.get(i).setTblItemTypeHk(roomCheckInputController.getParentController().getService().getDataItemTypeHK(list.get(i).getTblItemTypeHk().getIditemTypeHk()));
                            }
                            //data item type wh
                            if (list.get(i).getTblItemTypeWh() != null) {
                                list.get(i).setTblItemTypeWh(roomCheckInputController.getParentController().getService().getDataItemTypeWH(list.get(i).getTblItemTypeWh().getIditemTypeWh()));
                            }
                        }
                    }
                    break;
                case "Tipe Kamar":
                    List<TblRoomTypeItem> roomTypeItems = roomCheckInputController.getParentController().getService().getAllDataRoomTypeItemByIDRoomType(roomCheckInputController.getParentController().selectedData.getTblRoom().getTblRoomType().getIdroomType());
                    for (TblRoomTypeItem roomTypeItem : roomTypeItems) {
                        if (roomTypeItem.getTblItem().getGuestStatus()) {  //Guest
                            //data item
                            TblItem data = roomCheckInputController.getParentController().getService().getDataItem(roomTypeItem.getTblItem().getIditem());
                            //data unit
                            data.setTblUnit(roomCheckInputController.getParentController().getService().getDataUnit(data.getTblUnit().getIdunit()));
                            //data item type hk
                            if (data.getTblItemTypeHk() != null) {
                                data.setTblItemTypeHk(roomCheckInputController.getParentController().getService().getDataItemTypeHK(data.getTblItemTypeHk().getIditemTypeHk()));
                            }
                            //data item type wh
                            if (data.getTblItemTypeWh() != null) {
                                data.setTblItemTypeWh(roomCheckInputController.getParentController().getService().getDataItemTypeWH(data.getTblItemTypeWh().getIditemTypeWh()));
                            }
                ...
                case "Reservasi Kamar":
                    TblReservationRoomTypeDetail previousDataRRTD = getPreviousRRTD(roomCheckInputController.getParentController().selectedData.getTblRoom(), Date.valueOf(LocalDate.now().minusDays(1)));
                    TblReservationRoomTypeDetail nextDataRRTD = getNextRRTD(roomCheckInputController.getParentController().selectedData.getTblRoom(), Date.valueOf(LocalDate.now()));
                    if (previousDataRRTD != null
                            && nextDataRRTD != null) {
                        if (previousDataRRTD.getIddetail()
                                == nextDataRRTD.getIddetail()) {
                            SysCurrentHotelDate schd = roomCheckInputController.getParentController().getService().getDataCurrentHotelDate(2); //House Keeping - Daily Cleaning = '2'
    ...
    private List<TblItem> getListItemByDataRRTDAndDate(
        ...
        List<TblReservationRoomTypeDetailItem> dataReservationRoomTypeDetailItems = roomCheckInputController.getParentController().getService().getAllDataReservationRoomTypeDetailItemByIDReservationRoomTypeDetail(dataRRTD.getIddetail());
        for (TblReservationRoomTypeDetailItem dataReservationRoomTypeDetailItem : dataReservationRoomTypeDetailItems) {
            ...
            if (!hasBeenAdded) {
                //data item
                TblItem data = roomCheckInputController.getParentController().getService().getDataItem(dataReservationRoomTypeDetailItem.getTblItem().getIditem());
                //data unit
                data.setTblUnit(roomCheckInputController.getParentController().getService().getDataUnit(data.getTblUnit().getIdunit()));
                //data item type hk
                if (data.getTblItemTypeHk() != null) {
        ...
        List<TblReservationAdditionalItem> dataReservationAdditionalItems = roomCheckInputController.getParentController().getService().getAllDataReservationAdditionalItemByIDReservationRoomTypeDetailAndAdditionalDate(
        ...
        for (TblReservationAdditionalItem dataReservationAdditionalItem : dataReservationAdditionalItems) {
            ...
            if (!hasBeenAdded) {
                //data item
                TblItem data = roomCheckInputController.getParentController().getService().getDataItem(dataReservationAdditionalItem.getTblItem().getIditem());
                //data unit
                data.setTblUnit(roomCheckInputController.getParentController().getService().getDataUnit(data.getTblUnit().getIdunit()));
                //data item type hk
                if (data.getTblItemTypeHk() != null) {
    ...
    private TblReservationRoomTypeDetail getPreviousRRTD(
        ...
        if (dataRoom != null
                && date != null) {
            List<TblReservationRoomPriceDetail> dataReservationRoomPriceDetails = roomCheckInputController.getParentController().getService().getAllDataReservationRoomPriceDetailByDetailDate(date);
            for (TblReservationRoomPriceDetail dataReservationRoomPriceDetail : dataReservationRoomPriceDetails) {
                TblReservationRoomTypeDetailRoomPriceDetail dataRelation = roomCheckInputController.getParentController().getService().getReservationRoomTypeDetailRoomPriceDetailByIDRoomPriceDetail(dataReservationRoomPriceDetail.getIddetail());
                if (dataRelation != null
                        && dataRelation.getTblReservationRoomTypeDetail() != null) {
                    TblReservationRoomTypeDetail rrtd = roomCheckInputController.getParentController().getService().getDataReservationRoomTypeDetail(dataRelation.getTblReservationRoomTypeDetail().getIddetail());
                    if (rrtd != null
                        ...
                        //data checkin-out
                        rrtd.setTblReservationCheckInOut(roomCheckInputController.getParentController().getService().getDataReservationCheckInOut(rrtd.getTblReservationCheckInOut().getIdcheckInOut()));
                        //data room
                        rrtd.getTblReservationCheckInOut().setTblRoom(roomCheckInputController.getParentController().getService().getDataRoom(rrtd.getTblReservationCheckInOut().getTblRoom().getIdroom()));
                        //data checkin/out - status
                        rrtd.getTblReservationCheckInOut().setRefReservationCheckInOutStatus(roomCheckInputController.getParentController().getService().getDataReservationCheckInOutStatus(rrtd.getTblReservationCheckInOut().getRefReservationCheckInOutStatus().getIdstatus()));
                        if (rrtd.getTblReservationCheckInOut().getTblRoom().getIdroom() == dataRoom.getIdroom()
                            ...
                            //data reservation
                            rrtd.setTblReservation(roomCheckInputController.getParentController().getService().getDataReservation(rrtd.getTblReservation().getIdreservation()));
                            //data reservation - status
                            rrtd.getTblReservation().setRefReservationStatus(roomCheckInputController.getParentController().getService().getDataReservationStatus(rrtd.getTblReservation().getRefReservationStatus().getIdstatus()));
    ...
    private List<RefLocationType> loadAllDataLocationType(TblItem dataItem) {
        ...
        if (dataItem != null) {
            list = roomCheckInputController.getParentController().getService().getAllDataLocationType();
            for (int i = list.size() - 1; i > -1; i--) {
                if (list.get(i).getIdtype() == 3 //Supplier = '3'
                    ...
                    List<TblItemLocation> itemLocations = roomCheckInputController.getParentController().getService().getAllDataItemLocationByIDLocationTypeAndItemQuantityNotZero(list.get(i).getIdtype());
    ...
    private List<TblRoom> loadAllDataRoom(TblItem dataItem) {
        ...
        if (dataItem != null) {
            list = roomCheckInputController.getParentController().getService().getAllDataRoom();
            for (int i = list.size() - 1; i > -1; i--) {
                if (list.get(i).getIdroom()
                        == roomCheckInputController.getParentController().selectedData.getTblRoom().getIdroom()) {
                    list.remove(i);
                } else {
                    list.get(i).setTblLocation(roomCheckInputController.getParentController().getService().getDataLocation(list.get(i).getTblLocation().getIdlocation()));
                    TblItemLocation itemLocation = roomCheckInputController.getParentController().getService().getDataItemLocationByIDLocationAndIDItem(list.get(i).getTblLocation().getIdlocation(), dataItem.getIditem());
    ...
    private List<TblLocationOfWarehouse> loadAllDataWarehouse(TblItem dataItem) {
        ...
        if (dataItem != null) {
            list = roomCheckInputController.getParentController().getService().getAllDataWarehouse();
            for (int i = list.size() - 1; i > -1; i--) {
                list.get(i).setTblLocation(roomCheckInputController.getParentController().getService().getDataLocation(list.get(i).getTblLocation().getIdlocation()));
                TblItemLocation itemLocation = roomCheckInputController.getParentController().getService().getDataItemLocationByIDLocationAndIDItem(list.get(i).getTblLocation().getIdlocation(), dataItem.getIditem());
                if (itemLocation == null
    ...
    private List<TblLocationOfLaundry> loadAllDataLaundry(TblItem dataItem) {
        ...
        if (dataItem != null) {
            list = roomCheckInputController.getParentController().getService().getAllDataLaundry();
            for (int i = list.size() - 1; i > -1; i--) {
                list.get(i).setTblLocation(roomCheckInputController.getParentController().getService().getDataLocation(list.get(i).getTblLocation().getIdlocation()));
                TblItemLocation itemLocation = roomCheckInputController.getParentController().getService().getDataItemLocationByIDLocationAndIDItem(list.get(i).getTblLocation().getIdlocation(), dataItem.getIditem());
                if (itemLocation == null
    ...
    private List<TblPropertyBarcode> loadAllDataPropertyBarcode(
        ...
        if (dataItem != null
                ...
                && dataLocation != null) {
            TblItemLocation itemLocation = roomCheckInputController.getParentController().getService().getDataItemLocationByIDLocationAndIDItem(
                    dataLocation.getIdlocation(),
                    dataItem.getIditem());
            if (itemLocation != null) {
                List<TblItemLocationPropertyBarcode> tempList = roomCheckInputController.getParentController().getService().getAllDataItemLocationPropertyBarcodeByIDItemLocation(itemLocation.getIdrelation());
                for (TblItemLocationPropertyBarcode tempData : tempList) {
                    //data property barcode
                    TblPropertyBarcode data = roomCheckInputController.getParentController().getService().getDataPropertyBarcode(tempData.getTblPropertyBarcode().getIdbarcode());
                    data.setTblItem(dataItem);
                    data.setTblFixedTangibleAsset(roomCheckInputController.getParentController().getService().getDataFixedTangibleAsset(data.getTblFixedTangibleAsset().getIdasset()));
    ...
    private List<TblItemExpiredDate> loadAllDataItemExpiredDate(
        ...
        if (dataItem != null
            ...
            TblItemLocation itemLocation = roomCheckInputController.getParentController().getService().getDataItemLocationByIDLocationAndIDItem(
                    ...
                    dataItem.getIditem());
            if (itemLocation != null) {
                List<TblItemLocationItemExpiredDate> tempList = roomCheckInputController.getParentController().getService().getAllDataItemLocationItemExpiredDateByIDItemLocation(itemLocation.getIdrelation());
                for (TblItemLocationItemExpiredDate tempData : tempList) {
                    //data item expired date
                    TblItemExpiredDate data = roomCheckInputController.getParentController().getService().getDataItemExpiredDate(tempData.getTblItemExpiredDate().getIditemExpiredDate());
                    data.setTblItem(dataItem);
...
//        cbpDataFilter.setItems(dataFilterItems);
    ...
    private void setSelectedDataToInputForm() {
        ...
        lblUnit.setText("");
        Bindings.bindBidirectional(txtItemQuantity.textProperty(), roomCheckInputController.selectedDataIn.getTblItemMutationHistory().itemQuantityProperty(), new ClassFormatter.CBigDecimalStringConverter());

        cbpPropertyBarcode.setVisible(false);
        cbpItemExpiredDate.setVisible(false);

//        cbpLocationType.valueProperty().bindBidirectional(roomCheckInputController.getParentController().selectedDataIn.getTblItemMutationHistory().getTblLocationByIdlocationOfSource().refLocationTypeProperty());
//
//        cbpItem.valueProperty().bindBidirectional(roomCheckInputController.getParentController().selectedDataIn.getTblItemMutationHistory().tblItemProperty());
        cbpItem.hide();
        cbpDataFilter.hide();
    ...
    private void dataInSaveHandle() {
        if (checkDataInputDataIn()) {
            Alert alert = ClassMessage.showConfirmationSavingDataMessage("", roomCheckInputController.dialogStageIn);
            if (alert.getResult() == ButtonType.OK) {
                roomCheckInputController.selectedDataIn.getTblItemMutationHistory().setTblLocationByIdlocationOfSource(getSourceLocation());
                roomCheckInputController.selectedDataIn.getTblItemMutationHistory().setTblItem(cbpItem.getValue());
                roomCheckInputController.selectedDataIn.getTblItemMutationHistory().setRefItemMutationType(roomCheckInputController.getParentController().getService().getDataMutationType(0));   //Dipindahkan '0'
                switch (roomCheckInputController.dataInputInStatus) {
                    case 0:
                        ClassMessage.showSucceedAddingDataMessage("", roomCheckInputController.dialogStageIn);
                        //data rc - itmh
                        roomCheckInputController.tableDataIn.getTableView().getItems().add(roomCheckInputController.selectedDataIn);
                        //data itmh - pb
                        if (cbpItem.getValue().getPropertyStatus()) { //Property
                            ...
                            itemMutationHistoryPropertyBarcode.setTblItemMutationHistory(roomCheckInputController.selectedDataIn.getTblItemMutationHistory());
                            itemMutationHistoryPropertyBarcode.setTblPropertyBarcode(cbpPropertyBarcode.getValue());
                            roomCheckInputController.getParentController().itemMutationHistoryPropertyBarcodes.add(itemMutationHistoryPropertyBarcode);
                        }
                        ...
                        if (cbpItem.getValue().getConsumable()) { //Consumable
                            ...
                            itemMutationHistoryItemExpiredDate.setTblItemMutationHistory(roomCheckInputController.selectedDataIn.getTblItemMutationHistory());
                            itemMutationHistoryItemExpiredDate.setTblItemExpiredDate(cbpItemExpiredDate.getValue());
                            roomCheckInputController.getParentController().itemMutationHistoryItemExpiredDates.add(itemMutationHistoryItemExpiredDate);
                        }
                        //close form data - in
                        roomCheckInputController.dialogStageIn.close();
                        break;
                    case 1:
                        ClassMessage.showSucceedUpdatingDataMessage("", roomCheckInputController.dialogStageIn);
                        //data rc - itmh
                        roomCheckInputController.tableDataIn.getTableView().getItems().remove(roomCheckInputController.tableDataIn.getTableView().getSelectionModel().getSelectedItem());
                        roomCheckInputController.tableDataIn.getTableView().getItems().add(roomCheckInputController.selectedDataIn);
                        //data itmh - pb
                        for (int i = roomCheckInputController.getParentController().itemMutationHistoryPropertyBarcodes.size() - 1; i > -1; i--) {
                            if (roomCheckInputController.getParentController().itemMutationHistoryPropertyBarcodes.get(i).getTblItemMutationHistory().equals(roomCheckInputController.selectedDataIn.getTblItemMutationHistory())) {
                                roomCheckInputController.getParentController().itemMutationHistoryPropertyBarcodes.remove(i);
                                break;
                            }
                        }
                        if (cbpItem.getValue().getPropertyStatus()) { //Property
                        ...
                        //data itmh - expired date
                        for (int i = roomCheckInputController.getParentController().itemMutationHistoryItemExpiredDates.size() - 1; i > -1; i--) {
                            if (roomCheckInputController.getParentController().itemMutationHistoryItemExpiredDates.get(i).getTblItemMutationHistory().equals(roomCheckInputController.selectedDataIn.getTblItemMutationHistory())) {
                                roomCheckInputController.getParentController().itemMutationHistoryItemExpiredDates.remove(i);
                                break;
                            }
                        }
                        if (cbpItem.getValue().getConsumable()) { //Consumable
            ...
            ClassMessage.showWarningInputDataMessage(errDataInput, roomCheckInputController.dialogStageIn);
        }
    }
    ...
    private boolean checkDataInputDataIn() {
        ...
        if (txtItemQuantity.getText() == null
            ...
            if (roomCheckInputController.selectedDataIn.getTblItemMutationHistory().getItemQuantity()
    ...
    public DataInController(RoomCheckInputController parentController) {
        roomCheckInputController = parentController;
    }

    private final RoomCheckInputController roomCheckInputController;
