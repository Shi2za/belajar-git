 ...
 * @author ANDRI
 */
public class FReservationManagerImpl implements FReservationManager {
    
    private Session session;
    
    private String errMsg;
    
    public FReservationManagerImpl() {
...
//            //data checkout-bill
//            session.saveOrUpdate(checkOutBill);
    ...
    public TblReservation insertDataReservation(TblReservation reservation,
        ...
        if (ClassSession.checkUserSession()) {  //check user current session
            try {
                ...
                dataLog += "-----------------------------------------------------------------------\n";
                //data checkout-bill
                checkOutBill.setCodeBill(ClassCoder.getCode("Reservation Bill", session));
                checkOutBill.setCreateDate(Timestamp.valueOf(LocalDateTime.now()));
                checkOutBill.setTblEmployeeByCreateBy(session.find(TblEmployee.class, ClassSession.currentUser.getTblEmployeeByIdemployee().getIdemployee()));
                checkOutBill.setLastUpdateDate(Timestamp.valueOf(LocalDateTime.now()));
                checkOutBill.setTblEmployeeByLastUpdateBy(session.find(TblEmployee.class, ClassSession.currentUser.getTblEmployeeByIdemployee().getIdemployee()));
                checkOutBill.setRefRecordStatus(session.find(RefRecordStatus.class, 1));
                session.saveOrUpdate(checkOutBill);
                //data log
                dataLog += "No. Tagihan (Check-Out) : " + checkOutBill.getCodeBill() + "\n";
                dataLog += "Service Charge (%) : " + ClassFormatter.decimalFormat.cFormat((checkOutBill.getServiceChargePercentage().multiply(new BigDecimal("100")))) + "% \n";
                dataLog += "Pajak (%) : " + ClassFormatter.decimalFormat.cFormat((checkOutBill.getTaxPercentage().multiply(new BigDecimal("100")))) + "% \n";
                dataLog += "Tipe Diskon : " + (checkOutBill.getRefReservationBillDiscountType() != null ? checkOutBill.getRefReservationBillDiscountType().getTypeName() : "-") + " \n";
                dataLog += "-----------------------------------------------------------------------\n";
...
//        LocalDate dateCount = ((java.sql.Date) reservationRoomTypeDetail.getCheckInDateTime()).toLocalDate();
...
//                        }
                        ...
                        if (reservationPayment.getRefFinanceTransactionPaymentType().getIdtype() > (long) 0
                            ...
                            if (companyBalanceBankAccount != null) {
                                ...
                                logCompanyBalanceCashFlow.setTblCompanyBalanceByIdsenderCompanyBalance(null);
                                logCompanyBalanceCashFlow.setTblBankAccountByIdsenderCbbankAccount(customerBankAccount);
                                logCompanyBalanceCashFlow.setTblCompanyBalanceByIdreceiverCompanyBalance(session.find(TblCompanyBalance.class, (long) 3));
                ...
                if (tblReservationPayment.getRefFinanceTransactionPaymentType().getIdtype() > (long) 0
                    ...
                    if (tblReservationPayment.getRefFinanceTransactionPaymentType().getIdtype() == (long) 16) {
                        result += "Pengurangan Kas karena adanya Transaksi Pengembalian Pembayaran Reservasi \n No. "
                                ...
                                + "\n";
                        result += "Nominal pembayaran transaksi reservasi : " + ClassFormatter.currencyFormat.cFormat(tblReservationPayment.getUnitNominal());
//                        result += "Lainnya (Pengembalian) : Transaksi Reservasi No. "
//                                + tblReservationPayment.getCodePayment()
//                                + " (" + tblReservationPayment.getRefFinanceTransactionPaymentType().getTypeName() + ")"
//                                + "\n";
//                        result += "Nominal transaksi reservasi : " + ClassFormatter.currencyFormat.cFormat(tblReservationPayment.getUnitNominal());
...
//                    * reservationRoomTypeDetail.getTblReservationCheckInOut().getBrokenCardCharge());
                ...
                logCompanyBalanceCashFlow.setTblCompanyBalanceByIdsenderCompanyBalance(null);
                logCompanyBalanceCashFlow.setTblBankAccountByIdsenderCbbankAccount(null);
                logCompanyBalanceCashFlow.setTblCompanyBalanceByIdreceiverCompanyBalance(dataDepositBalance);
                logCompanyBalanceCashFlow.setTblBankAccountByIdreceiverCbbankAccount(null);
                logCompanyBalanceCashFlow.setTransferNominal(dataReservationPayment.getUnitNominal());
                logCompanyBalanceCashFlow.setHistoryDate(Timestamp.valueOf(LocalDateTime.now()));
                logCompanyBalanceCashFlow.setTblEmployee(session.find(TblEmployee.class, ClassSession.currentUser.getTblEmployeeByIdemployee().getIdemployee()));
                ...
                if (isAllCheckOut(reservation)
                    ...
                    //data reservation payment (cash) with nominal transaction = '0' - insert
                    TblReservationPayment dataReservationPaymentForCheckOut = new TblReservationPayment();
                    dataReservationPaymentForCheckOut.setTblReservationBill(checkOutBill);
                    dataReservationPaymentForCheckOut.setCodePayment(ClassCoder.getCode("Reservation Payment", session));
                    dataReservationPaymentForCheckOut.setPaymentDate(Timestamp.valueOf(LocalDateTime.now()));
                    dataReservationPaymentForCheckOut.setUnitNominal(new BigDecimal("0"));
                    dataReservationPaymentForCheckOut.setRoundingValue(new BigDecimal("0"));
                    dataReservationPaymentForCheckOut.setTblEmployeeByIdcashier(session.find(TblEmployee.class, ClassSession.currentUser.getTblEmployeeByIdemployee().getIdemployee()));
                    dataReservationPaymentForCheckOut.setRefFinanceTransactionPaymentType(session.find(RefFinanceTransactionPaymentType.class, 0));     //Tunai = '0'
                    dataReservationPaymentForCheckOut.setRefReservationBillType(session.find(RefReservationBillType.class, 1));     //Check Out = '1'
                    dataReservationPaymentForCheckOut.setCreateDate(Timestamp.valueOf(LocalDateTime.now()));
                    dataReservationPaymentForCheckOut.setTblEmployeeByCreateBy(session.find(TblEmployee.class, ClassSession.currentUser.getTblEmployeeByIdemployee().getIdemployee()));
                    dataReservationPaymentForCheckOut.setLastUpdateDate(Timestamp.valueOf(LocalDateTime.now()));
                    dataReservationPaymentForCheckOut.setTblEmployeeByLastUpdateBy(session.find(TblEmployee.class, ClassSession.currentUser.getTblEmployeeByIdemployee().getIdemployee()));
                    dataReservationPaymentForCheckOut.setRefRecordStatus(session.find(RefRecordStatus.class, 1));
                    session.saveOrUpdate(dataReservationPaymentForCheckOut);
                    ...
                    session.update(reservation);
                    //data log
                    headerLog = "BUAT DATA TRANSAKSI PEMBAYARAN RESERVASI";
                    dataLog = "";
                    dataLog += "No. Reservasi : " + reservation.getCodeReservation() + "\n";
                    dataLog += "Tgl. Reservasi : " + ClassFormatter.dateFormate.format(reservation.getReservationDate()) + "\n";
                    dataLog += "Nama Customer : " + reservation.getTblCustomer().getTblPeople().getFullName() + "\n";
                    dataLog += "Travel Agent : " + (reservation.getTblPartner() != null ? reservation.getTblPartner().getPartnerName() : "-") + "\n";
                    dataLog += "-----------------------------------------------------------------------\n";
                    dataLog += "No. Tagihan (" + checkOutBill.getRefReservationBillType().getTypeName() + ") : " + checkOutBill.getCodeBill() + "\n";
                    dataLog += "Service Charge (%) : " + ClassFormatter.decimalFormat.cFormat((checkOutBill.getServiceChargePercentage().multiply(new BigDecimal("100")))) + "% \n";
                    dataLog += "Pajak (%) : " + ClassFormatter.decimalFormat.cFormat((checkOutBill.getTaxPercentage().multiply(new BigDecimal("100")))) + "% \n";
                    dataLog += "Tipe Diskon : " + (checkOutBill.getRefReservationBillDiscountType() != null ? checkOutBill.getRefReservationBillDiscountType().getTypeName() : "-") + " \n";
                    dataLog += "-----------------------------------------------------------------------\n";
                    dataLog += "Transaksi Pembayaran : \n";
                    dataLog += "- " + dataReservationPaymentForCheckOut.getCodePayment() + " - " + ClassFormatter.dateTimeFormate.format(dataReservationPaymentForCheckOut.getPaymentDate()) + " \t "
                            + "Nominal : " + ClassFormatter.currencyFormat.cFormat(dataReservationPaymentForCheckOut.getUnitNominal()) + " \t "
                            + "Tipe : " + dataReservationPaymentForCheckOut.getRefFinanceTransactionPaymentType().getTypeName() + " \n";
                    dataLog += "-----------------------------------------------------------------------\n";
                    LogSystem checkOutLogSystem = new LogSystem();
                    checkOutLogSystem.setLogDateTime(Timestamp.valueOf(LocalDateTime.now()));
                    checkOutLogSystem.setTblSystemUser(session.find(TblSystemUser.class, ClassSession.currentUser.getIduser()));
                    checkOutLogSystem.setTblEmployee(session.find(TblEmployee.class, ClassSession.currentUser.getTblEmployeeByIdemployee().getIdemployee()));
                    checkOutLogSystem.setLogHeader(headerLog);
                    checkOutLogSystem.setLogDetail(dataLog);
                    checkOutLogSystem.setHostName(ClassComp.getHostName());
                    session.saveOrUpdate(checkOutLogSystem);
                    //open close company balance - detail (cashier)
                    if (dataOpenCloseCashierBalance != null) {
                        TblOpenCloseCashierBalanceDetail dataOpenCloseCashierBalanceDetailCheckOut = new TblOpenCloseCashierBalanceDetail();
                        dataOpenCloseCashierBalanceDetailCheckOut.setTblOpenCloseCashierBalance(dataOpenCloseCashierBalance);
                        dataOpenCloseCashierBalanceDetailCheckOut.setTblReservationPayment(dataReservationPaymentForCheckOut);
                        dataOpenCloseCashierBalanceDetailCheckOut.setDetailData(getDetailData(dataReservationPaymentForCheckOut));
                        dataOpenCloseCashierBalanceDetailCheckOut.setCreateDate(Timestamp.valueOf(LocalDateTime.now()));
                        dataOpenCloseCashierBalanceDetailCheckOut.setTblEmployeeByCreateBy(session.find(TblEmployee.class, ClassSession.currentUser.getTblEmployeeByIdemployee().getIdemployee()));
                        dataOpenCloseCashierBalanceDetailCheckOut.setLastUpdateDate(Timestamp.valueOf(LocalDateTime.now()));
                        dataOpenCloseCashierBalanceDetailCheckOut.setTblEmployeeByLastUpdateBy(session.find(TblEmployee.class, ClassSession.currentUser.getTblEmployeeByIdemployee().getIdemployee()));
                        dataOpenCloseCashierBalanceDetailCheckOut.setRefRecordStatus(session.find(RefRecordStatus.class, 1));
                        session.saveOrUpdate(dataOpenCloseCashierBalanceDetailCheckOut);
                    }
                }
                session.getTransaction().commit();
            } catch (Exception e) {
    ...
    private RefReservationBillType getReservationBillTypeByTypeName(String typeName) {
        //@@@---
        List<RefReservationBillType> list = session.getNamedQuery("findAllRefReservationBillTypeByTypeName")
    ...
    private List<TblItemLocation> getAllItemLocationByIDItemAndNotBin(long idItem) {
        List<TblItemLocation> list = session.getNamedQuery("findAllTblItemLocationByIdItem")
                .setParameter("idItem", idItem)
                .list();
        TblLocationOfBin dataBin = getBin();
        //remove data location item : bin
        if (!list.isEmpty() && dataBin != null) {
            for (int i = list.size() - 1; i > -1; i--) {
                if (list.get(i).getTblLocation().getIdlocation()
                        == dataBin.getTblLocation().getIdlocation()) {
                    list.remove(i);
                }
            }
        }
        session.evict(dataBin);
        return list;
    }
    ...
    public boolean updateDataReservationRoomTypeDetailChangeRoom(
        ...
        if (ClassSession.checkUserSession()) {  //check user current session
            try {
                ...
                reservation.setTblEmployeeByLastUpdateBy(session.find(TblEmployee.class, ClassSession.currentUser.getTblEmployeeByIdemployee().getIdemployee()));
                session.update(reservation);
//                //data guarantee letter - item detail (old, if previouse data use 'reservation payment with guarantee payment')
//                LocalDateTime currentDate = currentDateForChangeRoomProcess;
//                LocalDateTime maxDate = LocalDateTime.of(reservationRoomTypeDetail.getCheckOutDateTime().getYear() + 1900,
...
//                System.out.println("true : " + currentDate.isBefore(maxDate));
//                while (currentDate.isBefore(maxDate)) {
//                    System.out.println("bbb");
//                    String code = reservationRoomTypeDetail.getCodeDetail();
//                    //find code
//                    for (TblGuaranteeLetterItemDetail guaranteeLetterItemDetail : guaranteeLetterItemDetails) {
//                        if (currentDate.toLocalDate().equals((LocalDateTime.of(guaranteeLetterItemDetail.getCheckInDateTime().getYear() + 1900,
//                                guaranteeLetterItemDetail.getCheckInDateTime().getMonth() + 1,
//                                guaranteeLetterItemDetail.getCheckInDateTime().getDate(),
//                                guaranteeLetterItemDetail.getCheckInDateTime().getHours(),
//                                guaranteeLetterItemDetail.getCheckInDateTime().getMinutes(),
//                                0)).toLocalDate())) {
//                            code = guaranteeLetterItemDetail.getCodeRtd();
//                            System.out.println("bb2222");
//                            break;
//                        }
//                    }
//                    System.out.println("ccc");
//                    //set code
//                    List<TblGuaranteeLetterItemDetail> list = getAllDataGuaranteeLetterItemDetailByIDReservationAndCodeRTDAndCheckInDateTimeInDate(
...
//                    for (TblGuaranteeLetterItemDetail data : list) {
//                        data.setCodeRtd(code);
...
//                }
                ...
                if (LocalDateTime.of(reservationRoomTypeDetail.getCheckInDateTime().getYear() + 1900,
                    ...
                    if (reservationRoomTypeDetail.getTblReservationCheckInOut() == null
                        ...
                        dataCheckInOut.setRefReservationCheckInOutStatus(session.find(RefReservationCheckInOutStatus.class, 3)); //Checked Out = '3'
    ...
    public TblReservationCanceled insertDataReservationCanceled(
        ...
        if (ClassSession.checkUserSession()) {  //check user current session
            try {
                ...
                //data reservation payment (canceling fee)
                reservationPayment.setCodePayment(ClassCoder.getCode("Reservation Payment", session));
                reservationPayment.setCreateDate(Timestamp.valueOf(LocalDateTime.now()));
                reservationPayment.setTblEmployeeByCreateBy(session.find(TblEmployee.class, ClassSession.currentUser.getTblEmployeeByIdemployee().getIdemployee()));
                reservationPayment.setLastUpdateDate(Timestamp.valueOf(LocalDateTime.now()));
