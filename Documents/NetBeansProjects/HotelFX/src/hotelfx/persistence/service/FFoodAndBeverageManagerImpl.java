 ...
 * @author ANDRI
 */
public class FFoodAndBeverageManagerImpl implements FFoodAndBeverageManager {

    private Session session;

    private String errMsg;

    public FFoodAndBeverageManagerImpl() {
    ...
    public TblItem insertDataFoodAndBeverage(TblItem fab) {
        ...
        if (ClassSession.checkUserSession()) {  //check user current sessio
            try {
                ...
                tblItem.setCodeItem(ClassCoder.getCode("Food And Beverage", session));
                ...
                //data item - location (food & beverage - all location(warehouse, laundry, room))
    ...
    public boolean updateDataFoodAndBeverage(TblItem fab) {
        ...
        if (ClassSession.checkUserSession()) {  //check user current sessio
            try {
                session = HibernateUtil.getSessionFactory().getCurrentSession();
                session.beginTransaction();
                fab.setLastUpdateDate(Timestamp.valueOf(LocalDateTime.now()));
                fab.setTblEmployeeByLastUpdateBy(session.find(TblEmployee.class, ClassSession.currentUser.getTblEmployeeByIdemployee().getIdemployee()));
                session.update(fab);
                session.getTransaction().commit();
            } catch (Exception e) {
                if (session.getTransaction().isActive()) {
    ...
    public boolean deleteDataFoodAndBeverage(TblItem fab) {
        ...
        if (ClassSession.checkUserSession()) {  //check user current sessio
            try {
                session = HibernateUtil.getSessionFactory().getCurrentSession();
                session.beginTransaction();
                if (fab.getRefRecordStatus().getIdstatus() == 1) {
                    fab.setLastUpdateDate(Timestamp.valueOf(LocalDateTime.now()));
                    fab.setTblEmployeeByLastUpdateBy(session.find(TblEmployee.class, ClassSession.currentUser.getTblEmployeeByIdemployee().getIdemployee()));
                    fab.setRefRecordStatus(session.find(RefRecordStatus.class, 2));
                    session.update(fab);
                } else {
                    if (session.getTransaction().isActive()) {
                        session.getTransaction().rollback();
