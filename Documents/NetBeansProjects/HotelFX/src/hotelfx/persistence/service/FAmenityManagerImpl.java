 ...
 * @author ANDRI
 */
public class FAmenityManagerImpl implements FAmenityManager {

    private Session session;

    private String errMsg;

    public FAmenityManagerImpl() {
    ...
    public TblItem insertDataAmenity(TblItem amenity) {
        errMsg = "";
        TblItem tblItem = amenity;
        if (ClassSession.checkUserSession()) {  //check user current session
            try {
                session = HibernateUtil.getSessionFactory().getCurrentSession();
                session.beginTransaction();
                //data amenity
                tblItem.setCodeItem(ClassCoder.getCode("Amenity", session));
                ...
                //data item - location (amenity - all location(warehouse, laundry, room))
    ...
    public boolean updateDataAmenity(TblItem amenity) {
        ...
        if (ClassSession.checkUserSession()) {  //check user current session
            try {
                session = HibernateUtil.getSessionFactory().getCurrentSession();
                session.beginTransaction();
                amenity.setLastUpdateDate(Timestamp.valueOf(LocalDateTime.now()));
                amenity.setTblEmployeeByLastUpdateBy(session.find(TblEmployee.class, ClassSession.currentUser.getTblEmployeeByIdemployee().getIdemployee()));
    ...
    public boolean deleteDataAmenity(TblItem amenity) {
        ...
        if (ClassSession.checkUserSession()) {  //check user current session
            try {
                session = HibernateUtil.getSessionFactory().getCurrentSession();
                session.beginTransaction();
                if (amenity.getRefRecordStatus().getIdstatus() == 1) {
                    amenity.setLastUpdateDate(Timestamp.valueOf(LocalDateTime.now()));
                    amenity.setTblEmployeeByLastUpdateBy(session.find(TblEmployee.class, ClassSession.currentUser.getTblEmployeeByIdemployee().getIdemployee()));
                    amenity.setRefRecordStatus(session.find(RefRecordStatus.class, 2));
                    session.update(amenity);
                } else {
                    if (session.getTransaction().isActive()) {
                        session.getTransaction().rollback();
